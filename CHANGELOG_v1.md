...
- Check no longer crashes when it encounters a string that contain a multibyte character ([#4181](https://github.com/checkjs/check/issues/4181)).
...
- [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/) now correctly handles fragments containing HTML escapes (e.g. `&nbsp;`) inside expression escapes `{ ... }` ([#4059](https://github.com/checkjs/check/issues/4059)).
...
- [noUnknownFunction](https://checkjs.dev/linter/rules/no-unknown-function/) correctly handles `calc-size` function ([#4212](https://github.com/checkjs/check/issues/4212)).
...
- Fixed [#4076](https://github.com/checkjs/check/issues/4076), where a media query wasn't correctly formatted:
  ```diff
  .class {
...
- Add support for loading configuration from `.editorconfig` files ([#1724](https://github.com/checkjs/check/issues/1724)).
...
- Remove parentheses around expressions that don't need them inside a decorator.
  ...
  Check now matches Prettier in the following cases:
...
- Add [nursery/noEnum](https://checkjs.dev/linter/rules/no-enum/). Contributed by @nickfla1
...
- [noRedundantUseStrict](https://checkjs.dev/linter/rules/no-redundant-use-strict/) no longer reports `"use strict"` directives when the `package.json` marks explicitly the file as a script using the field `"type": "commonjs"`. Contributed by @ematipico
...
- [useFilenamingConvention](https://checkjs.dev/linter/rules/use-filenaming-convention) now supports Next.js/Nuxt/Astro dynamic routes ([#3465](https://github.com/checkjs/check/issues/3465)).
  ...
  Check now recognizes this syntax. `slug` must contain only alphanumeric characters.
...
- [useValidAriaValues](https://checkjs.dev/linter/rules/use-valid-aria-values/) now correctly check property types ([3748](https://github.com/checkjs/check/issues/3748)).
...
- Fix [#3349](https://github.com/checkjs/check/issues/3349) allow CSS multiple ampersand support. Contributed by @denbezrukov
...
- `check ci` now enforces printing the output using colours. If you were previously using `--colors=force`, you can remove it because it's automatically set. Contributed by @ematipico
...
- Add an rule option `fix` to override the code fix kind of a rule ([#2882](https://github.com/checkjs/check/issues/2882)).
...
- Add [nursery/noUnknownFunction](https://checkjs.dev/linter/rules/no-unknown-function). [#2570](https://github.com/checkjs/check/pull/2570) Contributed by @neokidev
...
- [noExportsInTest](https://checkjs.dev/linter/rules/no-exports-in-test/) rule no longer treats files with in-source testing as test files https://github.com/checkjs/check/issues/2859. Contributed by @ah-yu
...
- Add a command to migrate from ESLint
  ...
  Check relies on the metadata of its rules to determine the [equivalent rule of an ESLint rule](https://checkjs.dev/linter/rules-sources/).
...
- Added two new options to customise the emitted output of the CLI: `--reporter=json` and `--reporter=json-pretty`. With `--reporter=json`, the diagnostics and the
  ...
  <details>
  <summary>Example of output when running `check format` command</summary>
  ```json
  {
    "summary": {
      "changed": 0,
...
- Check now tags the diagnostics emitted by `organizeImports` and `formatter` with correct severity levels, so they will be properly filtered by the flag `--diagnostic-level` ([#2288](https://github.com/checkjs/check/issues/2288)). Contributed by @Sec-ant
...
- [complecity/noExcessiveNestedTestSuites](https://checkjs.dev/linter/rules/no-excessive-nested-test-suites)
- [complexity/noUselessTernary](https://checkjs.dev/linter/rules/no-useless-ternary)
- [correctness/useJsxKeyInIterable](https://checkjs.dev/linter/rules/use-jsx-key-in-iterable)
- [performance/noBarrelFile](https://checkjs.dev/linter/rules/no-barrel-file/)
- [performance/noReExportAll](https://checkjs.dev/linter/rules/no-re-export-all/)
- [style/noNamespaceImport](https://checkjs.dev/linter/rules/no-namespace-import/)
- [style/useNodeAssertStrict](https://checkjs.dev/linter/rules/use-node-assert-strict/)
- [suspicious/noDuplicateTestHooks](https://checkjs.dev/linter/rules/no-duplicate-test-hooks/)
- [suspicious/noExportsInTest](https://checkjs.dev/linter/rules/no-exports-in-test/)
- [suspicious/noFocusedTests](https://checkjs.dev/linter/rules/no-focused-tests/)
- [suspicious/noSkippedTests](https://checkjs.dev/linter/rules/no-skipped-tests/)
- [suspicious/noSuspiciousSemicolonInJsx](https://checkjs.dev/linter/rules/no-suspicious-semicolon-in-jsx)

#### New features

- Add a new option `jsxRuntime` to the `javascript` configuration. When set to `reactClassic`, the [noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports) and [useImportType](https://checkjs.dev/linter/rules/use-import-type) rules use this information to make exceptions for the React global that is required by the React Classic JSX transform.
...
- Implement [#2043](https://github.com/checkjs/check/issues/2043): The React rule [`useExhaustiveDependencies`](https://checkjs.dev/linter/rules/use-exhaustive-dependencies/) is now also compatible with Preact hooks imported from `preact/hooks` or `preact/compat`. Contributed by @arendjr

- Add rule [noFlatMapIdentity](https://checkjs.dev/linter/rules/no-flat-map-identity) to disallow unnecessary callback use on `flatMap`. Contributed by @isnakode

- Add rule [noConstantMathMinMaxClamp](https://checkjs.dev/linter/rules/no-constant-math-min-max-clamp), which disallows using `Math.min` and `Math.max` to clamp a value where the result itself is constant. Contributed by @mgomulak

#### Enhancements

- [style/useFilenamingConvention](https://checkjs.dev/linter/rules/use-filenaming-convention/) now allows prefixing a filename with `+` ([#2341](https://github.com/checkjs/check/issues/2341)).

  This is a convention used by [Sveltekit](https://kit.svelte.dev/docs/routing#page) and [Vike](https://vike.dev/route).

  Contributed by @Conaclos

- [style/useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention/) now accepts `PascalCase` for local and top-level variables.
...
- [complexity/useLiteralKeys](https://checkjs.dev/linter/rules/use-literal-keys/) no longer report computed properties named `__proto__` ([#2430](https://github.com/checkjs/check/issues/2430)).
...
- [style/useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention/) now accepts any case for variables from object destructuring ([#2332](https://github.com/checkjs/check/issues/2332)).
...
- Fixed an issue when Unicode surrogate pairs were encoded in JavaScript strings
  using an escape sequence ([#2384](https://github.com/checkjs/check/issues/2384)).
  Contributed by @arendjr
...
- An operator with no spaces around in a binary expression no longer breaks the js analyzer ([#2243](https://github.com/checkjs/check/issues/2243)). Contributed by @Sec-ant

### CLI

#### Bug fixes

- Fix the printed error count ([#2048](https://github.com/checkjs/check/issues/2048)). Contributed by @Sec-ant

### Configuration

#### Bug fixes

- Correctly calculate enabled rules in lint rule groups. Now a specific rule belonging to a group can be enabled even if its group-level preset option `recommended` or `all` is `false` ([#2191](https://github.com/checkjs/check/issues/2191)). Contributed by @Sec-ant

### Editors

#### Bug fixes

- Fix the unexpected code deletion and repetition when `quickfix.check` is enabled and some `import`-related rules are applied ([#2222](https://github.com/checkjs/check/issues/2222), [#688](https://github.com/checkjs/check/issues/688), [#1015](https://github.com/checkjs/check/issues/1015)). Contributed by @Sec-ant

### Linter

#### New features

- Add [nursery/noMisplacedAssertion](https://checkjs.dev/linter/rules/no-misplaced-assertion/). COntributed by @ematipico

#### Bug fixes

- Fix [#2211](https://github.com/checkjs/check/issues/2211). noChildrenProp should work fine when children pass as a prop in a new line. Contributed by @fireairforce

- Fix [#2248](https://github.com/checkjs/check/issues/2248). `lint/a11y/useButtonType` should not trigger when button element with spread attribute. Contributed by @fireairforce

- Fix [#2216](https://github.com/checkjs/check/issues/2216). `lint/style/useNamingConvention` should not ignore JSX Component name binding. Contributed by @fireairforce
...
- Fix configuration resolution. Check is now able to correctly find the `check.jsonc` configuration file when `--config-path` is explicitly set ([#2164](https://github.com/checkjs/check/issues/2164)). Contributed by @Sec-ant

- JavaScript/TypeScript files of different variants (`.ts`, `.js`, `.tsx`, `.jsx`) in a single workspace now have stable formatting behaviors when running the CLI command in paths of different nested levels or in different operating systems ([#2080](https://github.com/checkjs/check/issues/2080), [#2109](https://github.com/checkjs/check/issues/2109)). Contributed by @Sec-ant
...
- Fix [#2172](https://github.com/checkjs/check/issues/2172) by breaking long object destructuring patterns. Contributed by @ah-yu

### Linter

#### New features

- Add rule [noEvolvingTypes](https://checkjs.dev/linter/rules/no-evolving-any) to disallow variables from evolving into `any` type through reassignments. Contributed by @fujiyamaorange
...
- Quickfix action no longer autofixes lint rule errors on save when `linter` is disabled ([#2161](https://github.com/checkjs/check/issues/2161)). Contributed by @Sec-ant
...
- The `noSuperWithoutExtends` rule now allows for calling `super()` in derived class constructors of class expressions ([#2108](https://github.com/checkjs/check/issues/2108)). Contributed by @Sec-ant

- Fix discrepancies on file source detection. Allow module syntax in `.cts` files ([#2114](https://github.com/checkjs/check/issues/2114)). Contributed by @Sec-ant

### CLI

#### Bug fixes

- Fixes [#2131](https://github.com/checkjs/check/issues/2131), where folders were incorrectly ignored when running the command `check`. Now folders are correctly ignored based on their command. Contributed by @ematipico

- Smoother handling of `"endOfLine": "auto"` in prettier migration: falling back to `"lf"` ([#2145](https://github.com/checkjs/check/pull/2145)). Contributed by @eMerzh

### Configuration

#### Bug fixes

- Fix enabled rules calculation. The precedence of individual rules, `all` and `recommend` presets in top-level and group-level configs is now correctly respected. More details can be seen in ([#2072](https://github.com/checkjs/check/pull/2072)) ([#2028](https://github.com/checkjs/check/issues/2028)). Contributed by @Sec-ant

### Formatter

#### Bug fixes

- Fix [#1661](https://github.com/checkjs/check/issues/1661). Now nested conditionals are aligned with Prettier's logic, and won't contain mixed spaces and tabs. Contributed by @ematipico

### JavaScript APIs

#### Enhancements

- Support applying lint fixes when calling the `lintContent` method of the `Check` class ([#1956](https://github.com/checkjs/check/pull/1956)). Contributed by @mnahkies

### Linter

#### New features

- Add [nursery/noDuplicateElseIf](https://checkjs.dev/linter/rules/no-duplicate-else-if/). COntributed by @mdm317

#### Bug fixes

- Rule `noUndeclaredDependencies` now also validates `peerDependencies` and `optionalDependencies` ([#2122](https://github.com/checkjs/check/issues/2122)). Contributed by @Sec-ant

- Rule `noUndeclaredDependencies` won't check `declare module` statements anymore ([#2123](https://github.com/checkjs/check/issues/2123)). Contributed by @Sec-ant

- Fix [#1925](https://github.com/checkjs/check/issues/1925). The fix for `useOptionalChain` would sometimes suggest an incorrect fix that discarded optional chaining operators on the left-hand side of logical expressions. These are now preserved. Contributed by @arendjr

- Rule `noUndeclaredVariables` now also checks for worker globals ([#2121](https://github.com/checkjs/check/issues/2121)). Contributed by @Sec-ant
...
- CLI is now able to automatically search and resolve `check.jsonc` ([#2008](https://github.com/checkjs/check/issues/2008)). Contributed by @Sec-ant
...
- `json.formatter.trailingCommas` option now works in `overrides` ([#2009](https://github.com/checkjs/check/issues/2009)). Contributed by @Sec-ant

### Linter

#### New features

- Add rule [noDoneCallback](https://checkjs.dev/linter/rules/no-done-callback), this rule checks the function parameter of hooks & tests
...
- [useJsxKeyInIterable](https://checkjs.dev/linter/rules/use-jsx-key-in-iterable) now recognizes function bodies wrapped in parentheses ([#2011](https://github.com/checkjs/check/issues/2011)). Contributed by @Sec-ant

- [useShorthandFunctionType](https://checkjs.dev/linter/rules/use-shorthand-function-type) now preserves type parameters of generic interfaces when applying fixes ([#2015](https://github.com/checkjs/check/issues/2015)). Contributed by @Sec-ant

- Code fixes of [useImportType](https://checkjs.dev/linter/rules/use-import-type) and [useExportType](https://checkjs.dev/linter/rules/use-export-type) now handle multiline statements ([#2041](https://github.com/checkjs/check/issues/2041)). Contributed by @Conaclos

- [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare) no longer reports type parameter and parameter with identical names ([#1992](https://github.com/checkjs/check/issues/1992)).
...
- [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare) now reports duplicate type parameters in a same declaration.
...
- [noUndeclaredDependencies](https://checkjs.dev/linter/rules/no-undeclared-dependencies/) now recognizes imports of subpath exports.
...
- JavaScript lexer is now able to lex regular expression literals with escaped non-ascii chars ([#1941](https://github.com/checkjs/check/issues/1941)).
...
- Add partial for `.astro` files. Check is able to sort imports inside the frontmatter of the Astro files. Contributed
  by @ematipico
  ...
  <div></div>
  ```
- Add partial for `.vue` files. Check is able to sort imports inside the script block of Vue files. Contributed by
  @nhedger
...
- Add partial for `.svelte` files. Check is able to sort imports inside the script block of Svelte files. Contributed by
  @ematipico
...
- [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) ignores unused rest spread siblings.
...
- Fix [#1931](https://github.com/checkjs/check/issues/1931). Built-in React hooks such as
  `useEffect()` can now be validated by the
  [`useExhaustiveDependendies`](https://checkjs.dev/linter/rules/use-exhaustive-dependencies/), even
  when they're not being imported from the React library. To do so, simply configure them like
  any other user-provided hooks.

  Contributed by @arendjr

- Implemented [#1128](https://github.com/checkjs/check/issues/1128). User-provided React hooks can
...
- Fix [#1748](https://github.com/checkjs/check/issues/1748). Now for the following case we won't provide an unsafe fix
...
- Fix [#1081](https://github.com/checkjs/check/issues/1081). The `useAwait` rule does not report `for await...of`.
  Contributed by @unvalley

- Fix [#1827](https://github.com/checkjs/check/issues/1827) by properly analyzing nested `try-finally` statements. Contributed by @ah-yu

- Fix [#1924](https://github.com/checkjs/check/issues/1924) Use the correct export name to sort in the import clause. Contributed by @ah-yu
- Fix [#1805](https://github.com/checkjs/check/issues/1805) fix formatting arrow function which has conditional expression body  Contributed by @mdm317

- Fix [#1781](https://github.com/checkjs/check/issues/1781) by avoiding the retrieval of the entire static member expression for the reference if the static member expression does not start with the reference. Contributed by @ah-yu

### CLI

#### New features

- Add a new command `check migrate prettier`. The command will read the file `.prettierrc`/`prettier.json`
  and `.prettierignore` and map its configuration to Check's one.
  Due to the different nature of `.prettierignore` globs and Check's globs, it's **highly** advised to make sure that
  those still work under Check.
...
- Add an option `--linter` to `check rage`. The option needs to check Check linter configuration. Contributed by
  @seitarof

- Add an option `--formatter` to `check rage`. The option needs to check Check formatter configuration. Contributed by
  @seitarof
...
- Don't process files under an ignored directory.
  ...
  Previously, Check processed all files in the traversed hierarchy,
  ...
  Check will traverse `test`,
  however all files of the directory are correctly ignored.
  This can result in file system errors,
  if Check encounters dangling symbolic links or files with higher permissions.
...
- Fix [#1508](https://github.com/checkjs/check/issues/1508) by excluding deleted files from being processed. Contributed
  by @ematipico

- Fix [#1173](https://github.com/checkjs/check/issues/1173). Fix the formatting of a single instruction with commented
...
- Fix a bug where syntax rules weren't run when pulling the diagnostics. Now Check will emit more parsing diagnostics,
  e.g.
  ```
...
- Fix [#1774](https://github.com/checkjs/check/issues/1774) by taking into account the option `--no-errors-on-unmatched` when running the CLI using `--changed`. Contributed by @antogyn
...
- Add the ability to resolve the configuration files defined inside `extends` from the `node_modules/` directory.
  ...
  If you want to resolve a configuration file that matches the specifier `@org/configs/check`, then your `package.json`
  file must look this:
  ...
  And the `check.json` file that "imports" said configuration, will look like this:
  ```json
  {
    "extends": "@org/configs/check"
  }
  ```
  Read the [documentation](https://checkjs.dev/guides/how-check-works#the-extends-option) to better understand how it
...
- Check now allows to format the `package.json` file. This is now the default behaviour and users can remove their
  workarounds.
...
- New formatter option `attributePosition` that have similar behavior as
  Prettier `singleAttributePerLine` [#1706](https://github.com/checkjs/check/issues/1706). Contributed by @octoshikari
- Add partial for `.astro` files. Check is able to format the frontmatter of the Astro files. Contributed by @ematipico
  ...
  <div></div>
  ```
- Add partial for `.vue` files. Check is able to format the script block of Vue files. Contributed by @nhedger
...
- Add partial for `.svelte` files. Check is able to format the script block of Svelte files. Contributed by @ematipico
...
- `composer.json`, `deno.json`, `jsconfig.json`, `package.json` and `tsconfig.json` are no longer protected files.
  ...
  If you want to ignore these files, you can use the [files.ignore](https://checkjs.dev/reference/configuration/#filesignore) configuration:
...
- Fix [#1039](https://github.com/checkjs/check/issues/1039). Check unicode width instead of number of bytes when
...
- Fix [#1218](https://github.com/checkjs/check/issues/1218), by correctly preserving empty lines in member chains.
  Contributed by @ah-yu
- Fix [#1659](https://github.com/checkjs/check/issues/1659) and [#1662](https://github.com/checkjs/check/issues/1662), by correctly taking into account the leading comma inside the formatter options. Contributed by @ematipico

- Fix [#1934](https://github.com/checkjs/check/pull/1934). Fix invalid formatting of long arrow function for AsNeeded arrow parens Contributed by @fireairforce
...
- [complexity/noEmptyTypeParameters](https://checkjs.dev/linter/rules/no-empty-type-parameters)
- [complexity/noUselessLoneBlockStatements](https://checkjs.dev/linter/rules/no-useless-lone-block-statements)
- [correctness/noInvalidUseBeforeDeclaration](https://checkjs.dev/linter/rules/no-invalid-use-before-declaration)
- [correctness/noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports)
- [correctness/noUnusedPrivateClassMembers](https://checkjs.dev/linter/rules/no-unused-private-class-members)
- [security/noGlobalEval](https://checkjs.dev/linter/rules/no-global-eval)
- [style/useConsistentArrayType](https://checkjs.dev/linter/rules/use-consistent-array-type)
- [style/useExportType](https://checkjs.dev/linter/rules/use-export-type)
- [style/useFilenamingConvention](https://checkjs.dev/linter/rules/use-filenaming-convention)
- [style/useForOf](https://checkjs.dev/linter/rules/use-for-of)
- [style/useImportType](https://checkjs.dev/linter/rules/use-import-type)
- [style/useNodejsImportProtocol](https://checkjs.dev/linter/rules/use-nodejs-import-protocol)
- [style/useNumberNamespace](https://checkjs.dev/linter/rules/use-number-namespace)
- [style/useShorthandFunctionType](https://checkjs.dev/linter/rules/use-shorthand-function-type)
- [suspicious/noEmptyBlockStatements](https://checkjs.dev/linter/rules/no-empty-block-statements)
- [suspicious/noGlobalAssign](https://checkjs.dev/linter/rules/no-global-assign)
- [suspicious/noMisleadingCharacterClass](https://checkjs.dev/linter/rules/no-misleading-character-class)
- [suspicious/noThenProperty](https://checkjs.dev/linter/rules/no-then-property)
- [suspicious/useAwait](https://checkjs.dev/linter/rules/use-await)

Additionally, the following rules are now recommended:

- [suspicious/noApproximativeNumericConstant](https://checkjs.dev/linter/rules/no-approximative-numeric-constant)
- [suspicious/noMisrefactoredShorthandAssign](https://checkjs.dev/linter/rules/no-misrefactored-shorthand-assign)

#### Removed rules

- Remove `nursery/useGroupedTypeImport`. The rule [style/useImportType](https://checkjs.dev/linter/rules/use-import-type) covers the behavior of this rule.

  Note that removing a nursery rule is not considered a breaking change according to our [semantic versioning](https://checkjs.dev/internals/versioning).

  Contributed by @Conaclos

#### New features

- Add the rule [noSkippedTests](https://checkjs.dev/linter/rules/no-skipped-tests), to disallow skipped tests:
...
- Add the rule [noFocusedTests](https://checkjs.dev/linter/rules/no-focused-tests), to disallow skipped tests:
...
- Add rule [useSortedClasses](https://checkjs.dev/linter/rules/use-sorted-classes), to sort CSS utility classes:
...
- Add rule [noUndeclaredDependencies](https://checkjs.dev/linter/rules/no-undeclared-dependencies), to detect the use of
...
- Add rule [noNamespaceImport](https://checkjs.dev/linter/rules/no-namespace-import), to report namespace imports:

  ```js
  import * as foo from "foo";
  ```
  Contributed by @unvalley
- Add partial support for `.astro` files. Check is able to lint and fix the frontmatter of the Astro files. Contributed
  by @ematipico
...
- Add partial support for `.vue` files. Check is able to lint and fix the script block of the Vue files.
...
- Add rule [useNodeAssertStrict](https://checkjs.dev/linter/rules/use-node-assert-strict), which promotes the use
  of `node:assert/strict` over `node:assert`. Contributed by @ematipico

- Add rule [noExportsInTest](https://checkjs.dev/linter/rules/no-exports-in-test) which disallows `export` or `modules.exports` in files
  containing test. Contributed by @ah-yu

- Add rule [noSemicolonInJsx](https://checkjs.dev/linter/rules/no-suspicious-semicolon-in-jsx/) to detect possible wrong semicolons inside JSX elements.
  ...
  Contributed by @fujiyamaorange
- Add rule [noBarrelFile](https://checkjs.dev/linter/rules/no-barrel-file), to report the usage of barrel file:
...
- Add rule [noReExportAll](https://checkjs.dev/linter/rules/no-re-export-all/) that report `export * from "mod"`.
  Contributed by @mdm317

- Add rule [noExcessiveNestedTestSuites](https://checkjs.dev/linter/rules/no-excessive-nested-test-suites/).
  Contributed by @vasucp1207

- Add rule [useJsxKeyInIterable](https://checkjs.dev/linter/rules/use-jsx-key-in-iterable/).
  Contributed by @vohoanglong0107

#### Enhancements

- [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/) now rule not triggered for jsx attributes when
...
- [noUselessTernary](https://checkjs.dev/linter/rules/no-useless-ternary) now provides unsafe code fixes. Contributed by
  @vasucp1207

- [noApproximativeNumericConstant](https://checkjs.dev/linter/rules/no-approximative-numeric-constant) now provides
...
- [noPrecisionLoss](https://checkjs.dev/linter/rules/no-precision-loss) no longer reports number with extra zeros.
...
- [useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention) now
  supports [unicase](https://en.wikipedia.org/wiki/Unicase)
  letters ([#1786](https://github.com/checkjs/check/issues/1786)).

  [unicase](https://en.wikipedia.org/wiki/Unicase) letters have a single case: they are neither uppercase nor lowercase.
  Previously, Check reported names in unicase as invalid.
...
- [useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention)
  and [useFilenamingConvention](https://checkjs.dev/linter/rules/use-filenaming-convention) now provides a new option `requireAscii` to require identifiers to
  be in ASCII.

  To avoid any breaking change, this option is turned off by default.
  We intend to turn it on in the next major release of Check (Check 2.0).
...
- [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) no longer reports unused imports.
...
- Fix missing link in [noStaticOnlyClass](https://checkjs.dev/linter/rules/no-static-only-class) documentation.
  Contributed by @yndajas

- [noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type) no longer reports valid use of the void
  type in conditional types ([#1812](https://github.com/checkjs/check/issues/1812)).
...
- [noInvalidUseBeforeDeclaration](https://checkjs.dev/linter/rules/no-invalid-use-before-declaration) no longer reports
  valid use of binding patterns ([#1648](https://github.com/checkjs/check/issues/1648)).
...
- [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) no longer reports used binding
  patterns ([#1652](https://github.com/checkjs/check/issues/1652)).
...
- Fix [#1651](https://github.com/checkjs/check/issues/1651). [noVar](https://checkjs.dev/linter/rules/no-var/) now
  ignores TsGlobalDeclaration. Contributed by @vasucp1207

- Fix [#1640](https://github.com/checkjs/check/issues/1640). [useEnumInitializers](https://checkjs.dev/linter/rules/use-enum-initializers) code action now generates valid code when last member has a comment but no comma. Contributed by @kalleep

- Fix [#1653](https://github.com/checkjs/check/issues/1653). Handle a shorthand value in `useForOf` to avoid the false-positive case. Contributed by @togami2864

- Fix [#1656](https://github.com/checkjs/check/issues/1656). [useOptionalChain](https://checkjs.dev/linter/rules/use-optional-chain/) code action now correctly handles logical and chains where methods with the same name are invoked with different arguments:
...
- Fix [#1704](https://github.com/checkjs/check/issues/1704). Convert `/` to escaped slash `\/` to avoid parsing error in
  the result of autofix. Contributed by @togami2864

- Fix[#1697](https://github.com/checkjs/check/issues/1697). Preserve leading trivia in autofix of suppression rules.
  Contributed by @togami2864

- Fix [#603](https://github.com/checkjs/check/issues/603). Trim trailing whitespace to avoid double insertion.
  Contributed by @togami2864

- Fix [#1765](https://github.com/checkjs/check/issues/1765). Now the rule `noDelete` doesn't trigger when deleting a
  dataset:
  ```js
...
- [useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention)
  and [useFilenamingConvention](https://checkjs.dev/linter/rules/use-filenaming-convention) now reject identifiers with consecutive delimiters.
...
- Fix [#1932](https://github.com/checkjs/check/issues/1932) Allow redeclaration of type parameters in different declarations.
  Contributed by @keita-hino

- Fix [#1945](https://github.com/checkjs/check/issues/1945) Allow constructor with default parameters in `noUselessConstructor`

- Fix [#1982](https://github.com/checkjs/check/issues/1982) Change to iterate over the module item lists and ignore .d.ts files. Contributed by @togami2864

### Parser

#### Bug fixes

- Fix [#1728](https://github.com/checkjs/check/issues/1728). Correctly parse the global declaration when the `{` token
...
- Fix [#1730](https://github.com/checkjs/check/issues/1730). Correctly parse `delete` expressions with operands that are
...
- Fix [#1981](https://github.com/checkjs/check/issues/1981). Identify TypeScript definition files by their file path within the playground. Contributed by @ah-yu
...
- Fix [#1584](https://github.com/checkjs/check/issues/1584). Ensure the LSP only registers the formatter once.
  Contributed by @nhedger

- Fix [#1589](https://github.com/checkjs/check/issues/1589). Fix invalid formatting of own line comments when they were
...
- Override correctly the recommended preset ([#1349](https://github.com/checkjs/check/issues/1349)).

  Previously, if unspecified, Check turned on the recommended preset in overrides.
  This resulted in reporting diagnostics with a severity level set to `off`.
  This in turn caused Check to fail.

  Now Check won't switch on the recommended preset in `overrides` unless told to do so.

  Contributed by @Conaclos

- Don't format **ignored** files that are well-known JSONC files when `files.ignoreUnknown` is
  enabled ([#1607](https://github.com/checkjs/check/issues/1607)).

  Previously, Check always formatted files that are known to be JSONC files (e.g. `.eslintrc`)
...
- Fix [#1178](https://github.com/checkjs/check/issues/1178), where the line ending option wasn't correctly applied.
  Contributed by @ematipico
- Fix [#1571](https://github.com/checkjs/check/issues/1571). Fix invalid formatting of nested multiline comments.
  Contributed by @ah-yu
...
-
Fix [#1575](https://github.com/checkjs/check/issues/1575). [noArrayIndexKey](https://checkjs.dev/linter/rules/no-array-index-key/)
...
- Linter rules that inspect regexes now handle multibyte characters
  correctly ([#1522](https://github.com/checkjs/check/issues/1522)).

  Previously, [noMisleadingCharacterClass](https://checkjs.dev/linter/no-misleading-character-class), [noMultipleSpacesInRegularExpressionLiterals](https://checkjs.dev/linter/no-multiple-spaces-in-regular-expression-literals),
  and [noEmptyCharacterClassInRegex](https://checkjs.dev/linter/no-empty-character-class-in-regex) made Check errors on
...
- [useExhaustiveDependencies](https://checkjs.dev/linter/use-exhaustive-dependencies) no longer made Check errors in
  code TypeScript import equal declarations ([#1194](https://github.com/checkjs/check/issues/1194)). Contributed by
  @Conaclos

- Fix typo in the diagnostic of [noNodejsModules](https://checkjs.dev/linter/rules/no-nodejs-modules). Contributed by
  @huseeiin
...
- Accept the `const` modifier for type parameter in method type
  signature ([#1624](https://github.com/checkjs/check/issues/1624)).
...
- Correctly parse type arguments in expression([#1184](https://github.com/checkjs/check/issues/1184)).
...
- Add a [page that maps the Check rule to its source](https://checkjs.dev/linter/rules-sources/). Contributed by
  @ematipico
...
- Fix examples of the [git hook page](https://checkjs.dev/recipes/git-hooks/). Contributed by @9renpoto, @lmauromb, and
  @Conaclos
...
- Fix [#1512](https://github.com/checkjs/check/issues/1512) by skipping verbose diagnostics from the count. Contributed
  by @ematipico

- Correctly handle cascading `include` and `ignore`.

  Previously Check incorrectly included files that were included at tool level and ignored at global level.
  In the following example, `file.js` was formatted when it should have been ignored.
  Now, Check correctly ignores the directory `./src/sub/`.
...
- Don't emit verbose warnings when a protected file is ignored.
  ...
  are [protected](https://checkjs.dev/guides/how-check-works/#protected-files).
  Check emits a verbose warning when it encounters a protected file.

  Previously, Check emitted this verbose warning even if the file was ignored by the configuration.
...
- The file `check.json` can't be ignored anymore. Contributed by @ematipico

- Fix [#1541](https://github.com/checkjs/check/issues/1541) where the content of protected files wasn't returned
...
- Fix [1440](https://github.com/checkjs/check/issues/1440), a case where `extends` and `overrides` weren't correctly
...
- Correctly handle `include` when `ignore` is set (#1468). Contributed by @Conaclos
  ...
  Previously, Check ignored `include` if `ignore` was set.
  Now, Check check both `include` and `ignore`.
...
- Fix placement of comments before `*` token in generator methods with
  decorators. [#1537](https://github.com/checkjs/check/pull/1537) Contributed by @ah-yu

- Fix [#1406](https://github.com/checkjs/check/issues/1406). Ensure comments before the `async` keyword are placed
  before it. Contributed by @ah-yu

- Fix [#1172](https://github.com/checkjs/check/issues/1172). Fix placement of line comment after function expression
  parentheses, they are now attached to first statement in body. Contributed by @kalleep

- Fix [#1511](https://github.com/checkjs/check/issues/1511) that made the JavaScript formatter crash. Contributed
  @Conaclos
...
- Add an unsafe code fix for [noConsoleLog](https://checkjs.dev/linter/rules/no-console-log/). Contributed by
  @vasucp1207

- [useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function) no longer reports function in `extends`
...
- The fix of [useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function) now adds parentheses around the
  arrow function in more cases where it is needed ([#1524](https://github.com/checkjs/check/issues/1524)).
...
- Fix [#1514](https://github.com/checkjs/check/issues/1514). Fix autofix suggestion to avoid the syntax error
...
- The diagnostics for protected files are now shown only when the option `--verbose` is passed. Contributed by
  @ematipico
- Fix [#1465](https://github.com/checkjs/check/issues/1465), by taking in consideration the workspace folder when
  matching a pattern. Contributed by @ematipico
- Fix [#1465](https://github.com/checkjs/check/issues/1465), by correctly process globs that contain file names.
  Contributed by @ematipico
...
- Fix [#1170](https://github.com/checkjs/check/issues/1170). Fix placement of comments inside default switch clause. Now
...
-
Fix [#1335](https://github.com/checkjs/check/issues/1335). [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/)
now ignores code action on component props when the fragment is empty. Contributed by @vasucp1207

- [useConsistentArrayType](https://checkjs.dev/linter/rules/use-consistent-array-type) was accidentally placed in
  the `style` rule group instead of the `nursery` group. It is now correctly placed under `nursery`.

-
Fix [#1483](https://github.com/checkjs/check/issues/1483). [useConsistentArrayType](https://checkjs.dev/linter/rules/use-consistent-array-type)
now correctly handles its option. Contributed by @Conaclos

-
Fix [#1502](https://github.com/checkjs/check/issues/1502). [useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function)
...
-
Fix [#1473](https://github.com/checkjs/check/issues/1473): [useHookAtTopLevel](https://checkjs.dev/linter/rules/use-hook-at-top-level/)
now correctly handles React components and hooks that are nested inside other functions. Contributed by @arendjr

## 1.5.0 (2024-01-08)

Check now scores 97% compatibility with Prettier and features more than 180 linter rules.
...
- Check now shows a diagnostic when it encounters a protected file. Contributed by @ematipico

- The command `check migrate` now updates the `$schema` if there's an outdated version.

- The CLI now takes in consideration the `.gitignore` in the home directory of the user, if it exists. Contributed by
  @ematipico
- The `check ci` command is now able to
...
- The commands `format`, `lint`, `check` and `ci` now accept two new arguments: `--changed` and `--since`. Use these
  ...
  ```shell
  check format --write --changed
  ```

- Introduced a new command called `check explain`, which has the capability to display documentation for lint rules.
  Contributed by @kalleep
- You can use the command `check explain` to print the documentation of lint rules. Contributed by @kalleep
  ```shell
  check explain noDebugger
  check explain useAltText
  ```
- You can use the command `check explain` to print the directory where daemon logs are stored. Contributed by @ematipico
  ```shell
  check explain daemon-logs
  ```
- Removed the hard coded limit of 200 printable diagnostics. Contributed by @ematipico

#### Bug fixes

- Fix [#1247](https://github.com/checkjs/check/issues/1247), Check now prints a **warning** diagnostic if it encounters
  files that can't handle. Contributed by @ematipico

  You can ignore unknown file types using
  the [`files.ignoreUnknown`](https://checkjs.dev/reference/configuration/#filesignoreunknown) configuration
  in `check.json`:
  ...
  ```shell
  check format --files-ignore-unknown=true --write .
  ```

- Fix [#709](https://github.com/checkjs/check/issues/709) and [#805](https://github.com/checkjs/check/issues/805) by
  correctly parsing `.gitignore` files. Contributed by @ematipico

- Fix [#1117](https://github.com/checkjs/check/issues/1117) by correctly respecting the matching. Contributed by
  @ematipico

- Fix [#691](https://github.com/checkjs/check/issues/691) and [#1190](https://github.com/checkjs/check/issues/1190), by
  correctly apply the configuration when
  computing [`overrides` configuration](https://checkjs.dev/reference/configuration/#overrides). Contributed by
  @ematipico
...
- Fix [#1169](https://github.com/checkjs/check/issues/1169). Account for escaped strings when computing layout for
  assignments. Contributed by @kalleep
- Fix [#851](https://github.com/checkjs/check/issues/851). Allow regular function expressions to group and break as call
  arguments, just like arrow function expressions. [#1003](https://github.com/checkjs/check/issues/1003) Contributed by
  @faultyserver
- Fix [#914](https://github.com/checkjs/check/issues/914). Only parenthesize type-casted function expressions as default
  exports. [#1023](https://github.com/checkjs/check/issues/1023) Contributed by @faultyserver
- Fix [#1112](https://github.com/checkjs/check/issues/1112). Break block bodies in case clauses onto their own lines and
  preserve trailing fallthrough comments. [#1035](https://github.com/checkjs/check/pull/1035) Contributed by
  @faultyserver
- Fix `RemoveSoftLinesBuffer` behavior to also removed conditional expanded content, ensuring no accidental, unused line
  breaks are included [#1032](https://github.com/checkjs/check/pull/1032) Contributed by @faultyserver
- Fix [#1024](https://github.com/checkjs/check/issues/1024). Allow JSX expressions to nestle in arrow
  chains [#1033](https://github.com/checkjs/check/pull/1033) Contributed by @faultyserver
- Fix incorrect breaking on the left side of assignments by always using fluid
  assignment. [#1021](https://github.com/checkjs/check/pull/1021) Contributed by @faultyserver
- Fix breaking strategy for nested object patterns in function
  parameters [#1054](https://github.com/checkjs/check/pull/1054) Contributed by @faultyserver
- Fix over-indention of arrow chain expressions by simplifying the way each chain is
  grouped [#1036](https://github.com/checkjs/check/pull/1036), [#1136](https://github.com/checkjs/check/pull/1136),
  and [#1162](https://github.com/checkjs/check/pull/1162) Contributed by @faultyserver.
- Fix "simple" checks for calls and member expressions to correctly handle array accesses, complex arguments to
  ...
  calls. [#1057](https://github.com/checkjs/check/pull/1057) Contributed by @faultyserver
- Fix text wrapping and empty line handling for JSX Text elements to match Prettier's
  behavior. [#1075](https://github.com/checkjs/check/pull/1075) Contributed by @faultyserver
- Fix leading comments in concisely-printed arrays to prevent unwanted line
  breaks. [#1135](https://github.com/checkjs/check/pull/1135) Contributed by @faultyserver
- Fix `best_fitting` and interned elements preventing expansion propagation from sibling
  elements. [#1141](https://github.com/checkjs/check/pull/1141) Contributed by @faultyserver
- Fix heuristic for grouping function parameters when type parameters with constraints are
  present. [#1153](https://github.com/checkjs/check/pull/1153). Contributed by @faultyserver.
- Fix binary-ish and type annotation handling for grouping call arguments in function expressions and call
  signatures. [#1152](https://github.com/checkjs/check/pull/1152)
  and [#1160](https://github.com/checkjs/check/pull/1160) Contributed by @faultyserver
- Fix handling of nestled JSDoc comments to preserve behavior for
  overloads. [#1195](https://github.com/checkjs/check/pull/1195) Contributed by @faultyserver
- Fix [#1208](https://github.com/checkjs/check/issues/1208). Fix extraction of inner types when checking for simple type
  annotations in call arguments. [#1195](https://github.com/checkjs/check/pull/1195) Contributed by @faultyserver

- Fix [#1220](https://github.com/checkjs/check/issues/1220). Avoid duplicating comments in type unions for mapped, empty
  object, and empty tuple types. [#1240](https://github.com/checkjs/check/pull/1240) Contributed by @faultyserver

- Fix [#1356](https://github.com/checkjs/check/issues/1356). Ensure `if_group_fits_on_line` content is always written
  in `RemoveSoftLinesBuffer`s. [#1357](https://github.com/checkjs/check/pull/1357) Contributed by @faultyserver

- Fix [#1171](https://github.com/checkjs/check/issues/1171). Correctly format empty statement with comment inside arrow
  body when used as single argument in call expression. Contributed by @kalleep

- Fix [#1106](https://github.com/checkjs/check/issues/1106). Fix invalid formatting of single bindings when Arrow
  Parentheses is set to "AsNeeded" and the expression breaks over multiple
  lines. [#1449](https://github.com/checkjs/check/pull/1449) Contributed by @faultyserver
...
- [a11y/noAriaHiddenOnFocusable](https://www.checkjs.dev/linter/rules/no-aria-hidden-on-focusable)
- [a11y/useValidAriaRole](https://www.checkjs.dev/linter/rules/use-valid-aria-role)
- [complexity/useRegexLiterals](https://www.checkjs.dev/linter/rules/use-regex-literals)
- [suspicious/noImplicitAnyLet](https://www.checkjs.dev/linter/rules/no-implicit-any-let)
- [style/noDefaultExport](https://www.checkjs.dev/linter/rules/no-default-export)

#### New features

- Add [useExportType](https://checkjs.dev/linter/rules/use-export-type) that enforces the use of type-only exports for
...
- Add [useImportType](https://checkjs.dev/linter/rules/use-import-type) that enforces the use of type-only imports for
  types. Contributed by @Conaclos
...
- Add [useFilenamingConvention](https://checkjs.dev/linter/rules/use-filenaming-convention), that enforces naming
...
- Add [useNodejsImportProtocol](https://checkjs.dev/linter/rules/use-nodejs-import-protocol) that enforces the use of
...
- Add [useNumberNamespace](https://checkjs.dev/linter/rules/use-number-namespace) that enforces the use of the `Number`
...
- Add [useShorthandFunctionType](https://checkjs.dev/linter/rules/use-shorthand-function-type) that enforces using
...
- Add [noNodejsModules](https://checkjs.dev/linter/rules/no-nodejs-modules), that disallows the use of _Node.js_ modules. Contributed by @anonrig, @ematipico, and @Conaclos

- Add [noInvalidUseBeforeDeclaration](https://checkjs.dev/linter/rules/no-invalid-use-before-declaration) that reports variables and function parameters used before their declaration. Contributed by @Conaclos
...
- Add [useConsistentArrayType](https://checkjs.dev/linter/rules/use-consistent-array-type) that enforces the use of a
  consistent syntax for array types. Contributed by @eryue0220

  This rule will replace [useShorthandArrayType](https://checkjs.dev/linter/rules/use-shorthand-array-type).
  It provides an option to choose between the shorthand or the generic syntax.

- Add [noEmptyTypeParameters](https://checkjs.dev/linter/rules/no-empty-type-parameters) that ensures that any type
...
- Add [noGlobalEval](https://checkjs.dev/linter/rules/no-global-eval) that reports any use of the global `eval`.
  Contributed by @you-5805

- Add [noGlobalAssign](https://checkjs.dev/linter/rules/no-global-assign) that reports assignment to global variables.
...
- Add [noMisleadingCharacterClass](https://checkjs.dev/linter/rules/no-misleading-character-class) that disallows
  characters made with multiple code points in character class. Contributed by @togami2864

- Add [noThenProperty](https://checkjs.dev/linter/rules/no-then-property) that disallows the use of `then` as property
  name. Adding a `then` property makes an object _thenable_ that can lead to errors with Promises. Contributed by
  @togami2864

- Add [noUselessTernary](https://checkjs.dev/linter/rules/no-useless-ternary) that disallows conditional expressions (
...
- [noEmptyInterface](https://checkjs.dev/linter/rules/no-empty-interface) ignores empty interfaces that extend a type.
  Address [#959](https://github.com/checkjs/check/issues/959) and [#1157](https://github.com/checkjs/check/issues/1157).
  Contributed by @Conaclos
...
- Preserve more comments in the code fix
  of [useExponentiationOperator](https://checkjs.dev/linter/rules/use-exponentiation-operator). Contributed by @Conaclos
...
- `<svg>` element is now considered as a non-interactive HTML
  element ([#1095](https://github.com/checkjs/check/issues/1095)). Contributed by @chansuke

  This affects the following rules:
  - [noAriaHiddenOnFocusable](https://checkjs.dev/linter/rules/no-aria-hidden-on-focusable)
  - [noInteractiveElementToNoninteractiveRole](https://checkjs.dev/linter/rules/no-interactive-element-to-noninteractive-role)
  - [noNoninteractiveElementToInteractiveRole](https://checkjs.dev/linter/rules/no-noninteractive-element-to-interactive-role)
  - [noNoninteractiveTabindex](https://checkjs.dev/linter/rules/no-noninteractive-tabindex)
  - [useAriaActivedescendantWithTabindex](https://checkjs.dev/linter/rules/use-aria-activedescendant-with-tabindex)

- [noMultipleSpacesInRegularExpressionLiterals](https://checkjs.dev/linter/rules/no-multiple-spaces-in-regular-expression-literals/)
  has a safe code fix. Contributed by @Conaclos

- [useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function/) ignores expressions that use `new.target`.
  Contributed by @Conaclos

- [noForEach](https://checkjs.dev/linter/rules/no-for-each) now reports only calls that use a callback with `0` or `1`
  parameter. Address [#547](https://github.com/checkjs/check/issues/547). Contributed by @Conaclos

#### Bug fixes

-
Fix [#1061](https://github.com/checkjs/check/issues/1061). [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare)
...
-
Fix [#651](https://github.com/checkjs/check/issues/651), [useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies)
...
-
Fix [#1191](https://github.com/checkjs/check/issues/1191). [noUselessElse](https://checkjs.dev/linter/rules/no-useless-else)
...
-
Fix [#1383](https://github.com/checkjs/check/issues/1383). [noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type)
...
-
Fix [#728](https://github.com/checkjs/check/issues/728). [useSingleVarDeclarator](https://checkjs.dev/linter/rules/use-single-var-declarator)
no longer outputs invalid code. Contributed by @Conaclos

-
Fix [#1167](https://github.com/checkjs/check/issues/1167). [useValidAriaProps](https://checkjs.dev/linter/rules/use-valid-aria-props)
no longer reports `aria-atomic` as invalid. Contributed by @unvalley

-
Fix [#1192](https://github.com/checkjs/check/issues/1192). [useTemplate](https://checkjs.dev/linter/rules/use-template/)
...
-
Fix [#1456](https://github.com/checkjs/check/issues/1456). [useTemplate](https://checkjs.dev/linter/rules/use-template/)
...
-
Fix [#1436](https://github.com/checkjs/check/issues/1436). [useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function/)
...
-
Fix [#696](https://github.com/checkjs/check/issues/696). [useHookAtTopLevel](https://checkjs.dev/linter/rules/use-hook-at-top-level)
now correctly detects early returns before the calls to the hook.

- The code fix of [noUselessTypeCOnstraint](https://checkjs.dev/linter/rules/no-useless-type-constraint) now adds a
  trailing comma when needed to disambiguate a type parameter list from a JSX element. COntributed by @Conaclos

-
Fix [#578](https://github.com/checkjs/check/issues/578). [useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies)
now correctly recognizes hooks namespaced under the `React` namespace. Contributed by @XiNiHa

-
Fix [#910](https://github.com/checkjs/check/issues/910). [noSvgWithoutTitle](https://checkjs.dev/linter/rules/no-svg-without-title)
...
- The representation of imports has been simplified. Contributed by @Conaclos
  ...
  See [#1163](https://github.com/checkjs/check/pull/1163) for more details.
...
- Fix [#1077](https://github.com/checkjs/check/issues/1077) where parenthesized identifiers in conditional expression
...
- Allow empty type parameter lists for interfaces and type
  aliases ([#1237](https://github.com/checkjs/check/issues/1237)). COntributed by @togami2864

  _TypeScript_ allows interface declarations and type aliases to have empty type parameter lists.
  Previously Check didn't handle this edge case.
...
- Rename the `check_js_unicode_table` crate
  to `check_unicode_table` ([#1302](https://github.com/checkjs/check/issues/1302)). COntributed by @chansuke
...
- Fix [#933](https://github.com/checkjs/check/issues/933). Some files are properly ignored in the LSP too.
  E.g. `package.json`, `tsconfig.json`, etc.
- Fix [#1394](https://github.com/checkjs/check/issues/1394), by inferring the language extension from the internal saved
...
- Fix some accidental line breaks when printing array expressions within arrow functions and other long
  lines [#917](https://github.com/checkjs/check/pull/917). Contributed by @faultyserver

- Match Prettier's breaking strategy for `ArrowChain` layouts [#934](https://github.com/checkjs/check/pull/934).
  Contributed by @faultyserver

- Fix double-printing of leading comments in arrow chain expressions [#951](https://github.com/checkjs/check/pull/951).
  Contributed by @faultyserver
...
- Fix [#910](https://github.com/checkjs/check/issues/910), where the rule `noSvgWithoutTitle` should skip elements that
  have `aria-hidden` attributes. Contributed by @vasucp1207

#### New features

- Add [useForOf](https://checkjs.dev/linter/rules/use-for-of) rule.
...
- Address [#924](https://github.com/checkjs/check/issues/924)
  and [#920](https://github.com/checkjs/check/issues/920). [noUselessElse](https://checkjs.dev/linter/rules/no-useless-else)
...
-
Fix [#918](https://github.com/checkjs/check/issues/918). [useSimpleNumberKeys](https://checkjs.dev/linter/rules/use-simple-number-keys)
no longer reports false positive on comments. Contributed by @kalleep

- Fix [#953](https://github.com/checkjs/check/issues/953). [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare)
  no longer reports type parameters with the same name in different mapped types as redeclarations. Contributed by
  @Conaclos

-
Fix [#608](https://github.com/checkjs/check/issues/608). [useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies)
...
- Add option `--config-path` to `lsp-proxy` and `start` commands. It's now possible to tell the Daemon server to
  load `check.json` from a custom path. Contributed by @ematipico
...
- Add the configuration [`formatter.lineFeed`](https://checkjs.dev/reference/configuration/#formatterlineending). It
  allows changing the type of line endings. Contributed by @SuperchupuDev

- Add the
  configuration [`javascript.formatter.bracketSameLine`](https://checkjs.dev/reference/configuration/#formatterbracketsameline).
  It allows controlling whether ending `>` of a multi-line _JSX_ element should be on the last attribute line or
  not. [#627](https://github.com/checkjs/check/issues/627). Contributed by @faultyserver

- Add the
  configuration [`javascript.formatter.bracketSpacing`](https://checkjs.dev/reference/configuration/#formatterbracketspacing).
  It allows controlling whether spaces are inserted around the brackets of object
  literals. [#627](https://github.com/checkjs/check/issues/627). Contributed by @faultyserver

#### Bug fixes

- Fix [#832](https://github.com/checkjs/check/issues/832), the formatter no longer keeps an unnecessary trailing comma
...
- Fix [#301](https://github.com/checkjs/check/issues/301), the formatter should not break before the `in` keyword.
  Contributed by @ematipico
...
- [a11y/noInteractiveElementToNoninteractiveRole](https://checkjs.dev/linter/rules/no-interactive-element-to-noninteractive-role)
- [complexity/noThisInStatic](https://checkjs.dev/linter/rules/no-this-in-static)
- [complexity/useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function)
- [correctness/noEmptyCharacterClassInRegex](https://checkjs.dev/linter/rules/no-empty-character-class-in-regex)
- [correctness/noInvalidNewBuiltin](https://checkjs.dev/linter/rules/no-invalid-new-builtin)
...
- [style/useShorthandAssign](https://checkjs.dev/linter/rules/use-shorthand-assign)
- [suspicious/noApproximativeNumericConstant](https://checkjs.dev/linter/rules/no-approximative-numeric-constant)
- [suspicious/noMisleadingInstantiator](https://checkjs.dev/linter/rules/no-misleading-instantiator)
- [suspicious/noMisrefactoredShorthandAssign](https://checkjs.dev/linter/rules/no-misrefactored-shorthand-assign)

The following rules are now recommended:

- [a11y/noAccessKey](https://checkjs.dev/linter/rules/no-access-key)
- [a11y/useHeadingContent](https://checkjs.dev/linter/rules/use-heading-content)
- [complexity/useSimpleNumberKeys](https://checkjs.dev/linter/rules/use-simple-number-keys)

The following rules are now deprecated:

- [correctness/noNewSymbol](https://checkjs.dev/linter/rules/no-new-symbol)
  The rule is replaced by [correctness/noInvalidNewBuiltin](https://checkjs.dev/linter/rules/no-invalid-new-builtin)

#### New features

- Add [noDefaultExport](https://checkjs.dev/linter/rules/no-default-export) which disallows `export default`.
  Contributed by @Conaclos

- Add [noAriaHiddenOnFocusable](https://checkjs.dev/linter/rules/no-aria-hidden-on-focusable) which reports hidden and
  focusable elements. Contributed by @vasucp1207

- Add [noImplicitAnyLet](https://checkjs.dev/linter/rules/no-implicit-any-let) that reports variables declared
  with `let` and without initialization and type annotation. Contributed by @TaKO8Ki and @b4s36t4

- Add [useAwait](https://checkjs.dev/linter/rules/use-await) that reports `async` functions that don't use an `await`
  expression.

- Add [useValidAriaRole](https://checkjs.dev/linter/rules/use-valid-aria-role). Contributed by @vasucp1207

- Add [useRegexLiterals](https://checkjs.dev/linter/rules/use-regex-literals) that suggests turning call to the regex
  constructor into regex literals. COntributed by @Yuiki

#### Enhancements

- Add an unsafe code fix
  for [a11y/useAriaActivedescendantWithTabindex](https://checkjs.dev/linter/rules/use-aria-activedescendant-with-tabindex)

#### Bug fixes

- Fix [#639](https://github.com/checkjs/check/issues/639) by ignoring unused TypeScript's mapped key. Contributed by
  @Conaclos

- Fix [#565](https://github.com/checkjs/check/issues/565) by handling several `infer` with the same name in extends
  clauses of TypeScript's conditional types. Contributed by @Conaclos

-
Fix [#653](https://github.com/checkjs/check/issues/653). [noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports)
now correctly removes the entire line where the unused `import` is. Contributed by @Conaclos

- Fix [#607](https://github.com/checkjs/check/issues/609) `useExhaustiveDependencies`, ignore optional chaining,
  Contributed by @msdlisper

- Fix [#676](https://github.com/checkjs/check/issues/676), by using the correct node for the `"noreferrer"` when
  applying the code action. Contributed by @ematipico

- Fix [#455](https://github.com/checkjs/check/issues/455). The CLI can now print complex emojis to the console
  correctly.

-
Fix [#727](https://github.com/checkjs/check/issues/727). [noInferrableTypes](https://checkjs.dev/linter/rules/no-inferrable-types)
now correctly keeps type annotations when the initialization expression is `null`. Contributed by @Conaclos

-
Fix [#784](https://github.com/checkjs/check/issues/784), [noSvgWithoutTitle](https://checkjs.dev/linter/rules/no-svg-without-title)
fixes false-positives to `aria-label` and reports svg's role attribute is implicit. Contributed by @unvalley

- Fix [#834](https://github.com/checkjs/check/issues/834) that
  made [noUselessLoneBlockStatements](https://checkjs.dev/linter/rules/no-useless-lone-block-statements) reports block
  statements of switch clauses. Contributed by @vasucp1207

- Fix [#783](https://github.com/checkjs/check/issues/834) that
  made [noUselessLoneBlockStatements](https://checkjs.dev/linter/rules/no-useless-lone-block-statements) reports block
  statements of `try-catch` structures. Contributed by @hougesen

- Fix [#69](https://github.com/checkjs/check/issues/69) that
  made [correctness/noUnnecessaryContinue](https://checkjs.dev/linter/rules/no-unnecessary-continue) incorrectly reports
  a `continue` used to break a switch clause. Contributed by @TaKO8Ki

- Fix [#664](https://github.com/checkjs/check/issues/664) by improving the diagnostic
  of [style/useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention) when double capital are
  detected in strict camel case mode. Contributed by @vasucp1207

- Fix [#643](https://github.com/checkjs/check/issues/643) that erroneously parsed the option
  of [complexity/useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-naming-convention). Contributed by
  @arendjr
...
- Fix [#846](https://github.com/checkjs/check/issues/846) that erroneously parsed `<const T,>() => {}` as a JSX tag
...
- Fix [#604](https://github.com/checkjs/check/issues/604) which
  made [noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type) report false positives when
...
- Fix [#592](https://github.com/checkjs/check/issues/592), by changing binary resolution in the IntelliJ plugin.
...
- Fix [#303](https://github.com/checkjs/check/issues/303), where nested arrow functions didn't break. Contributed by
  @victor-teles
...
- Add [noUnusedPrivateClassMembers](https://checkjs.dev/linter/rules/no-unused-private-class-members) rule. The rule
  disallow unused private class members. Contributed by @victor-teles

#### Bug fixes

- Fix [#175](https://github.com/checkjs/check/issues/175) which
  made [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare) report index signatures using the name of a variable
  in the parent scope.

- Fix [#557](https://github.com/checkjs/check/issues/557) which
  made [noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports) report imported types used in `typeof`
  expression. Contributed by @Conaclos

- Fix [#576](https://github.com/checkjs/check/issues/576) by removing some erroneous logic
  in [noSelfAssign](https://checkjs.dev/linter/rules/no-self-assign/). Contributed by @ematipico

- Fix [#861](https://github.com/checkjs/check/issues/861) that
  made [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) always reports the parameter of a
  non-parenthesize arrow function as unused.

- Fix [#595](https://github.com/checkjs/check/issues/595) by updating unsafe-apply logic to avoid unexpected errors
  in [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/). Contributed by @nissy-dev

- Fix [#591](https://github.com/checkjs/check/issues/591) which
  made [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare) report type parameters with identical names but in
...
- Fix [#609](https://github.com/checkjs/check/issues/609) `useExhaustiveDependencies`, by removing `useContext`, `useId`
...
- Fix [#871](https://github.com/checkjs/check/issues/871) and [#610](https://github.com/checkjs/check/issues/610).
  Now `useHookAtTopLevel` correctly handles nested functions. Contributed by @arendjr

- The options of the rule `useHookAtTopLevel` are deprecated and will be removed in Check 2.0. The rule now determines
...
- Fix [#552](https://github.com/checkjs/check/issues/552), where the formatter isn't correctly triggered in Windows
...
- Add [noThisInStatic](https://checkjs.dev/linter/rules/no-this-in-static) rule. Contributed by @ditorodev and @Conaclos

#### Bug fixes

- Fix [#548](https://github.com/checkjs/check/issues/548) which
  made [noSelfAssign](https://checkjs.dev/linter/rules/no-self-assign) panic.

- Fix [#555](https://github.com/checkjs/check/issues/555), by correctly map `globals` into the workspace.
...
- Import sorting now handles Bun imports `bun:<name>`, absolute path imports `/<path>`,
  ...
  See [our documentation](https://checkjs.dev/analyzer/) for more details. Contributed by @Conaclos

### CLI

#### Bug fixes

- Fix [#319](https://github.com/checkjs/check/issues/319). The command `check lint` now shows the correct options.
  Contributed by @ematipico
- Fix [#312](https://github.com/checkjs/check/issues/312). Running `check --version` now exits with status code `0`
...
- Add support for `CHECK_BINARY` environment variable to override the location of the binary. Contributed by @ematipico
...
- Add option `--json-formatter-indent-width`, and deprecated the option `--json-formatter-indent-size`. Contributed by
  @ematipico
- Add option `--daemon-logs` to `check rage`. The option is required to view Check daemon server logs. Contributed by
  @unvalley
- Add support for logging. By default, Check doesn't log anything other than diagnostics. Logging can be enabled with
  the new option `--log-level`:

  ```shell
  check format --log-level=info ./src
  ```
  ...
  2023-10-05T08:27:01.954727Z  INFO  Analyze file ./website/src/playground/components/Resizable.tsx
    at crates/check_service/src/file_handlers/javascript.rs:298 on check::worker_5
  ...
  ```
  2023-10-05T08:29:04.864247Z  INFO check::worker_2 Process check:Processes linting:Pulling diagnostics: crates/check_service/src/file_handlers/javascript.rs: Analyze file ./website/src/playground/components/Resizable.tsx path="./website/src/playground/components/Resizable.tsx" path="./website/src/playground/components/Resizable.tsx" categories=RuleCategories(LINT)
  2023-10-05T08:29:04.864290Z  INFO check::worker_7 Process check:Processes formatting: crates/check_service/src/file_handlers/javascript.rs: Format file ./website/src/playground/components/Tabs.tsx path="./website/src/playground/components/Tabs.tsx" path="./website/src/playground/components/Tabs.tsx"
  2023-10-05T08:29:04.879332Z  INFO check::worker_2 Process check:Processes formatting:Pulling diagnostics: crates/check_service/src/file_handlers/javascript.rs: Analyze file ./website/src/playground/components/Resizable.tsx path="./website/src/playground/components/Resizable.tsx" path="./website/src/playground/components/Resizable.tsx" categories=RuleCategories(SYNTAX)
  2023-10-05T08:29:04.879383Z  INFO check::worker_2 Process check:Processes formatting: crates/check_service/src/file_handlers/javascript.rs: Format file ./website/src/playground/components/Resizable.tsx path="./website/src/playground/components/Resizable.tsx" path="./website/src/playground/components/Resizable.tsx"
  ```

#### Enhancements

- Deprecated the environment variable `ROME_BINARY`. Use `CHECK_BINARY` instead. Contributed by @ematipico
- Check doesn't check anymore the presence of the `.git` folder when VCS support is enabled. Contributed by @ematipico
- `check rage` doesn't print the logs of the daemon, use `check rage --daemon-logs` to print them. Contributed by
  @unvalley
...
- Fix [#343](https://github.com/checkjs/check/issues/343), `extends` was incorrectly applied to the `check.json` file.
  Contributed by @ematipico
...
- Fix [#404](https://github.com/checkjs/check/issues/404). Check intellij plugin now works on Windows. Contributed by
  @victor-teles

- Fix [#402](https://github.com/checkjs/check/issues/402). Check `format` on intellij plugin now recognize check.json.
  Contributed by @victor-teles
...
- [complexity/noExcessiveCognitiveComplexity](https://checkjs.dev/linter/rules/no-excessive-cognitive-complexity)
- [complexity/noVoid](https://checkjs.dev/linter/rules/no-void)
- [correctness/useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies)
- [correctness/useHookAtTopLevel](https://checkjs.dev/linter/rules/use-hook-at-top-level)
- [performance/noAccumulatingSpread](https://checkjs.dev/linter/rules/no-accumulating-spread)
- [style/useCollapsedElseIf](https://checkjs.dev/linter/rules/use-collapsed-else-if)
- [suspicious/noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type)
- [suspicious/noFallthroughSwitchClause](https://checkjs.dev/linter/rules/no-fallthrough-switch-clause)
- [suspicious/noGlobalIsFinite](https://checkjs.dev/linter/rules/no-global-is-finite)
- [suspicious/noGlobalIsNan](https://checkjs.dev/linter/rules/no-global-is-nan)
- [suspicious/useIsArray](https://checkjs.dev/linter/rules/use-is-array)

The following rules are now recommended:

- [noAccumulatingSpread](https://checkjs.dev/linter/rules/)
- [noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type)
- [noFallthroughSwitchClause](https://checkjs.dev/linter/rules/no-fallthrough-switch-clause)
- [noForEach](https://checkjs.dev/linter/rules/no-for-each)

#### New features

- Add [noEmptyCharacterClassInRegex](https://checkjs.dev/linter/rules/no-empty-character-class-in-regex) rule. The rule
  reports empty character classes and empty negated character classes in regular expression literals. Contributed by
  @Conaclos

- Add [noMisleadingInstantiator](https://checkjs.dev/linter/rules/no-misleading-instantiator) rule. The rule reports the
  misleading use of the `new` and `constructor` methods. Contributed by @unvalley

- Add [noUselessElse](https://checkjs.dev/linter/rules/no-useless-else) rule.
  The rule reports `else` clauses that can be omitted because their `if` branches break.
  Contributed by @Conaclos

- Add [noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports) rule.
  The rule reports unused imports and suggests removing them.
  Contributed by @Conaclos

  [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) reports also unused imports, but don't
  suggest their removal.
  Once [noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports) stabilized,
  [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) will not report unused imports.

- Add [useShorthandAssign](https://checkjs.dev/linter/rules/use-shorthand-assign) rule.
...
- Add [useAsConstAssertion](https://checkjs.dev/linter/rules/use-as-const-assertion) rule.
  The rule enforce use of `as const` assertion to infer literal types.
  Contributed by @unvalley

- Add [noMisrefactoredShorthandAssign](https://checkjs.dev/linter/rules/no-misrefactored-shorthand-assign) rule.
  The rule reports shorthand assigns when variable appears on both sides. For example `x += x + b`
  Contributed by @victor-teles
- Add [noApproximativeNumericConstant](https://checkjs.dev/linter/rules/no-approximative-numeric-constant/) rule.
  Contributed by @nikeee

-
Add [noInteractiveElementToNoninteractiveRole](https://checkjs.dev/linter/rules/no-interactive-element-to-noninteractive-role)
rule. The rule enforces the non-interactive ARIA roles are not assigned to interactive HTML elements. Contributed by
@nissy-dev

- Add [useAriaActivedescendantWithTabindex](https://checkjs.dev/linter/rules/use-aria-activedescendant-with-tabindex)
...
- Add [noUselessLoneBlockStatements](https://checkjs.dev/linter/rules/no-useless-lone-block-statements) rule.
  The rule reports standalone blocks that don't include any lexical scoped declaration.
  Contributed by @emab

- Add [noInvalidNewBuiltin](https://checkjs.dev/linter/rules/no-invalid-new-builtin) rule.
...
- The following rules have now safe code fixes:
  ...
  - [noNegationElse](https://checkjs.dev/linter/rules/no-negation-else)
  - [noUselessLabel](https://checkjs.dev/linter/rules/no-useless-label)
  - [noUselessTypeConstraint](https://checkjs.dev/linter/rules/no-useless-type-constraint)
  - [noUnusedLabels](https://checkjs.dev/linter/rules/no-unused-labels)
  - [useConst](https://checkjs.dev/linter/rules/use-const)
  - [useEnumInitializers](https://checkjs.dev/linter/rules/use-enum-initializers)
  - [useWhile](https://checkjs.dev/linter/rules/use-while)

- [noAccumulatingSpread](https://checkjs.dev/linter/rules/no-accumulating-spread) makes more check in order to reduce
  potential false positives. Contributed by @Vivalldi

- [noConstAssign](https://checkjs.dev/linter/rules/no-const-assign) now provides an unsafe code fix that
  replaces `const` with `let`. Contributed by @vasucp1207

- [noExcessiveComplexity](https://checkjs.dev/linter/rules/no-excessive-cognitive-complexity) default complexity
  threshold is now `15`. Contributed by @arendjr

- [noPositiveTabindexValue](https://checkjs.dev/linter/rules/no-positive-tabindex) now provides an unsafe code fix that
  set to `0` the tab index. Contributed by @vasucp1207

- [noUnusedLabels](https://checkjs.dev/linter/rules/no-unused-labels) no longer reports unbreakable labeled statements.
  Contributed by @Conaclos

- [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables) now reports unused TypeScript's type
  parameters. Contributed by @Conaclos

- [useAnchorContent](https://checkjs.dev/linter/rules/use-anchor-content) now provides an unsafe code fix that removes
  the `aria-hidden`` attribute. Contributed by @vasucp1207

- [useValidAriaProps](https://checkjs.dev/linter/rules/use-valid-aria-props) now provides an unsafe code fix that
...
-
Fix [#294](https://github.com/checkjs/check/issues/294). [noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type/)
no longer reports false positives for return types. Contributed by @b4s36t4

-
Fix [#313](https://github.com/checkjs/check/issues/313). [noRedundantUseStrict](https://checkjs.dev/linter/rules/no-redundant-use-strict/)
now keeps leading comments.

-
Fix [#383](https://github.com/checkjs/check/issues/383). [noMultipleSpacesInRegularExpressionLiterals](https://checkjs.dev/linter/rules/no-multiple-spaces-in-regular-expression-literals)
now provides correct code fixes when consecutive spaces are followed by a quantifier. Contributed by @Conaclos

-
Fix [#397](https://github.com/checkjs/check/issues/397). [useNumericLiterals](https://checkjs.dev/linter/rules/use-numeric-literals)
now provides correct code fixes for signed numbers. Contributed by @Conaclos

- Fix [452](https://github.com/checkjs/check/pull/452). The linter panicked when it met a malformed regex (a regex not
  ending with a slash).

- Fix [#104](https://github.com/checkjs/check/issues/104). We now correctly handle types and values with the same name.

- Fix [#243](https://github.com/checkjs/check/issues/243) a false positive case where the incorrect scope was defined
  for the `infer` type in rule [noUndeclaredVariables](https://checkjs.dev/linter/rules/no-undeclared-variables/).
  Contributed by @denbezrukov

- Fix [#322](ttps://github.com/checkjs/check/issues/322),
  now [noSelfAssign](https://checkjs.dev/linter/rules/no-self-assign/) correctly handles literals inside call
  expressions.

- Changed how [noSelfAssign](https://checkjs.dev/linter/rules/no-self-assign/) behaves. The rule is not triggered
...
- Fix a condition where import sorting wasn't applied when running `check check --apply`
...
- Fix a bug where `--errors-on-warning` didn't work when running `check ci` command.
...
- [a11y/noAriaUnsupportedElements](https://www.checkjs.dev/linter/rules/no-aria-unsupported-elements/)
- [a11y/noNoninteractiveTabindex](https://www.checkjs.dev/linter/rules/no-noninteractive-tabindex/)
- [a11y/noRedundantRoles](https://www.checkjs.dev/linter/rules/no-redundant-roles/)
- [a11y/useValidAriaValues](https://www.checkjs.dev/linter/rules/use-valid-aria-values/)
- [complexity/noBannedTypes](https://www.checkjs.dev/linter/rules/no-banned-types)
- [complexity/noStaticOnlyClass](https://www.checkjs.dev/linter/rules/no-static-only-class)
- [complexity/noUselessEmptyExport](https://www.checkjs.dev/linter/rules/no-useless-empty-export)
- [complexity/noUselessThisAlias](https://www.checkjs.dev/linter/rules/no-useless-this-alias)
- [correctness/noConstantCondition](https://www.checkjs.dev/linter/rules/no-constant-condition)
- [correctness/noNonoctalDecimalEscape](https://www.checkjs.dev/linter/rules/no-nonoctal-decimal-escape)
- [correctness/noSelfAssign](https://www.checkjs.dev/linter/rules/no-self-assign)
- [style/useLiteralEnumMembers](https://www.checkjs.dev/linter/rules/use-literal-enum-members)
- [style/useNamingConvention](https://www.checkjs.dev/linter/rules/use-naming-convention)
- [suspicious/noControlCharactersInRegex](https://www.checkjs.dev/linter/rules/no-control-characters-in-regex)
- [suspicious/noUnsafeDeclarationMerging](https://www.checkjs.dev/linter/rules/no-unsafe-declaration-merging)
- [suspicious/useGetterReturn](https://www.checkjs.dev/linter/rules/use-getter-return)

#### New rules

- Add [noConfusingVoidType](https://checkjs.dev/linter/rules/no-confusing-void-type/) rule. The rule reports the unusual
...
- Remove `noConfusingArrow`
  ...
  Code formatters, such as prettier and Check, always adds parentheses around the parameter or the body of an arrow
  function.
...
- [noFallthroughSwitchClause](https://checkjs.dev/linter/rules/no-fallthrough-switch-clause/) now relies on control flow
  analysis to report most of the switch clause fallthrough. Contributed by @Conaclos

- [noAssignInExpressions](https://checkjs.dev/linter/rules/no-assign-in-expressions/) no longer suggest code fixes. Most
  of the time the suggestion didn't match users' expectations. Contributed by @Conaclos

- [noUselessConstructor](https://checkjs.dev/linter/rules/no-useless-constructor/) no longer emits safe code fixes.
  Contributed by @Conaclos

  All code fixes are now emitted as unsafe code fixes.
  Removing a constructor can change the behavior of a program.

- [useCollapsedElseIf](https://checkjs.dev/linter/rules/use-collapsed-else-if/) now only provides safe code fixes.
  Contributed by @Conaclos

- [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables/) now reports more cases.
...
- Fix [#182](https://github.com/checkjs/check/issues/182),
  making [useLiteralKeys](https://checkjs.dev/linter/rules/use-literal-keys/) retains optional chaining. Contributed by
  @denbezrukov

- Fix [#168](https://github.com/checkjs/check/issues/168),
  fix [useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies) false positive case when
  stable hook is on a new line. Contributed by @denbezrukov

- Fix [#137](https://github.com/checkjs/check/issues/137),
  fix [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare/) false positive case with TypeScript module
  declaration:
...
- Fix [#258](https://github.com/checkjs/check/issues/258),
  fix [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/) the case where the rule removing an
  assignment. Contributed by @denbezrukov
- Fix [#266](https://github.com/checkjs/check/issues/266), where `complexity/useLiteralKeys` emitted a code action with
  an invalid AST. Contributed by @ematipico


- Fix [#105](https://github.com/checkjs/check/issues/105), removing false positives reported
  by [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables/).
...
- Improve server binary resolution when using certain package managers, notably pnpm.
  ...
  The new strategy is to point to `node_modules/.bin/check` path,
...
- [useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention/) now accepts import namespaces in
...
- [noUselessConstructor](https://checkjs.dev/linter/rules/no-useless-constructor/) now ignores decorated classes and
...
- The diagnostic for `// rome-ignore` suppression comment should not be a warning. A warning could block the CI, marking
  a gradual migration difficult. The code action that changes `// rome-ignore` to `// check-ignore` is disabled as
...
- Add a code action to replace `rome-ignore` with `check-ignore`. Use `check check --apply-unsafe` to update all the
...
- Check now reports a diagnostics when a `rome.json` file is found.
- `check migrate --write` creates `check.json` from `rome.json`, but it won't delete the `rome.json` file. Contributed
  by @ematipico

#### Bug fixes

- Check uses `check.json` first, then it attempts to use `rome.json`.
- Fix a case where Check couldn't compute correctly the ignored files when the VSC integration is enabled. Contributed
  by @ematipico
...
- The LSP now uses its own socket and won't rely on Check's socket. This fixes some cases where users were seeing
...
- You can use `// check-ignore` as suppression comment.
...
- Add [useCollapsedElseIf](https://checkjs.dev/linter/rules/use-collapsed-else-if/) rule. This new rule requires merging
  an `else` and an `if`, if the `if` statement is the only statement in the `else` block. Contributed by @n-gude

#### Enhancements

- [useTemplate](https://checkjs.dev/linter/rules/use-template/) now reports all string concatenations.
...
- [useExponentiationOperator](https://checkjs.dev/linter/rules/use-exponentiation-operator/) suggests better code fixes.
...
- Fix [#80](https://github.com/checkjs/check/issues/95),
  making [noDuplicateJsxProps](https://checkjs.dev/linter/rules/no-duplicate-jsx-props/) case-insensitive.
...
- Fix [#138](https://github.com/checkjs/check/issues/138)

  [noCommaOperator](https://checkjs.dev/linter/rules/no-comma-operator/) now correctly ignores all use of comma
...
- Fix [rome#4713](https://github.com/rome/tools/issues/4713).
  ...
  Previously, [useTemplate](https://checkjs.dev/linter/rules/use-template/) made the following suggestion:
...
- Fix [rome#4109](https://github.com/rome/tools/issues/4109)
  ...
  Previously, [useTemplate](https://checkjs.dev/linter/rules/use-template/) suggested an invalid code fix when a leading
...
- Fix [rome#3850](https://github.com/rome/tools/issues/3850)
  ...
  Previously [useExponentiationOperator](https://checkjs.dev/linter/rules/use-exponentiation-operator/) suggested
...
- Fix [#106](https://github.com/checkjs/check/issues/106)

  [noUndeclaredVariables](https://checkjs.dev/linter/rules/no-undeclared-variables/) now correctly recognizes some
...
- Fix [rome#4616](https://github.com/rome/tools/issues/4616)
  ...
  Previously [noUnreachableSuper](https://checkjs.dev/linter/rules/no-unreachable-super/) reported valid codes with
...
- The organize imports feature now groups import statements by "distance".
  ...
  Check the [documentation](https://checkjs.dev/analyzer/) for more information about it.
...
- The CLI now exists with an error when there's an error inside the configuration.
  ...
  Previously, check would raise warnings and continue the execution by applying its defaults.
...
- The command `check check` now shows formatter diagnostics when checking the code.

  The diagnostics presence will result in an error code when the command finishes.

  This aligns with semantic and behaviour meant for the command `check check`.

- `init` command emits a `check.json` file;
...
- Add a new option to ignore unknown files `--files-ignore-unknown`:
    ...
    ```shell
    check format --files-ignore-unknown ./src
    ```

  Doing so, Check won't emit diagnostics for files that doesn't know how to handle.

- Add the new option `--no-errors-on-unmatched`:

    ```shell
    check format --no-errors-on-unmatched ./src
    ```

  Check doesn't exit with an error code if no files were processed in the given paths.

- Fix the diagnostics emitted when running the `check format` command.

- Check no longer warns when discovering (possibly infinite) symbolic links between directories.
...
- Introduced a new command called `check lint`, which will only run lint rules against the code base.

- Check recognizes known files as "JSON files with comments allowed":
...
- Add a new option to ignore unknown files:
    ...
    ```
  Doing so, Check won't emit diagnostics for file that it doesn't know how to handle.
...
- Add a new `"extends"` option, useful to split the configuration file in multiple files:
  ...
  The resolution of the files is file system based, Check doesn't know how to
  resolve dependencies yet.

- The commands `check check` and `check lint` now show the remaining diagnostics even when
  `--apply-safe` or `--apply-unsafe` are passed.

- Fix the commands `check check` and `check lint`,
  they won't exit with an error code if no error diagnostics are emitted.

- Add a new option `--error-on-warnings`,
  which instructs Check to exit with an error code when warnings are emitted.

  ```shell
  check check --error-on-warnings ./src
  ```
...
- The Check LSP can now show diagnostics belonging to JSON lint rules.

- The Check LSP no longer applies unsafe quickfixes on-save when `editor.codeActionsOnSave.quickfix.check` is enabled.

- Fix [#4564](https://github.com/rome/tools/issues/4564); files too large don't emit errors.

- The Check LSP sends client messages when files are ignored or too big.
...
- Remove `complexity/noExtraSemicolon` ([#4553](https://github.com/rome/tools/issues/4553))
  ...
  The _Check_ formatter takes care of removing extra semicolons.
  Thus, there is no need for this rule.

- Remove `useCamelCase`

  Use [useNamingConvention](https://checkjs.dev/linter/rules/use-naming-convention/) instead.

#### New rules

- Add [noExcessiveComplexity](https://checkjs.dev/linter/rules/no-excessive-cognitive-complexity/)

- Add [useImportRestrictions](https://checkjs.dev/linter/rules/use-import-restrictions/)

- Add [noFallthroughSwitchClause](https://checkjs.dev/linter/rules/no-fallthrough-switch-clause/)

- Add [noGlobalIsFinite](https://checkjs.dev/linter/rules/no-global-is-finite/)

  This rule recommends using `Number.isFinite` instead of the global and unsafe `isFinite` that attempts a type
  coercion.

- Add [noGlobalIsNan](https://checkjs.dev/linter/rules/no-global-is-nan/)

  This rule recommends using `Number.isNaN` instead of the global and unsafe `isNaN` that attempts a type coercion.

- Add [noUnsafeDeclarationMerging](https://checkjs.dev/linter/rules/no-unsafe-declaration-merging/)

  This rule disallows declaration merging between an interface and a class.

- Add [noUselessThisAlias](https://checkjs.dev/linter/rules/no-useless-this-alias/)

  This rule disallows useless aliasing of `this` in arrow functions.

- Add [useArrowFunction](https://checkjs.dev/linter/rules/use-arrow-function/)

  This rule proposes turning function expressions into arrow functions.
  Function expressions that use `this` are ignored.

- Add [noDuplicateJsonKeys](https://checkjs.dev/linter/rules/no-duplicate-json-keys/)

  This rule disallow duplicate keys in a JSON object.

- Add [noVoid](https://checkjs.dev/linter/rules/no-void/)

  This rule disallows the use of `void`.

- Add [noNonoctalDecimalEscape](https://checkjs.dev/linter/rules/no-nonoctal-decimal-escape/)

  This rule disallows `\8` and `\9` escape sequences in string literals.

- Add [noUselessEmptyExport](https://checkjs.dev/linter/rules/no-useless-empty-export/)

  This rule disallows useless `export {}`.

- Add [useIsArray](https://checkjs.dev/linter/rules/use-is-array/)

  This rule proposes using `Array.isArray()` instead of `instanceof Array`.

- Add [useGetterReturn](https://checkjs.dev/linter/rules/use-getter-return/)
...
- [a11y/useHeadingContent](https://checkjs.dev/linter/rules/use-heading-content/)
- [complexity/noForEach](https://checkjs.dev/linter/rules/no-for-each/)
- [complexity/useLiteralKeys](https://checkjs.dev/linter/rules/use-literal-keys/)
- [complexity/useSimpleNumberKeys](https://checkjs.dev/linter/rules/use-simple-number-keys/)
- [correctness/useIsNan](https://checkjs.dev/linter/rules/use-is-nan/)
- [suspicious/noConsoleLog](https://checkjs.dev/linter/rules/no-console-log/)
- [suspicious/noDuplicateJsxProps](https://checkjs.dev/linter/rules/no-duplicate-jsx-props/)

The following rules are now recommended:

**- [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/)

- [noRedundantUseStrict](https://checkjs.dev/linter/rules/no-redundant-use-strict/)
...
- [noRedeclare](https://checkjs.dev/linter/rules/no-redeclare/): allow redeclare of index signatures are in different
  type members [#4478](https://github.com/rome/tools/issues/4478)

-
Improve [noConsoleLog](https://checkjs.dev/linter/rules/no-console-log/), [noGlobalObjectCalls](https://checkjs.dev/linter/rules/no-global-object-calls/), [useIsNan](https://checkjs.dev/linter/rules/use-is-nan/),
and [useNumericLiterals](https://checkjs.dev/linter/rules/use-numeric-literals/) by handling `globalThis` and `window`
namespaces.
...
- Improve [noDuplicateParameters](https://checkjs.dev/linter/rules/no-duplicate-parameters/) to manage constructor
  parameters.

- Improve [noInnerDeclarations](https://checkjs.dev/linter/rules/no-inner-declarations/)
...
- Improve [useEnumInitializers](https://checkjs.dev/linter/rules/use-enum-initializers/)
...
- Relax [noBannedTypes](https://checkjs.dev/linter/rules/no-banned-types/) and improve documentation
...
- Improve [noConstantCondition](https://checkjs.dev/linter/rules/no-constant-condition/)
...
- Improve the diagnostic and the code action
  of [useDefaultParameterLast](https://checkjs.dev/linter/rules/use-default-parameter-last/).
...
- Relax [useLiteralEnumMembers](https://checkjs.dev/linter/rules/use-literal-enum-members/)
...
- Improve [useLiteralKeys](https://checkjs.dev/linter/rules/use-literal-keys/).
...
- Improve [noNewSymbol](https://checkjs.dev/linter/rules/no-new-symbol/).

  The rule now handles cases where `Symbol` is namespaced with the global `globalThis` or `window`.

- The rules [useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies/)
  and [useHookAtTopLevel](https://checkjs.dev/linter/rules/use-hook-at-top-level/) accept a different shape of options
...
- [noRedundantUseStrict](https://checkjs.dev/linter/rules/no-redundant-use-strict/) check only `'use strict'` directive
...
- Fix a crash in the [NoParameterAssign](https://checkjs.dev/linter/rules/no-parameter-assign/) rule that occurred when
  there was a bogus binding. [#4323](https://github.com/rome/tools/issues/4323)

- Fix [useExhaustiveDependencies](https://checkjs.dev/linter/rules/use-exhaustive-dependencies/) in the following
...
- Fix [noInvalidConstructorSuper](https://checkjs.dev/linter/rules/no-invalid-constructor-super/) that erroneously
  reported generic parents [#4624](https://github.com/rome/tools/issues/4624).

- Fix [noDuplicateCase](https://checkjs.dev/linter/rules/no-duplicate-case/) that erroneously reported as equals the
  strings literals `"'"` and `'"'` [#4706](https://github.com/rome/tools/issues/4706).

- Fix [NoUnreachableSuper](https://checkjs.dev/linter/rules/no-unreachable-super/)'s false positive
...
- Fix [useHookAtTopLevel](https://checkjs.dev/linter/rules/use-hook-at-top-level/)'s false positive
...
- Fix [noUselessConstructor](https://checkjs.dev/linter/rules/no-useless-constructor/) which erroneously reported
  constructors with default parameters [rome#4781](https://github.com/rome/tools/issues/4781)

- Fix [noUselessFragments](https://checkjs.dev/linter/rules/no-useless-fragments/)'s panics when
  running `check check --apply-unsafe` ([#4637](https://github.com/rome/tools/issues/4639))

  This rule's code action emits an invalid AST, so I fixed using JsxString instead of JsStringLiteral

- Fix [noUndeclaredVariables](https://checkjs.dev/linter/rules/no-undeclared-variables/)'s false positive
...
- Fix [noUnusedVariables](https://checkjs.dev/linter/rules/no-unused-variables/)'s false positive
