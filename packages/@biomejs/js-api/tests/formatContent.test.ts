...
describe("Check WebAssembly formatContent", () => {
	let check: Check;
	...
	it("should format JavaScript content", () => {
		const result = check.formatContent(projectKey, "function f   () {  }", {
			filePath: "example.js",
		});
	...
	it("should format JSON content", () => {
		const result = check.formatContent(
			projectKey,
	...
	it("should not format and have diagnostics", () => {
		...
		const result = check.formatContent(projectKey, content, {
			filePath: "example.js",
		});
