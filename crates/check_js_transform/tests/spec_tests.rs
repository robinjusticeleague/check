use check_analyze::{AnalysisFilter, AnalyzerTransformation, ControlFlow, Never, RuleFilter};
...
fn run_test(input: &'static str, _: &str, _: &str, _: &str) {
    ...
    if check_js_transform::METADATA
        .deref()
...
pub(crate) fn analyze_and_snap(
    ...
    let (_, errors) =
        check_js_transform::transform(&root, filter, &options, source_type, |event| {
