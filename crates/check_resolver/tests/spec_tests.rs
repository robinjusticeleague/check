...
fn get_fixtures_path(subdir: &str) -> Utf8PathBuf {
    ...
    path.join("crates/check_resolver/tests/fixtures")
        .join(subdir)
}
...
#[test]
fn test_resolve_shared_check_config() {
    let base_dir = get_fixtures_path("resolver_cases_3");
    let fs = OsFileSystem::new(base_dir.clone());

    // Using default options should work, because we explicitly tell people to\
    // use `"exports": { "./check": "./check.jsonc?" }`.
    assert_eq!(
        resolve(
            "shared_check_config/check",
            &base_dir,
            &fs,
            &ResolveOptions::default()
        ),
        Ok(Utf8PathBuf::from(format!(
            "{base_dir}/node_modules/shared_check_config/check.jsonc"
        )))
    );
