...
use check_json_syntax::{JsonSyntaxKind, JsonSyntaxKind::*, T, TextLen, TextRange, TextSize};
use check_parser::diagnostic::ParseDiagnostic;
use check_unicode_table::{Dispatch::*, is_js_id_continue, is_js_id_start, lookup_byte};
...
impl<'src> Lexer<'src> {
    ...
    /// anyway, and Check should not remove those characters when present, so
    ...
    fn consume_potential_bom(&mut self) -> Option<JsonSyntaxKind> {
        ...
        // to more bytes to support other BOM characters if Check decides to
