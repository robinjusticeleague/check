...
use check_console::fmt::Formatter;
use check_console::markup;
use check_diagnostics::{Category, Diagnostic, Location, LogCategory, Severity, Visit, category};
use check_rowan::{TextLen, TextRange, TextSize};
use std::ops::Add;

/// Single instance of a suppression comment, with the following syntax:
///
/// `// check-ignore { <category> { (<value>) }? }+: <reason>`
///
...
impl Suppression<'_> {
    /// `check-ignore-*` text range relative to the beginning of the comment token.
    ///
    /// For example, in `// check-ignore lint: foo` this is `3..=15`
    ...
    ///
    /// For example, in `// check-ignore lint: foo` this is `22..=25`
...
pub enum SuppressionKind {
    /// Suppressions that start with `// check-ignore`
    Classic,
    /// Suppressions that start with `// check-ignore-all`
    All,
    /// Suppressions that start with `// check-ignore-start`
    RangeStart,
    /// Suppressions that start with `// check-ignore-end`
    RangeEnd,
}
...
impl SuppressionKind {
    pub fn as_str(&self) -> &str {
        match self {
            Self::Classic => "check-ignore",
            Self::All => "check-ignore-all",
            Self::RangeStart => "check-ignore-start",
            Self::RangeEnd => "check-ignore-end",
        }
    }
...
pub fn parse_suppression_comment(
    ...
    comment.lines().filter_map(move |line| {
        ...
        // it's a check-ignore comment
        // Checks for `/check[-_]ignore/i` without a regex, or skip the line
    ...
    fn advices(&self, visitor: &mut dyn Visit) -> std::io::Result<()> {
        match self.message {
            ...
            SuppressionDiagnosticKind::ParseCategory(_) => {
                visitor.record_log(
                    LogCategory::Info,
                    &"Check can't recognize the category, usually it's because it doesn't match the expected ones.",
                )?;
            }
        ...
        visitor.record_log(
            LogCategory::Info,
            &"Example of suppression: // check-ignore lint: reason",
        )
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingColon => {
                write!(
                    f,
                    "Unexpected token, expected one of ':', '(' or whitespace. Example of suppression: // check-ignore lint: reason"
                )
            }
            Self::ParseCategory(category) => {
                write!(
                    f,
                    "Failed to parse category {category:?}. Example of suppression: // check-ignore lint: reason"
                )
            }
            Self::MissingCategory => {
                write!(
                    f,
                    "Incorrect suppression: unexpected token, expected one of ':' or whitespace. Example of suppression: // check-ignore lint: false positive"
                )
            }
            Self::MissingClosingParen => {
                write!(
                    f,
                    "Unexpected token, expected ')'. Example of suppression: // check-ignore lint: false positive"
                )
            }
            Self::MissingReason => {
                write!(
                    f,
                    "Incorrect suppression: missing reason. Example of suppression: // check-ignore lint: false positive"
                )
            }
        }
    }
...
impl check_console::fmt::Display for SuppressionDiagnosticKind {
...
/// `// check-ignore` and the following category.
...
///
/// category is static, predefined in crates/check_diagnostics_categories/src/categories.rs
/// subcategory is dynamic (e.g., user-defined plugin name)
///
/// # Example
/// - No category:      `// check-ignore`                  -> `(None, None)`
/// - Custom Plugin:    `// check-ignore lint/plugin/myPlugin` -> `("lint/plugin", "myPlugin")`
/// - Valid category:   `// check-ignore lint/complexity`  -> `("lint/complexity", None)`
/// - Invalid category: `// check-ignore linx`             -> `Err(SuppressionDiagnostic)`
fn parse_category<'a>(
    base: &'a str,
    category: &'a str,
    ...
    if let Some(rest) = category.strip_prefix("lint/plugin/") {
        ...
        // if user doesn't specify plugin name: e.g. `// check-ignore lint/plugin: reason`
...
mod tests_suppression_kinds {
    ...
    use check_rowan::{TextRange, TextSize};

    #[test]
    fn classic() {
        assert_eq!(
            parse_suppression_comment("// check-ignore format lint: explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
    ...
    fn all() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all format lint: explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
                    (category!("format"), None, None),
    ...
    fn range_start() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-start format lint: explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
                    (category!("format"), None, None),
    ...
    fn range_end() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-end format lint: explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
                    (category!("format"), None, None),
...
mod tests_check_ignore_inline {
    ...
    fn parse_simple_suppression() {
        assert_eq!(
            parse_suppression_comment("// check-ignore parse: explanation1").collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment("/** check-ignore parse: explanation2 */")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![(category!("parse"), None, None)],
                reason: "explanation2",
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * check-ignore parse: explanation3
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * hello
                  * check-ignore parse: explanation4
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment("// check-ignore lint/plugin: explanation5")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment("// check-ignore lint/plugin/myPlugin: explanation6")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
    ...
    fn parse_unclosed_block_comment_suppressions() {
        assert_eq!(
            parse_suppression_comment("/* check-ignore format: explanation").collect::<Vec<_>>(),
            vec![Ok(Suppression {
    ...
    fn parse_multiple_suppression() {
        assert_eq!(
            parse_suppression_comment("// check-ignore parse(foo) parse(dog): explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
        ...
        assert_eq!(
            parse_suppression_comment("/** check-ignore parse(bar) parse(cat): explanation */")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * check-ignore parse(yes) parse(frog): explanation
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * hello
                  * check-ignore parse(wow) parse(fish): explanation
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
    ...
    fn diagnostic_missing_category() {
        assert_eq!(
            parse_suppression_comment("// check-ignore (value): explanation").collect::<Vec<_>>(),
    ...
    fn check_parse_category() {
        assert_eq!(
            parse_category("// check-ignore: reason", ""),
            Ok((None, None))
        );

        assert_eq!(
            parse_category(
                "// check-ignore lint/plugin/myPlugin: reason",
        ...
        assert_eq!(
            parse_category("// check-ignore lint/complexity: reason", "lint/complexity"),
            Ok((Some(category!("lint/complexity")), None))
        );

        let base = "// check-ignore linx: reason";
    ...
    fn diagnostic_missing_paren() {
        assert_eq!(
            parse_suppression_comment("// check-ignore format(:").collect::<Vec<_>>(),
            vec![Err(SuppressionDiagnostic {
                message: SuppressionDiagnosticKind::MissingClosingParen,
    ...
    fn diagnostic_unknown_category() {
        assert_eq!(
            parse_suppression_comment("// check-ignore unknown: explanation").collect::<Vec<_>>(),
            vec![Err(SuppressionDiagnostic {
...
#[cfg(test)]
mod tests_check_ignore_toplevel {
    use check_diagnostics::category;
    use check_rowan::{TextRange, TextSize};
    ...
    fn parse_simple_suppression() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all parse: explanation1")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![(category!("parse"), None, None)],
                reason: "explanation1",
        ...
        assert_eq!(
            parse_suppression_comment("/** check-ignore-all parse: explanation2 */")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![(category!("parse"), None, None)],
                reason: "explanation2",
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * check-ignore-all parse: explanation3
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * hello
                  * check-ignore-all parse: explanation4
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
    ...
    fn parse_unclosed_block_comment_suppressions() {
        assert_eq!(
            parse_suppression_comment("/* check-ignore-all format: explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![(category!("format"), None, None)],
                reason: "explanation",
    ...
    fn parse_multiple_suppression() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all parse(foo) parse(dog): explanation")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
                    (category!("parse"), None, Some("foo")),
        ...
        assert_eq!(
            parse_suppression_comment("/** check-ignore-all parse(bar) parse(cat): explanation */")
                .collect::<Vec<_>>(),
            vec![Ok(Suppression {
                categories: vec![
                    (category!("parse"), None, Some("bar")),
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * check-ignore-all parse(yes) parse(frog): explanation
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
        ...
        assert_eq!(
            parse_suppression_comment(
                "/**
                  * hello
                  * check-ignore-all parse(wow) parse(fish): explanation
                  */"
            )
            .collect::<Vec<_>>(),
            vec![Ok(Suppression {
    ...
    fn diagnostic_missing_category() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all (value): explanation")
                .collect::<Vec<_>>(),
            vec![Err(SuppressionDiagnostic {
                message: SuppressionDiagnosticKind::MissingCategory,
    ...
    fn diagnostic_missing_colon() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all format explanation").collect::<Vec<_>>(),
            vec![Err(SuppressionDiagnostic {
                message: SuppressionDiagnosticKind::MissingColon,
    ...
    fn diagnostic_missing_reason() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all format:").collect::<Vec<_>>(),
            vec![Err(SuppressionDiagnostic {
                message: SuppressionDiagnosticKind::MissingReason,
    ...
    fn diagnostic_unknown_category() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all unknown: explanation")
                .collect::<Vec<_>>(),
            vec![Err(SuppressionDiagnostic {
                message: SuppressionDiagnosticKind::ParseCategory(String::from("unknown")),
...
#[cfg(test)]
mod test_check_ignore_invalid {
    use super::parse_suppression_comment;

    #[test]
    fn parse_invalid_suppression() {
        assert_eq!(
            parse_suppression_comment("// check-ignore-all-start lint: explanation")
                .collect::<Vec<_>>(),
            vec![]
        );
        assert_eq!(
            parse_suppression_comment("// check-ignore-all-end lint: explanation")
                .collect::<Vec<_>>(),
            vec![]
        );
    }
