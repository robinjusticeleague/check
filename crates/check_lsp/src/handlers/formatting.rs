...
use check_service::{WorkspaceError, extension_error};
use std::ops::Sub;
...
pub(crate) fn format(
        ...
        })?
    {
        ...
        let indels = check_text_edit::TextEdit::from_unicode_words(input.as_str(), output.as_str());
...
pub(crate) fn format_range(
        ...
        })?
    {
        ...
        let indels = check_text_edit::TextEdit::from_unicode_words(
...
fn notify_user<T>(file_features: FeaturesSupported, check_path: CheckPath) -> Result<T, LspError> {
    let error = if file_features.is_ignored() {
        WorkspaceError::file_ignored(check_path.to_string())
    } else if file_features.is_protected() {
        WorkspaceError::protected_file(check_path.to_string())
    } else {
        extension_error(&check_path)
    };
