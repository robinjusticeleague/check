...
use check_line_index::LineIndex;
use check_lsp_converters::from_proto;
...
pub(crate) fn code_actions(
    ...
    let mut actions: Vec<_> = result
        ...
        .filter_map(|action| {
            ...
            // Filter out source.organizeImports.check action when assist is not supported.
            if action.category.matches("source.organizeImports.check")
                && !file_features.supports_assist()
            {
                return None;
            }
            // Filter out quickfix.check action when lint and assist aren't
            if action.category.matches("quickfix.check")
            ...
            if action.category.matches("source.check") && !file_features.supports_assist() {
                return None;
            }
...
/// Generate the code action `source.fixAll.check` for the current document
