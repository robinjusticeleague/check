...
use crate::utils;
use anyhow::Result;
use check_analyze::RuleCategoriesBuilder;
use check_configuration::ConfigurationPathHint;
use check_console::markup;
use check_deserialize::Merge;
use check_diagnostics::PrintDescription;
use check_fs::CheckPath;
use check_line_index::WideEncoding;
use check_lsp_converters::{PositionEncoding, negotiated_encoding};
use check_service::Workspace;
use check_service::WorkspaceError;
use check_service::configuration::{
    LoadedConfiguration, ProjectScanComputer, load_configuration, load_editorconfig,
};
use check_service::file_handlers::{AstroFileHandler, SvelteFileHandler, VueFileHandler};
...
use check_service::workspace::{FileFeaturesResult, ServiceNotification};
use check_service::workspace::{RageEntry, RageParams, RageResult, UpdateSettingsParams};
use check_service::workspace::{ScanKind, ScanProjectParams};
use camino::Utf8Path;
...
pub(crate) struct Session {
    ...
    /// The settings of the Check extension (under the `check` namespace)
    ...
    projects: HashMap<CheckPath, ProjectKey>,
...
pub(crate) enum ConfigurationStatus {
    ...
    Error = 2,
    /// The `.editorconfig` file has some errors that check can't solve
    EditorConfigError = 3,
...
impl Session {
    ...
    pub(crate) fn file_path(&self, url: &Uri) -> Result<CheckPath> {
    ...
    async fn update_diagnostics_for_document(
        ...
        let check_path = self.file_path(&url)?;
        ...
        if !self.notified_broken_configuration() {
            if self.configuration_status().is_editorconfig_error() {
                ...
                self.client
                    .show_message(MessageType::WARNING, "The .editorconfig file has errors. Check will report only parsing errors until the file is fixed or its usage is disabled.")
                    .await
            } else if self.configuration_status().is_error() {
                self.set_notified_broken_configuration();
                self.client
                    .show_message(MessageType::WARNING, "The configuration file has errors. Check will report only parsing errors until the configuration is fixed.")
                    .await;
            } else if self.configuration_status().is_plugin_error() {
                self.set_notified_broken_configuration();
                self.client.show_message(MessageType::WARNING, "The plugin loading has failed. Check will report only parsing errors until the file is fixed or its usage is disabled.").await
            }
        }
        ...
        let diagnostics: Vec<Diagnostic> = {
            ...
            let offset = match check_path.extension() {
    ...
    /// This function attempts to read the `check.json` configuration file from
    ...
    pub(crate) async fn load_workspace_settings(self: &Arc<Self>, reload: bool) {
            ...
            .and_then(|s| s.configuration_path())
        {
            ...
            let status = self
                .load_check_configuration_file(ConfigurationPathHint::FromUser(config_path), reload)
                .await;
            ...
            let status = self.load_check_configuration_file(base_path, reload).await;
            self.set_configuration_status(status);
        }
    }
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    pub(super) async fn load_check_configuration_file(
        self: &Arc<Self>,
        ...
        let status = self
            .load_check_configuration_file_internal(base_path.clone(), reload)
            .await;
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    pub(super) async fn load_check_configuration_file_internal(
        self: &Arc<Self>,
        base_path: ConfigurationPathHint,
        force: bool,
    ) -> ConfigurationStatus {
