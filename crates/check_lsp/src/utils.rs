...
use check_analyze::ActionCategory;
use check_console::fmt::Termcolor;
use check_console::fmt::{self, Formatter};
use check_console::{MarkupBuf, markup};
use check_diagnostics::termcolor::NoColor;
...
use check_lsp_converters::{PositionEncoding, from_proto, to_proto};
use check_rowan::{TextRange, TextSize};
use check_service::workspace::CodeAction;
use check_text_edit::{CompressedOp, DiffOp, TextEdit};
use std::any::Any;
use std::borrow::Cow;
...
/// Convert an [check_diagnostics::Diagnostic] to a [lsp::Diagnostic], using the span
...
pub(crate) fn panic_to_lsp_error(err: Box<dyn Any + Send>) -> LspError {
    ...
    match err.downcast::<String>() {
        ...
        Err(err) => match err.downcast::<&str>() {
            ...
            Err(_) => {
                error.message = Cow::Owned(String::from("Check encountered an unknown error"));
            }
        },
    }
...
mod tests {
    ...
    use check_line_index::{LineIndex, WideEncoding};
    use check_lsp_converters::PositionEncoding;
