...
use check_diagnostics::panic::PanicError;
use check_fs::{ConfigName, MemoryFileSystem, OsFileSystem};
...
use check_service::{WatcherInstruction, WorkspaceServer};
...
impl LSPServer {
    ...
    #[tracing::instrument(skip(self), name = "check/rage", level = "debug")]
    async fn rage(&self, params: RageParams) -> LspResult<RageResult> {
        let mut entries = vec![
            RageEntry::section("Server"),
            RageEntry::pair("Version", check_configuration::VERSION),
    ...
    async fn setup_capabilities(&self) {
        ...
        capabilities.add_capability(
            "check_did_change_extension_settings",
                        ...
                        vec![
                            FileSystemWatcher {
                                glob_pattern: GlobPattern::Relative(RelativePattern {
                                    pattern: "**/check.{json,jsonc}".to_string(),
        ...
        capabilities.add_capability(
            "check_did_change_watched_files",
...
impl LanguageServer for LSPServer {
    ...
    async fn initialize(&self, params: InitializeParams) -> LspResult<InitializeResult> {
        info!("Starting Check Language Server...");
        self.is_initialized.store(true, Ordering::Relaxed);

        let server_capabilities = server_capabilities(&params.capabilities);
        if params.root_path.is_some() {
            warn!(
                "The Check Server was initialized with the deprecated `root_path` parameter: this is not supported, use `root_uri` instead"
            );
        }
        ...
        let init = InitializeResult {
            ...
            server_info: Some(ServerInfo {
                ...
                version: Some(check_configuration::VERSION.to_string()),
            }),
        };
    ...
    async fn initialized(&self, _params: InitializedParams) {
        info!("Attempting to load the configuration from 'check.json' file");
    ...
    async fn did_change_watched_files(&self, params: DidChangeWatchedFilesParams) {
        ...
        for file_path in file_paths {
            ...
            if let Some(base_path) = base_path {
                let possible_check_json = file_path.strip_prefix(&base_path);
                if let Ok(watched_file) = possible_check_json
    ...
    async fn code_action(&self, params: CodeActionParams) -> LspResult<Option<CodeActionResponse>> {
        let result = check_diagnostics::panic::catch_unwind(move || {
...
macro_rules! workspace_method {
    ( $builder:ident, $method:ident ) => {
        $builder = $builder.custom_method(
            concat!("check/", stringify!($method)),
            |server: &LSPServer, params| {
                let span = tracing::trace_span!(concat!("check/", stringify!($method)), params = ?params).or_current();
...
impl ServerFactory {
    ...
    pub fn create(&self) -> ServerConnection {
        ...
        builder = builder.custom_method("check/shutdown", |server: &LSPServer, (): ()| {
            info!("Sending shutdown signal");
            server.session.broadcast_shutdown();
            ready(Ok(Some(())))
        });

        builder = builder.custom_method("check/rage", LSPServer::rage);
