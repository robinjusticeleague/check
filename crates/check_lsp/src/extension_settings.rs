...
pub(crate) const CONFIGURATION_SECTION: &str = "check";
...
pub struct WorkspaceSettings {
    ...
    /// Only run Check if a `check.json` configuration file exists.
    pub require_configuration: Option<bool>,

    /// Path to the configuration file to prefer over the default `check.json`.
