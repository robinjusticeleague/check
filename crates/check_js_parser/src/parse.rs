...
use crate::*;
pub use check_js_syntax::{
    AnyJsRoot, JsFileSource, JsLanguage, JsModule, JsScript, JsSyntaxNode, ModuleKind,
};
use check_parser::token_source::Trivia;
use check_parser::{AnyParse, event::Event};
use check_rowan::{AstNode, NodeCache, SyntaxNodeWithOffset};
...
impl<T> Parse<T> {
    ...
    ///
    /// ```
    /// use check_js_parser::{JsParserOptions, parse_script};
    /// use check_js_syntax::{JsIfStatement, JsSyntaxKind};
    /// use check_rowan::{AstNode, AstNodeList};
    ///
...
/// ```
/// use check_js_parser::{JsParserOptions, parse_script};
/// use check_js_syntax::{JsSyntaxToken, JsFileSource, JsSyntaxList, JsComputedMemberExpression};
/// use check_rowan::{AstNode, Direction};
///
...
/// ```
/// use check_js_parser::{JsParserOptions, parse_module};
/// let source = r#"
...
/// ```
/// use check_js_parser::{JsParserOptions, parse_module};
/// use check_js_syntax::JsSyntaxKind;
...
/// use check_js_syntax::{LanguageVariant, LanguageVersion, ModuleKind, JsFileSource};
...
///
/// ```
/// use check_js_parser::{JsParserOptions, parse_js_with_cache};
/// use check_js_syntax::JsFileSource;
/// use check_rowan::NodeCache;
///
...
pub fn parse_js_with_cache(
    ...
    check_parser::event::process(&mut tree_sink, events, errors);
...
impl JsOffsetParse {
    ...
    pub fn syntax(&self) -> &check_rowan::SyntaxNodeWithOffset<JsLanguage> {
        &self.root
    }
    ...
    pub fn base_offset(&self) -> check_rowan::TextSize {
...
/// # Examples
/// ```
/// use check_js_parser::{JsParserOptions, parse_js_with_offset};
/// use check_js_syntax::JsFileSource;
...
pub fn parse_js_with_offset(
    text: &str,
    base_offset: check_rowan::TextSize,
    source_type: JsFileSource,
    options: JsParserOptions,
) -> JsOffsetParse {
    let mut cache = NodeCache::default();
...
mod tests {
    ...
    fn test_offset_parsing_vs_regular_parsing() {
        ...
        let normal_parse = parse_js_with_cache(
            ...
            JsFileSource::js_module(),
            JsParserOptions::default(),
            &mut check_rowan::NodeCache::default(),
        );
