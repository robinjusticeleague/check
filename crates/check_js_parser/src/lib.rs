...
//! - Very easy tree traversal through [`SyntaxNode`](check_rowan::SyntaxNode).
...
//! The crate relies on a concept of untyped [check_js_syntax::JsSyntaxNode]s vs typed [check_rowan::AstNode]s.
//! Syntax nodes represent the syntax tree in an untyped way. They represent a location in an immutable
//! tree with two pointers. The syntax tree is composed of [check_js_syntax::JsSyntaxNode]s and [check_js_syntax::JsSyntaxToken]s in a nested
//! tree structure. Each node can have parents, siblings, children, descendants, etc.
//!
//! [check_rowan::AstNode]s represent a typed version of a syntax node. They have the same exact representation as syntax nodes
...
use check_js_factory::JsSyntaxFactory;
use check_js_syntax::{JsSyntaxKind, LanguageVariant};
...
pub(crate) type JsOffsetLosslessTreeSink<'source> =
    check_parser::tree_sink::OffsetLosslessTreeSink<'source, JsLanguage, JsSyntaxFactory>;
