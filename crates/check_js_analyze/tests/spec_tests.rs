...
use check_diagnostics::advice::CodeSuggestionAdvice;
use check_fs::OsFileSystem;
use check_js_analyze::JsAnalyzerServices;
use check_js_parser::{JsParserOptions, parse};
use check_js_syntax::{AnyJsRoot, EmbeddingKind, JsFileSource, JsLanguage, ModuleKind};
use check_package::PackageType;
use check_plugin_loader::AnalyzerGritPlugin;
use check_rowan::{AstNode, FileSourceError};
use check_service::file_handlers::VueFileHandler;
use check_test_utils::{
...
pub(crate) fn analyze_and_snap(
    ...
    let (_, errors) =
        check_js_analyze::analyze(&root, filter, &options, plugins, services, |event| {
