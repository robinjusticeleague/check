...
use check_rowan::{AstNode, BatchMutationExt, TextRange, TriviaPieceKind, chain_trivia_pieces};
use check_rule_options::{organize_imports::OrganizeImportsOptions, sort_order::SortOrder};
...
declare_source_rule! {
    ...
    /// ```
    ///
    /// Note that you can want to use the lint rule [`useImportType`](https://next.checkjs.dev/linter/rules/use-import-type/) and its [`style`](https://next.checkjs.dev/linter/rules/use-import-type/#style) to enforce the use of `import type` instead of `import { type }`.
    ///
