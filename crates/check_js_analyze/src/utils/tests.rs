...
use check_js_syntax::{JsSyntaxNode, TsIdentifierBinding};
use check_rowan::{AstNode, BatchMutationExt, SyntaxNodeCast};
...
pub fn assert_rename_binding_a_to_b_ok(before: &str, expected: &str) {
    ...
    assert!(!check_test_utils::has_bogus_nodes_or_empty_slots(&root));
}

pub fn assert_rename_ts_binding_a_to_b_ok(before: &str, expected: &str) {
    let r = check_js_parser::parse(before, JsFileSource::tsx(), JsParserOptions::default());
    let model = semantic_model(&r.tree(), SemanticModelOptions::default());
...
pub fn ok_find_attributes_by_name() {
    ...
    let list = r
        .syntax()
        .descendants()
        .find_map(check_js_syntax::JsxAttributeList::cast)
        .unwrap();
