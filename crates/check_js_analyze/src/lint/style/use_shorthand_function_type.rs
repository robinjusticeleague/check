...
use check_js_factory::make::ts_type_alias_declaration;
use check_js_syntax::AnyTsType::TsThisType;
use check_js_syntax::{
...
use check_rowan::{AstNode, AstNodeList, BatchMutationExt, SyntaxNodeOptionExt, TriviaPieceKind};
use check_rule_options::use_shorthand_function_type::UseShorthandFunctionTypeOptions;
...
declare_lint_rule! {
    ...
    pub UseShorthandFunctionType {
        ...
        language: "ts",
        sources: &[RuleSource::EslintTypeScript("prefer-function-type").same()],
        recommended: true,
...
impl Rule for UseShorthandFunctionType {
    ...
    fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
        ...
        if let Some(ts_object_type) = ts_type_member_list.parent::<TsObjectType>() {
            ...
            // available in check_js_formatter/src/ts/types/function_type.rs
