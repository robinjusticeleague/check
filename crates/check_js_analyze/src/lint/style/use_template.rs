...
use check_js_syntax::AnyJsTemplateElement;
use check_js_syntax::{
...
use check_rule_options::use_template::UseTemplateOptions;
...
declare_lint_rule! {
    ...
    pub UseTemplate {
        ...
        name: "useTemplate",
        language: "js",
        sources: &[RuleSource::Eslint("prefer-template").same()],
        recommended: true,
    ...
    while let Some(walk) = iter.next() {
        ...
        match &expression {
                ...
                if binary.operator() == Ok(JsBinaryOperator::Plus) =>
            {
                ...
                // see https://github.com/checkjs/check/issues/4947
