...
use check_analyze::{Rule, RuleDiagnostic, RuleSource, context::RuleContext, declare_lint_rule};
...
use check_rule_options::use_filenaming_convention::UseFilenamingConventionOptions;
...
declare_lint_rule! {
    ...
    /// Check ignore comments cannot be used because the rule applies on filenames not file contents.
    /// To ignore files, you can use [`overrides`](https://checkjs.dev/reference/configuration/#overrides).
        ...
        match state {
            FileNamingConventionState::Ascii => {
                Some(RuleDiagnostic::new(
                    ...
                    "If you want to use non-ASCII filenames, then set the "<Emphasis>"requireAscii"</Emphasis>" option to `false`.\nSee the rule "<Hyperlink href="https://checkjs.dev/linter/rules/use-filenaming-convention#options">"options"</Hyperlink>" for more details."
                }))
            },
                ...
                if options.strict_case && options.filename_cases.cases.contains(Case::Camel) {
                    ...
                    if case_type == Case::Camel && case_strict == Case::Unknown {
                        return Some(RuleDiagnostic::new(
                            ...
                            "If you want to use consecutive uppercase characters in camelCase then consider setting `strictCase` option to `false`.\n Check rule "<Hyperlink href="https://checkjs.dev/linter/rules/use-filenaming-convention#options">"options"</Hyperlink>" for more information."
                        }));
                    }
                }
