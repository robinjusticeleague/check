...
use check_js_syntax::{AnyJsStatement, JsIfStatement, T};
use check_rowan::{AstNode, AstNodeList, BatchMutationExt};
use check_rule_options::use_collapsed_if::UseCollapsedIfOptions;
...
declare_lint_rule! {
    ...
    pub UseCollapsedIf {
        ...
        language: "js",
        sources: &[
            RuleSource::EslintUnicorn("no-lonely-if").same(),
            RuleSource::Clippy("collapsible_if").same(),
        ],
        recommended: false,
