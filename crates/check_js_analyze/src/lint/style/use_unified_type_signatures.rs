...
use check_js_syntax::parameter_ext::{AnyJsParameterList, AnyParameter};
use check_js_syntax::static_value::StaticValue;
use check_js_syntax::{
...
use check_rule_options::use_unified_type_signatures::UseUnifiedTypeSignaturesOptions;
...
macro_rules! enum_type_equals {
    ( $($enum_type:ident : [ $($element:ident),+ $(,)?] ),+ $(,)? ) => {
        $(
            impl TypeEquals for check_js_syntax::$enum_type {
                fn is_type_equal(&self, other: &Self) -> bool {
                    match (&self, &other) {
...
macro_rules! always_type_equals [
    ( $($type:ident),+ $(,)? ) => {
        $(
            impl TypeEquals for check_js_syntax::$type {
...
macro_rules! compare_type_equals {
    ( $( $node_type:ident : [ $($prop:ident),+ $(,)? ] ),+ $(,)? ) => {
        $(
            impl TypeEquals for check_js_syntax::$node_type {
                fn is_type_equal(&self, other: &Self) -> bool {
                    $(
...
macro_rules! bogus_types [
    ( $($bogus:ident),+ $(,)? ) => {
        $(
            impl TypeEquals for check_js_syntax::$bogus {
                fn is_type_equal(&self, other: &Self) -> bool {
...
impl TypeEquals for check_js_syntax::AnyJsExpression {
    fn is_type_equal(&self, other: &Self) -> bool {
...
impl TypeEquals for check_js_syntax::JsSyntaxToken {
    fn is_type_equal(&self, other: &Self) -> bool {
