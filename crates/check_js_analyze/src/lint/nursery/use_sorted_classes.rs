...
use check_rule_options::use_sorted_classes::UseSortedClassesOptions;
...
declare_lint_rule! {
    ...
    /// ## Important notes
    ///
    /// This rule is a work in progress, and is only partially implemented. Progress is being tracked in the following GitHub issue: https://github.com/checkjs/check/issues/1274
    ///
    ...
    ///
    /// The main key difference is that Tailwind CSS and its Prettier plugin read and execute the `tailwind.config.js` JavaScript file, which Check can't do. Instead, Check implements a simpler version of the configuration. The trade-offs are explained below.
    ///
    ...
    ///
    /// The built-in Tailwind CSS preset (enabled by default) contains the set of utilities and variants that are available with the default configuration. More utilities and variants can be added through Tailwind CSS plugins. In Check, these need to be manually specified in the Check configuration file in order to "extend" the preset.
    ///
    /// ### Presets can't be modified
    ///
    /// In Tailwind CSS, core plugins (which provide the default utilities and variants) can be disabled. In Check, however, there is no way to disable parts of a preset: it's all or nothing. A work-around is to, instead of using a preset, manually specify all utilities and variants in the Check configuration file.
    ///
    ...
    ///
    /// This is a deliberate decision. We're unsure about this behavior, and would appreciate feedback on it. If this is a problem for you, please share a detailed explanation of your use case in [the GitHub issue](https://github.com/checkjs/check/issues/1274).
    ///
