use check_analyze::{Ast, Rule, RuleDiagnostic, RuleDomain, RuleSource, declare_lint_rule};
use check_console::markup;
use check_diagnostics::Severity;
use check_js_syntax::{AnyJsExpression, AnyJsxAttributeValue, JsxAttribute};
use check_rowan::AstNode;
use check_rule_options::use_qwik_classlist::UseQwikClasslistOptions;
...
impl Rule for UseQwikClasslist {
    ...
    type State = check_rowan::TextRange;
    type Signals = Option<Self::State>;
    type Options = UseQwikClasslistOptions;

    fn run(ctx: &check_analyze::context::RuleContext<Self>) -> Self::Signals {
        let attr = ctx.query();
    ...
    fn diagnostic(
        _: &check_analyze::context::RuleContext<Self>,
        range: &Self::State,
