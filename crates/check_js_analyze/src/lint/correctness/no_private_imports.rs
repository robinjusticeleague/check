...
use check_module_graph::{JsImportPath, JsModuleInfo, ModuleGraph};
use check_rowan::{AstNode, SyntaxResult, Text, TextRange};
use check_rule_options::no_private_imports::{NoPrivateImportsOptions, Visibility};
...
declare_lint_rule! {
    ...
    /// [overrides](https://checkjs.dev/reference/configuration/#overrides), you
    ...
    /// // See: https://checkjs.dev/reference/configuration/#overrides
...
impl Rule for NoPrivateImports {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        let Some(target_path) = node
            ...
            .filter(|path| !CheckPath::new(path).is_dependency())
        else {
            return Vec::new();
        };
