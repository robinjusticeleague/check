...
use check_analyze::{FixKind, Rule, RuleDiagnostic, context::RuleContext, declare_lint_rule};
use check_console::markup;
use check_diagnostics::Severity;
use check_js_semantic::{ReferencesExtensions, SemanticModel};
use check_js_syntax::binding_ext::{AnyJsBindingDeclaration, AnyJsIdentifierBinding};
use check_js_syntax::declaration_ext::is_in_ambient_context;
use check_js_syntax::{
...
use check_rowan::{AstNode, BatchMutationExt, Direction, SyntaxResult};
use check_rule_options::no_unused_variables::NoUnusedVariablesOptions;
...
declare_lint_rule! {
    ...
    /// pattern among programmers, and Check follows it.
    ///
    /// This rule won't report unused imports.
    /// If you want to report unused imports,
    /// enable [noUnusedImports](https://checkjs.dev/linter/rules/no-unused-imports/).
    ///
    ///
    /// ## Examples
    ///
...
fn suggested_fix_if_unused(
    ...
    match decl.parent_binding_pattern_declaration().unwrap_or(decl) {
        ...
        AnyJsBindingDeclaration::TsInferType(_) => {
            ...
            // See https://github.com/checkjs/check/issues/565
