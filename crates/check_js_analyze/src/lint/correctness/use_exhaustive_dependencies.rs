...
use check_analyze::{FixKind, RuleSource};
use check_analyze::{Rule, RuleDiagnostic, RuleDomain, context::RuleContext, declare_lint_rule};
use check_console::markup;
use check_diagnostics::Severity;
use check_js_factory::make;
use check_js_semantic::{Capture, SemanticModel};
use check_js_syntax::{
...
use check_rowan::{AstNode, AstSeparatedList, BatchMutationExt, SyntaxNodeCast, TriviaPieceKind};
...
use check_rule_options::use_exhaustive_dependencies::{
...
declare_lint_rule! {
    ...
    /// function component() {
    ///     let a = 1;
    ///     // check-ignore lint/correctness/useExhaustiveDependencies(a): suppress dependency a
    ///     useEffect(() => {
    ///         console.log(a);
    ///     }, []);
    /// }
    ...
    ///     let b = 1;
    ///     // check-ignore lint/correctness/useExhaustiveDependencies(a): suppress dependency a
    ///     // check-ignore lint/correctness/useExhaustiveDependencies(b): suppress dependency b
    ///     useEffect(() => {
    ///         console.log(a, b);
    ///     }, []);
    /// }
    ...
    fn diagnostic(ctx: &RuleContext<Self>, dep: &Self::State) -> Option<RuleDiagnostic> {
        match dep {
            ...
            Fix::NonLiteralDependenciesArray { expr } => Some(
                ...
                .note(markup! {"Check can't statically verify whether you've passed the correct dependencies."})
