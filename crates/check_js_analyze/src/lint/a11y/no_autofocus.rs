...
use check_js_syntax::{JsLanguage, JsSyntaxKind, JsxAttribute, jsx_ext::AnyJsxElement};
use check_rowan::{AstNode, BatchMutationExt, WalkEvent};
use check_rule_options::no_autofocus::NoAutofocusOptions;
...
impl Visitor for ValidAutofocusVisitor {
    ...
    fn visit(
        &mut self,
        event: &check_rowan::WalkEvent<check_rowan::SyntaxNode<Self::Language>>,
        mut ctx: check_analyze::VisitorContext<Self::Language>,
    ) {
        match event {
...
impl QueryMatch for ValidAutofocus {
    fn text_range(&self) -> check_rowan::TextRange {
        self.0.range()
    }
}
...
impl Queryable for ValidAutofocus {
    ...
    fn build_visitor(
        analyzer: &mut impl check_analyze::AddVisitor<Self::Language>,
        _: &<Self::Language as check_rowan::Language>::Root,
    ) {
        analyzer.add_visitor(Phases::Syntax, ValidAutofocusVisitor::default);
    }

    fn unwrap_match(_: &check_analyze::ServiceBag, query: &Self::Input) -> Self::Output {
        query.0.clone()
    }
}
