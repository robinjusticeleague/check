...
use check_js_syntax::{AnyJsExpression, JsInExpression, JsInstanceofExpression, is_negation};
use check_rowan::{AstNode, AstNodeExt, BatchMutationExt, declare_node_union};
use check_rule_options::no_unsafe_negation::NoUnsafeNegationOptions;
        ...
        match node {
            JsInOrInstanceOfExpression::JsInstanceofExpression(expr) => {
                ...
                let next_parenthesis_expression = make::parenthesized(
                    check_js_syntax::AnyJsExpression::JsInstanceofExpression(next_expr),
                );
            ...
            JsInOrInstanceOfExpression::JsInExpression(expr) => {
                ...
                let next_expr = expr.clone().replace_node_discard_trivia(
                    left.clone(),
                    check_js_syntax::AnyJsInProperty::AnyJsExpression(argument),
                )?;
                let next_parenthesis_expression = make::parenthesized(
                    check_js_syntax::AnyJsExpression::JsInExpression(next_expr),
                );
                let next_unary_expression = make::js_unary_expression(
                    unary_expression.operator_token().ok()?,
                    AnyJsExpression::JsParenthesizedExpression(next_parenthesis_expression),
