...
use check_js_factory::make::{js_directive, js_directive_list, token};
use check_js_syntax::{JsScript, JsSyntaxKind, JsSyntaxToken, T};
use check_rowan::{AstNode, AstNodeList, BatchMutationExt, TriviaPieceKind};
use check_rule_options::use_strict_mode::UseStrictModeOptions;
...
declare_lint_rule! {
    ...
    ///
    /// Check considers a CommonJS (`.cjs`) file as a script file.
    /// By default, Check recognizes a JavaScript file (`.js`) as a module file, except if `"type": "commonjs"` is specified in `package.json`.
    ///
    /// ## Examples
    ///
    /// ### Invalid
