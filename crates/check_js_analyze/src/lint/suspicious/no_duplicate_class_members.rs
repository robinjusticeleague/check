...
use check_rowan::{AstNode, declare_node_union};
use check_rowan::{AstNodeList, TokenText};
use check_rule_options::no_duplicate_class_members::NoDuplicateClassMembersOptions;
...
fn is_static_member(node: JsSyntaxList) -> bool {
    node.into_iter().any(|m| {
        if let check_rowan::SyntaxSlot::Node(node) = m {
