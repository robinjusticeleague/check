...
use check_rowan::{AstNode, AstNodeExt, BatchMutationExt};
use check_rule_options::use_simplified_logic_expression::UseSimplifiedLogicExpressionOptions;
...
impl Rule for UseSimplifiedLogicExpression {
    ...
    fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
        ...
        match node.operator().ok()? {
            check_js_syntax::JsLogicalOperator::NullishCoalescing
                if matches!(
                    left,
            ...
            check_js_syntax::JsLogicalOperator::LogicalOr => {
            ...
            check_js_syntax::JsLogicalOperator::LogicalAnd => {
                if let AnyJsExpression::AnyJsLiteralExpression(
                    AnyJsLiteralExpression::JsBooleanLiteralExpression(literal),
                ) = left
                {
