...
use check_rule_options::no_excessive_lines_per_function::NoExcessiveLinesPerFunctionOptions;
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        let function_lines_count = func_body
            ...
            .filter(|token| {
                !matches!(
                    token.kind(),
                    check_js_syntax::JsSyntaxKind::L_CURLY | check_js_syntax::JsSyntaxKind::R_CURLY
                )
            })
            .fold(0, |acc, token| {
