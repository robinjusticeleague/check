...
use check_js_factory::make::{ident, js_call_argument_list, js_call_arguments, js_name, token};
use check_js_syntax::{
...
use check_rule_options::no_flat_map_identity::NoFlatMapIdentityOptions;
            ...
            let (function_param, function_body) = match arg.as_any_js_expression()? {
                AnyJsExpression::JsArrowFunctionExpression(arg) => {
                    let parameter: String = match arg.parameters().ok()? {
                        check_js_syntax::AnyJsArrowFunctionParameters::AnyJsBinding(p) => {
                            p.to_trimmed_text().trim_matches(['(', ')']).to_owned()
                        }
                        check_js_syntax::AnyJsArrowFunctionParameters::JsParameters(p) => {
                            if p.items().len() == 1 {
