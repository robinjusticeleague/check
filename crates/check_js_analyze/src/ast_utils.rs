...
use check_rowan::{AstNode, AstSeparatedList, TriviaPiece};
...
mod tests {
    ...
    use check_js_syntax::{AnyJsLiteralExpression, JsFileSource};
    use check_rowan::SyntaxNodeCast;

    use super::get_boolean_value;

    fn assert_boolean_value(code: &str, value: bool) {
        let source = check_js_parser::parse(code, JsFileSource::tsx(), JsParserOptions::default());
