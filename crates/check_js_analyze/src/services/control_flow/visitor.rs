...
use check_analyze::{Visitor, VisitorContext, merge_node_visitors};
use check_js_syntax::{
...
use check_rowan::{AstNode, SyntaxError, SyntaxResult, declare_node_union};
    ...
    ( $vis:vis $name:ident { $( $id:ident: $visitor:ty, )* } ) => {
        ...
        $( impl<'a> MergedVisitor<'a, $visitor> for StatementStack<'a> {
            fn read_top(self) -> SyntaxResult<&'a mut $visitor> {
                #[cfg(debug_assertions)]
                let (_, visitor) =
                    self.$id.1.last_mut().ok_or(::check_rowan::SyntaxError::MissingRequiredChild)?;
                #[cfg(not(debug_assertions))]
                let (_, visitor) =
                    self.$id.last_mut().ok_or(::check_rowan::SyntaxError::MissingRequiredChild)?;
...
impl check_analyze::NodeVisitor<ControlFlowVisitor> for FunctionVisitor {
...
/// Wrapper trait for [check_analyze::NodeVisitor] adding control flow specific
...
/// Wrapper type implementing [check_analyze::NodeVisitor] for types
/// implementing the control-flow specific [NodeVisitor] trait
pub(super) struct VisitorAdapter<V>(SyntaxResult<V>);

impl<V> check_analyze::NodeVisitor<ControlFlowVisitor> for VisitorAdapter<V>
where
    V: NodeVisitor,
{
    type Node = V::Node;
