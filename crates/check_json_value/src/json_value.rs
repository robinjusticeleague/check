//! JSON value types based on `check_json_syntax`.
//!
//! Compared to the CST types defined in `check_json_syntax`, these have the
//! following characteristics:
//!
//! - Values can be constructed from [`check_json_syntax::AnyJsonValue`] with
...
use check_json_syntax::{AnyJsonValue, JsonArrayValue, JsonObjectValue, JsonStringValue};
use check_rowan::TokenText;
use indexmap::IndexMap;
use rustc_hash::FxBuildHasher;

/// JSON value based on types from `check_json_syntax`.
///
...
impl check_deserialize::Deserializable for JsonValue {
    fn deserialize(
