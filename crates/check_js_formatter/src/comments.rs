...
use check_formatter::comments::is_alignable_comment;
use check_formatter::{
    comments::{
...
use check_js_syntax::JsSyntaxKind::JS_EXPORT;
use check_js_syntax::{
...
use check_rowan::{AstNode, SyntaxNodeOptionExt, SyntaxTriviaPieceComments, TextLen};
...
fn handle_continue_break_comment(
    ...
    // continue /* comment */ a;
    // ```
    // This differs from Prettier because other ASTs use an identifier for the label whereas Check uses
    // a token.
    ...
    let following = match comment.following_node() {
            ...
            ) =>
        {
            ...
            let body = if let Some(catch) = JsCatchClause::cast_ref(following) {
                ...
                Err(check_rowan::SyntaxError::MissingRequiredChild)
            };
