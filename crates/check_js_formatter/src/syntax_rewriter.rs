...
use check_formatter::{TransformSourceMap, TransformSourceMapBuilder};
use check_js_syntax::{
...
impl JsFormatSyntaxRewriter {
    ...
    /// trivia is slightly inconsistent with our [check_rowan::SyntaxToken::trailing_trivia] definition as it can now happen that the
    ...
    fn visit_parenthesized(
        ...
        let (l_paren, inner, r_paren) = match (
            ...
            (Ok(l_paren), Ok(inner), Ok(r_paren)) => {
                ...
                // Keep parentheses around unknown expressions. Check can't know the precedence.
                    ...
                    AnyJsExpression::JsLogicalExpression(right_logical) => {
                        match (
                                ...
                                if right_operator.kind() == operator.kind() =>
                            {
                                logical
                                    .with_left(
                                        check_js_factory::make::js_logical_expression(
...
mod tests {
    ...
    use check_formatter::{SourceMarker, TransformSourceMap};
    use check_js_parser::{JsParserOptions, parse, parse_module};
    use check_js_syntax::{
    ...
    use check_rowan::{AstNode, SyntaxRewriter, TextSize};
