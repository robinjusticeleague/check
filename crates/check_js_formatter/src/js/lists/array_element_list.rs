...
use check_js_syntax::JsArrayElementList;
...
pub(crate) fn can_concisely_print_array_list(
    ...
    use check_js_syntax::AnyJsArrayElement::*;
    use check_js_syntax::AnyJsExpression::*;
    use check_js_syntax::JsUnaryOperator::*;
    ...
    list.elements().all(|item| {
        let syntax = match item.into_node() {
            Ok(AnyJsExpression(AnyJsLiteralExpression(
                check_js_syntax::AnyJsLiteralExpression::JsNumberLiteralExpression(literal),
            ))) => literal.into_syntax(),
