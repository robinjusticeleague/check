...
use check_formatter::prelude::{Tag, dynamic_text};
use check_formatter::trivia::{FormatLeadingComments, FormatTrailingComments};
use check_formatter::{
...
use check_js_syntax::{JsLanguage, JsSyntaxNode};
use check_rowan::{AstNode, Direction, SyntaxElement};
use check_text_size::TextRange;
...
impl Format<JsFormatContext> for FormatJsVerbatimNode<'_> {
    fn fmt(&self, f: &mut Formatter<JsFormatContext>) -> FormatResult<()> {
        ...
        if self.format_comments {
            ...
            check_formatter::write!(f, [FormatLeadingComments::Comments(outside_trimmed_range)])?;
        ...
        if self.format_comments {
            ...
            check_formatter::write!(f, [FormatTrailingComments::Comments(outside_trimmed_range)])?;
        }
