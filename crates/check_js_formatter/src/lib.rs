//! Check's official JavaScript formatter.
//!
...
//!
//! `check` has an automatic code generation that creates automatically the files out of the grammar.
...
//!    ```
//!
//! 4. Use the [playground](https://checkjs.dev/playground/) to inspect the code that you want to format.
...
//!    the [playground instructions](https://github.com/checkjs/check/blob/main/website/README.md).
...
//! - `TSX` => `ts/` directory
//!
//! To create a new snapshot test for JavaScript, create a new file to `crates/check_js_formatter/tests/specs/js/`, e.g. `arrow_with_spaces.js`
//!
//! ```javascript
//! const foo     = ()    => {
//!     return bar
...
//!
//! ```bash
//! touch crates/check_js_formatter/tests/spec_tests.rs && cargo test -p check_js_formatter formatter
//! ```
//!
//! For better test driven development flow, start the formatter tests with [`cargo-watch`](https://crates.io/crates/cargo-watch):
//!
//! ```bash
//! cargo watch -i '*.new' -x 'test -p check_js_formatter formatter'
//! ```
//!
...
//! to do it by using the formatter API [check_formatter::trivia::format_removed];
...
use check_formatter::format_element::tag::Label;
use check_formatter::prelude::*;
use check_formatter::trivia::{FormatToken, format_skipped_token_trivia};
use check_formatter::{Buffer, FormatOwnedWithRule, FormatRefWithRule, Formatted, Printed};
use check_formatter::{
...
use check_rowan::{AstNode, SyntaxNode};
...
{
    type Format<'a>
        = check_rowan::SyntaxResult<T::Format<'a>>
    where
        Self: 'a;
...
/// Formats a range within a file, supported by Check
///
...
pub fn format_range(
    ...
    check_formatter::format_range(root, range, JsFormatLanguage::new(options))
}
...
pub fn format_node(
    ...
    check_formatter::format_node(root, JsFormatLanguage::new(options))
}

/// Formats a single node within a file, supported by Check.
///
/// This runs a simple heuristic to determine the initial indentation
/// level of the node based on the provided [JsFormatContext], which
/// must match currently the current initial of the file. Additionally,
...
pub fn format_sub_tree(options: JsFormatOptions, root: &JsSyntaxNode) -> FormatResult<Printed> {
    check_formatter::format_sub_tree(root, JsFormatLanguage::new(options))
}
...
mod tests {
    ...
    use check_formatter::IndentStyle;
    use check_js_parser::{JsParserOptions, parse, parse_script};
