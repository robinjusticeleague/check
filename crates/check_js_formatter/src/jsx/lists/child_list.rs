...
use check_formatter::format_element::tag::{GroupMode, Tag};
use check_formatter::{CstFormatContext, FormatRuleWithOptions, VecBuffer, format_args, write};
use check_js_syntax::{AnyJsxChild, JsxChildList};
...
impl FormatJsxChildList {
    ...
    fn disarm_debug_assertions(&self, node: &JsxChildList, f: &mut JsFormatter) {
        use AnyJsxChild::*;
        use check_js_syntax::{AnyJsExpression, AnyJsLiteralExpression};
