...
pub(crate) mod tests {
    ...
    >(
        ...
        let parse = check_js_parser::parse(input, source_type, JsParserOptions::default());
    ...
    ///
    /// ```
    /// # use check_js_formatter::assert_needs_parentheses;
    /// use check_js_syntax::JsStaticMemberExpression;
    ///
    ...
    macro_rules! assert_needs_parentheses {
        ($input:expr, $Node:ident) => {{ $crate::assert_needs_parentheses!($input, $Node, check_js_syntax::JsFileSource::ts()) }};

        ($input:expr, $Node:ident[$index:expr]) => {{
            $crate::assert_needs_parentheses!(
                $input,
                $Node[$index],
                check_js_syntax::JsFileSource::ts()
            )
        }};
    ...
    /// ```
    /// # use check_js_syntax::JsStaticMemberExpression;
    /// use check_js_formatter::assert_not_needs_parentheses;
    ///
