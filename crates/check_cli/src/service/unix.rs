use check_lsp::{ServerConnection, ServerFactory};
...
fn get_socket_name() -> Utf8PathBuf {
    check_fs::ensure_cache_dir().join(format!("check-socket-{}", check_configuration::VERSION))
}

pub(crate) fn enumerate_pipes() -> io::Result<impl Iterator<Item = (String, Utf8PathBuf)>> {
    fs::read_dir(check_fs::ensure_cache_dir()).map(|iter| {
        iter.filter_map(|entry| {
            let entry = Utf8PathBuf::from_path_buf(entry.ok()?.path()).ok()?;
            let file_name = entry.file_name()?;

            let version = file_name.strip_prefix("check-socket")?;
            if version.is_empty() {
