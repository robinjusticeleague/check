...
use check_console::{ColorMode, Console};
use check_service::{App, Workspace, WorkspaceRef};
...
pub use crate::commands::{CheckCommand, check_command};
...
pub(crate) const VERSION: &str = match option_env!("CHECK_VERSION") {
...
pub const TEMPORARY_INTERNAL_REPORTER_FILE: &str = "__CHECK_INTERNAL_FILE__.json";
...
impl<'app> CliSession<'app> {
    ...
    /// Main function to run Check CLI
    pub fn run(self, command: CheckCommand) -> Result<(), CliDiagnostic> {
        match command {
            CheckCommand::Version(_) => commands::version::full_version(self),
            CheckCommand::Rage(_, daemon_logs, formatter, linter) => {
                commands::rage::rage(self, daemon_logs, formatter, linter)
            }
            CheckCommand::Clean => commands::clean::clean(self),
            CheckCommand::Start {
                log_path,
                log_prefix_name,
            } => commands::daemon::start(self, Some(log_path), Some(log_prefix_name)),
            CheckCommand::Stop => commands::daemon::stop(self),
            CheckCommand::Check {
                write,
                fix,
                unsafe_,
            ...
            CheckCommand::Explain { doc } => commands::explain::explain(self, doc),
            CheckCommand::Init(emit_jsonc) => commands::init::init(self, emit_jsonc),
            CheckCommand::LspProxy {
                log_path,
                log_prefix_name,
                ..
            } => commands::daemon::lsp_proxy(Some(log_path), Some(log_prefix_name)),
            CheckCommand::Migrate {
                cli_options,
                write,
                fix,
                sub_command,
            ...
            CheckCommand::PrintSocket => commands::daemon::print_socket(),
            CheckCommand::WhereAmI => {
