...
pub struct CliOptions {
    ...
    /// Connect to a running instance of the Check daemon server.
    ...
    /// Set the file path to the configuration file, or the directory path to find `check.json` or `check.jsonc`.
    /// If used, it disables the default configuration file resolution.
    #[bpaf(
        long("config-path"),
        env("CHECK_CONFIG_PATH"),
        argument("PATH"),
        optional
    )]
    ...
    /// Tell Check to exit with an error code if some diagnostics emit warnings.
    ...
    /// The level of diagnostics to show. In order, from the lowest to the most important: info, warn, error. Passing `--diagnostic-level=error` will cause Check to print only diagnostics that contain only errors.
    #[bpaf(
