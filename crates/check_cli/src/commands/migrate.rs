...
impl CommandRunner for MigrateCommandPayload {
    ...
    fn get_execution(
        ...
        if let Some(path) = self.configuration_file_path.clone() {
            ...
            console.log(markup! {
            <Info>"If this project has not yet been set up with Check yet, please follow the "<Hyperlink href="https://checkjs.dev/guides/getting-started/">"Getting Started guide"</Hyperlink>" first."</Info>
        });
            Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
                reason: "Check couldn't find the Check configuration file.".to_string(),
            }))
        }
    }
