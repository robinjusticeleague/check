...
use check_configuration::{ConfigurationPathHint, Rules};
...
use check_diagnostics::termcolor::{ColorChoice, WriteColor};
use check_diagnostics::{PrintDescription, termcolor};
use check_flags::check_env;
use check_fs::OsFileSystem;
use check_resolver::FsWithResolverProxy;
use check_service::Workspace;
use check_service::configuration::{LoadedConfiguration, load_configuration};
use check_service::settings::Settings;
use check_service::workspace::{RageEntry, RageParams, client};
        ...
        match session.app.workspace.server_info() {
            ...
            None => {
                session
                    .app
                    .console
                    .log(markup!("Discovering running Check servers..."));
                session.app.console.log(markup!({ RunningCheckServer }));
            }
        }
    }
    Ok(())
...
/// Prints information about other running check server instances.
struct RunningCheckServer;

impl Display for RunningCheckServer {
    fn fmt(&self, f: &mut Formatter) -> io::Result<()> {
        let versions = match enumerate_pipes() {
            Ok(iter) => iter,
            Err(err) => {
                (markup! {<Error>"\u{2716} Enumerating Check instances failed:"</Error>}).fmt(f)?;
                return writeln!(f, " {err}");
            }
        };

        for (version, path) in versions {
            if version == check_configuration::VERSION {
                ...
                match service::open_transport(runtime) {
                    ...
                    Ok(Some(transport)) => {
                        let header = "Running Check Server: ";
                        let width = {
...
"<Info>"\u{2139} The client isn't connected to any server but rage discovered this running Check server."</Info>"
")
                .fmt(f)?;
                ...
                CheckServerLog.fmt(f)?;
            } else {
                let header = "Incompatible Check Server: ";
                let width = {
                    if cfg!(debug_assertions) {
                        78
                    } else {
...
"<Info>"\u{2139} Rage discovered this running server using an incompatible version of Check."</Info>"
")
        .fmt(f)?;
...
impl Display for RageConfiguration<'_> {
    fn fmt(&self, fmt: &mut Formatter) -> io::Result<()> {
        Section("Check Configuration").fmt(fmt)?;
...
struct CheckServerLog;

impl Display for CheckServerLog {
    fn fmt(&self, fmt: &mut Formatter) -> io::Result<()> {
        if let Ok(Some(log)) = read_most_recent_log_file(
            check_env().check_log_path.value().map(Utf8PathBuf::from),
            check_env()
                .check_log_prefix_name
                .value()
                .unwrap_or("server.log".to_string()),
        ) {
            markup!("\n"<Emphasis><Underline>"Check Server Log:"</Underline></Emphasis>"
...
/// Prints the server logs but only if the client is connected to a check server.
