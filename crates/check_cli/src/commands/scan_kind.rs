...
use check_service::workspace::ScanKind;
use camino::Utf8Path;
    ...
    if let Some(stdin) = execution.as_stdin_file() {
            ...
            .is_some_and(|dir| dir == root_configuration_dir)
        {
            ...
            return Some(ScanKind::TargetedKnownFiles {
                target_paths: vec![CheckPath::new(working_dir.join(path))],
...
/// - Returns [ScanKind::KnownFiles] for `check format`, `check migrate`, and
///   `check search`, because we know there is no use for project analysis with
///   these commands.
...
mod tests {
    ...
    use check_configuration::{LinterConfiguration, analyzer::RuleSelector};
