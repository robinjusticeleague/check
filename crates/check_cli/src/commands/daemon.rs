...
use check_lsp::ServerFactory;
use check_service::{TransportError, Watcher, WorkspaceError, workspace::WorkspaceClient};
...
pub(crate) fn start(
    ...
    if did_spawn {
        session.app.console.log(markup! {
            "The Check server was successfully started"
        });
    } else {
        session.app.console.log(markup! {
            "The Check server was already running"
        });
    }
...
pub(crate) fn stop(session: CliSession) -> Result<(), CliDiagnostic> {
    ...
    if let Some(transport) = open_transport(rt)? {
        ...
        session.app.console.log(markup! {
            "The Check server was successfully stopped"
        });
    } else {
        session.app.console.log(markup! {
            "The Check server was not running"
        });
    }
...
pub(crate) fn read_most_recent_log_file(
    ...
    let check_log_path = log_path.unwrap_or(default_check_log_path());

    let most_recent = fs::read_dir(check_log_path)?
        .flatten()
...
/// `check-logs/server.log.yyyy-MM-dd-HH` files inside the system temporary
/// directory)
fn setup_tracing_subscriber(log_path: Option<&Utf8Path>, log_file_name_prefix: Option<&str>) {
    let check_log_path = log_path.map_or_else(
...
pub fn default_check_log_path() -> Utf8PathBuf {
    match env::var_os("CHECK_LOG_PATH") {
        Some(directory) => Utf8PathBuf::from(directory.as_os_str().to_str().unwrap()),
        None => check_fs::ensure_cache_dir().join("check-logs"),
    }
}

/// Tracing filter enabling:
/// - All spans and events at level info or higher
/// - All spans and events at level debug in crates whose name starts with `check`
struct LoggingFilter;

/// Tracing filter used for spans emitted by `check*` crates
...
impl LoggingFilter {
    fn is_enabled(&self, meta: &Metadata<'_>) -> bool {
        let filter = if meta.target().starts_with("check") {
            SELF_FILTER
        } else {
            LevelFilter::INFO
        };
