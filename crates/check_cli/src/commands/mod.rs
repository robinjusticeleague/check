...
use check_configuration::analyzer::assist::AssistEnabled;
use check_configuration::analyzer::{LinterEnabled, RuleSelector};
use check_configuration::css::{CssFormatterConfiguration, CssLinterConfiguration};
use check_configuration::formatter::FormatterEnabled;
use check_configuration::graphql::{GraphqlFormatterConfiguration, GraphqlLinterConfiguration};
use check_configuration::html::{HtmlFormatterConfiguration, html_formatter_configuration};
use check_configuration::javascript::{JsFormatterConfiguration, JsLinterConfiguration};
use check_configuration::json::{JsonFormatterConfiguration, JsonLinterConfiguration};
use check_configuration::vcs::VcsConfiguration;
use check_configuration::{CheckDiagnostic, Configuration};
...
use check_console::{Console, ConsoleExt, markup};
use check_deserialize::Merge;
use check_diagnostics::{Diagnostic, PrintDiagnostic, Severity};
use check_fs::{CheckPath, FileSystem};
use check_grit_patterns::GritTargetLanguage;
use check_resolver::FsWithResolverProxy;
use check_service::configuration::{
    LoadedConfiguration, ProjectScanComputer, load_configuration, load_editorconfig,
};
use check_service::documentation::Doc;
use check_service::projects::ProjectKey;
...
/// Check official CLI. Use it to check the health of your project or run it to check single files.
pub enum CheckCommand {
    /// Shows the Check version information and quit.
    #[bpaf(command)]
    ...
    Rage(
        ...
        /// Prints the Check daemon server logs
    ...
    Start {
        ...
        #[bpaf(
            ...
            argument("PATH"),
            hide_usage,
            fallback(check_fs::ensure_cache_dir().join("check-logs")),
        )]
        log_path: Utf8PathBuf,
    },

    /// Stops the Check daemon server process.
    #[bpaf(command)]
    Stop,
    ...
    Check {
        ...
        /// extension of the file. Based on the extension, Check knows how to
        /// check the code.
        ///
        ...
        /// Example:
        /// ```shell
        /// echo 'let a;' | check check --stdin-file-path=file.js --write
        /// ```
        ...
        /// flag and the `defaultBranch` is not set in your `check.json`
    ...
    Lint {
        ...
        ///
        /// Example: `check lint --only=correctness/noUnusedVariables --only=suspicious`
        ...
        ///
        /// Example: `check lint --skip=correctness/noUnusedVariables --skip=suspicious`
        #[bpaf(long("skip"), argument("GROUP|RULE"))]
        skip: Vec<RuleSelector>,

        /// Use this option when you want to format code piped from `stdin`, and print the output to `stdout`.
        ///
        /// The file doesn't need to exist on disk, what matters is the extension of the file. Based on the extension, Check knows how to lint the code.
        ///
        /// Example:
        /// ```shell
        /// echo 'let a;' | check lint --stdin-file-path=file.js --write
        /// ```
        #[bpaf(long("stdin-file-path"), argument("PATH"), hide_usage)]
        stdin_file_path: Option<String>,
        /// When set to true, only the files that have been staged (the ones prepared to be committed)
    ...
    Format {
        ...
        /// Use this option when you want to format code piped from `stdin`, and print the output to `stdout`.
        ///
        /// The file doesn't need to exist on disk, what matters is the extension of the file. Based on the extension, Check knows how to format the code.
        ///
        /// Example:
        /// ```shell
        /// echo 'let a;' | check format --stdin-file-path=file.js --write
        /// ```
        #[bpaf(long("stdin-file-path"), argument("PATH"), hide_usage)]
        stdin_file_path: Option<String>,
    ...
    /// Bootstraps a new check project. Creates a configuration file with some defaults.
    #[bpaf(command)]
    Init(
        /// Tells Check to emit a `check.jsonc` file.
    ...
    Search {
        ...
        /// Example:
        /// ```shell
        /// echo 'let a;' | check search '`let $var`' --stdin-file-path=file.js
        /// ```
        #[bpaf(long("stdin-file-path"), argument("PATH"), hide_usage)]
        stdin_file_path: Option<String>,
    ...
    RunServer {
        ...
        #[bpaf(
            ...
            hide_usage,
            fallback(
                check_fs::ensure_cache_dir().join("check-logs")
            ),
        )]
        log_path: Utf8PathBuf,
...
pub enum MigrateSubCommand {
    /// It attempts to find the files `.prettierrc`/`prettier.json` and `.prettierignore`, and map the Prettier's configuration into Check's configuration file.
    #[bpaf(command)]
    Prettier,
    /// It attempts to find the ESLint configuration file in the working directory, and update the Check's configuration file as a result.
    #[bpaf(command)]
    Eslint {
...
pub(crate) fn validate_configuration_diagnostics(
    ...
    if loaded_configuration.has_errors() {
        ...
        return Err(CliDiagnostic::workspace_error(
            CheckDiagnostic::invalid_configuration(
                "Check exited because the configuration resulted in errors. Please fix them.",
            )
            .into(),
        ));
    }
...
pub(crate) fn print_diagnostics_from_workspace_result(
    diagnostics: &[check_diagnostics::serde::Diagnostic],
    console: &mut dyn Console,
    verbose: bool,
) -> Result<(), CliDiagnostic> {
...
pub(crate) trait CommandRunner: Sized {
    ...
    fn configure_workspace(
        ...
        let open_project_result = workspace.open_project(OpenProjectParams {
            path: CheckPath::new(project_dir),
        ...
        let result = workspace.update_settings(UpdateSettingsParams {
            ...
            workspace_directory: Some(CheckPath::new(project_dir)),
            configuration,
        })?;
        if self.should_validate_configuration_diagnostics() {
        ...
        let scan_kind = match (scan_kind, execution.traversal_mode()) {
            ...
            (ScanKind::KnownFiles, _) => {
                let target_paths = paths
                    .iter()
                    .map(|path| CheckPath::new(working_dir.join(path)))
                    .collect();
...
pub(crate) struct ConfiguredWorkspace {
    ...
    pub configuration_files: Vec<CheckPath>,
    ...
    fn combine_configuration(
        ...
        Ok(
            if let Some(mut fs_configuration) =
                self.load_editor_config(configuration_path, &check_configuration, fs)?
            {
                // If both `check.json` and `.editorconfig` exist, formatter settings from the check.json take precedence.
                fs_configuration.merge_with(check_configuration);
                fs_configuration
            } else {
                check_configuration
            },
...
mod tests {
    ...
    #[test]
    fn check_options() {
        check_command().check_invariants(false);
    }
}
