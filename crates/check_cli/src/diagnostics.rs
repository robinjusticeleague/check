...
/// A diagnostic that is emitted when running check via CLI.
///
...
pub enum CliDiagnostic {
    ...
    /// Wrapper for an underlying `check_service` error
    WorkspaceError(WorkspaceError),
    /// Wrapper for an underlying `std::io` error
    IoError(IoDiagnostic),
    /// The daemon is not running
    ServerNotRunning(ServerNotRunning),
    /// The end configuration (`check.json` + other options) is incompatible with the command
    ...
    /// Errors thrown when running the `check migrate` command
...
#[diagnostic(
    category = "internalError/io",
    severity = Error,
    message = "No running instance of the Check daemon server was found."
)]
// TODO: add advice
...
#[diagnostic(
    ...
    advice = "Check your check.json or check.jsonc to ensure the paths are not ignored by the configuration.",
...
impl CliDiagnostic {
    ...
    /// Emitted when the end configuration (`check.json` file + CLI arguments + LSP configuration)
        ...
        match self {
            ...
            Self::NoExtension => {
                fmt.write_markup(markup!{
                    "The file passed via "<Emphasis>"--stdin-file-path"</Emphasis>" doesn't have an extension. Check needs a file extension to know how to handle the file."
                })
            }
        }
    }
