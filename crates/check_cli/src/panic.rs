...
fn panic_handler(info: &PanicHookInfo) {
    ...
    writeln!(error, "Check encountered an unexpected error").unwrap();
    writeln!(error).unwrap();

    writeln!(error, "This is a bug in Check, not an error in your code, and we would appreciate it if you could report it to https://github.com/checkjs/check/issues/ along with the following information to help us fixing the issue.").unwrap();
    writeln!(error).unwrap();
