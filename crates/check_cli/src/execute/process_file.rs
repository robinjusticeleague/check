...
use check_diagnostics::{DiagnosticExt, DiagnosticTags, Error, category};
...
pub(crate) fn process_file(ctx: &TraversalOptions, check_path: &CheckPath) -> FileResult {
    let _ = tracing::trace_span!("process_file", path = ?check_path).entered();
    ...
    } else if file_features.is_not_supported() || !DocumentFileSource::can_read(check_path) {
        return Err(Message::from(
            UnhandledDiagnostic.with_file_path(check_path.to_string()),
        ));
    }
    ...
    if let Some(reason) = unsupported_reason {
        match reason {
            ...
            SupportKind::Protected => {
                return Ok(FileStatus::Protected(check_path.to_string()));
            }
    ...
    match ctx.execution.traversal_mode {
        ...
        TraversalMode::Format { .. } => {
            // the unsupported case should be handled already at this point
            format(shared_context, check_path.clone())
        }
        TraversalMode::Check { .. } | TraversalMode::CI { .. } => {
            check_file(shared_context, check_path.clone(), file_features)
        }
        ...
        TraversalMode::Search { ref pattern, .. } => {
            // the unsupported case should be handled already at this point
            search(shared_context, check_path.clone(), pattern)
        }
    }
}
