...
use check_analyze::AnalysisFilter;
...
use check_fs::{CheckPath, ConfigName, OpenOptions};
use check_json_parser::{JsonParserOptions, parse_json_with_cache};
use check_json_syntax::{JsonFileSource, JsonRoot};
use check_migrate::{ControlFlow, migrate_configuration};
use check_rowan::{AstNode, NodeCache};
...
pub(crate) struct MigratePayload<'a> {
    ...
    pub(crate) nested_configuration_files: Vec<CheckPath>,
}
...
struct MigrateFile<'a> {
    ...
    pub(crate) project_key: ProjectKey,
    pub(crate) write: bool,
    pub(crate) configuration_file_path: &'a CheckPath,
    pub(crate) sub_command: Option<&'a MigrateSubCommand>,
}

#[derive(Debug, Default)]
struct MigrationResultDiagnostic {
    outcomes: Vec<(CheckPath, MigrationFileResult)>,
}
...
impl Diagnostic for MigrationResultDiagnostic {
    ...
    fn advices(&self, visitor: &mut dyn Visit) -> std::io::Result<()> {
            ...
            .any(|(_, result)| result == &MigrationFileResult::NeedsMigration)
        {
            ...
            visitor.record_command("check migrate --write")?;
        }
        Ok(())
    }
}
...
impl check_console::fmt::Display for MigrationFileResult {
...
fn migrate_file(payload: MigrateFile) -> Result<MigrationFileResult, CliDiagnostic> {
    ...
    let mut check_config_file =
        fs.open_with_options(configuration_file_path.as_path(), open_options)?;
    let mut check_config_content = String::new();
    check_config_file.read_to_string(&mut check_config_content)?;

    let check_path = CheckPath::new(configuration_file_path.as_path());
    ...
    workspace.open_file(OpenFileParams {
        ...
        content: FileContent::from_client(&check_config_content),
        document_file_source: Some(
            JsonFileSource::try_from(check_path.as_path())
                .unwrap_or_default()
                .into(),
        ),
        persist_node_cache: false,
    })?;
    let parsed = parse_json_with_cache(&check_config_content, &mut cache, parse_options);
    ...
    match sub_command {
        Some(MigrateSubCommand::Prettier) => {
            ...
            let Some(mut check_config) = check_config else {
                return Ok(MigrationFileResult::HasErrors);
            };
            let old_check_config = check_config.clone();
            let prettier_check_config = prettier_config.try_into().map_err(|err| {
                CliDiagnostic::MigrateError(MigrationDiagnostic {
                    reason: format!("{err:#}",),
                })
            })?;
            check_config.merge_with(prettier_check_config);
            if let Ok(ignore_patterns) = ignorefile::read_ignore_file(fs, prettier::IGNORE_FILE) {
                ...
                if write && check_config != old_check_config {
                    console.log(markup!{
                        <Info><Emphasis>{prettier::IGNORE_FILE}</Emphasis>" has been successfully migrated."</Info>
                    });
                }
            }
            if check_config == old_check_config {
                console.log(markup! {
                    <Info>"No changes to apply to the Check configuration file."</Info>
                });
                Ok(MigrationFileResult::NoMigrationNeeded)
            } else {
                let new_content = serde_json::to_string(&check_config).map_err(|err| {
                    CliDiagnostic::MigrateError(MigrationDiagnostic {
                ...
                if write {
                    check_config_file.set_content(printed.as_code().as_bytes())?;
                    console.log(markup!{
        ...
        Some(MigrateSubCommand::Eslint {
            ...
            let (check_eslint_config, mut results) =
                eslint_config.into_check_config(&eslint_to_check::MigrationOptions {
                    include_inspired: *include_inspired,
                    include_nursery: *include_nursery,
                });
            let old_check_config = check_config.clone();
            check_config.merge_with(check_eslint_config);
            ...
            let result = if check_config == old_check_config {
        ...
        None => {
            ...
            let is_root = workspace.fs().working_directory().is_some_and(|wd| {
                configuration_file_path.strip_prefix(wd).is_ok_and(|path| {
                    path.starts_with(ConfigName::check_json())
                        || path.starts_with(ConfigName::check_jsonc())
                })
            });
            loop {
            ...
            if check_config_content != new_configuration_content {
                if write {
                    let mut configuration_file = check_config_file;
                    workspace.change_file(ChangeFileParams {
                        project_key,
                        path: check_path.clone(),
