...
use super::{eslint_any_rule_to_check::migrate_eslint_any_rule, eslint_eslint, eslint_typescript};
use check_configuration::analyzer::SeverityOrGroup;
use check_configuration::{self as check_config};
use check_console::markup;
use check_deserialize::Merge;
use check_diagnostics::Location;
use check_rule_options::no_restricted_globals;
use rustc_hash::FxHashMap;

/// This modules includes implementations for converting an ESLint config to a Check config.
///
/// The conversion relies on:
/// - the generated [super::eslint_any_rule_to_check::migrate_eslint_any_rule]
///   module that relies on Check's rule metadata to determine
///   the equivalent Check's rule of an Eslint rule
/// - hand-written handling of Check rules that have options in the current module.
...
pub(crate) struct MigrationResults {
    ...
    /// Is the Check configuration updated?
...
impl check_diagnostics::Diagnostic for MigrationResults {
    fn category(&self) -> Option<&'static check_diagnostics::Category> {
        Some(check_diagnostics::category!("migrate"))
    }

    fn severity(&self) -> check_diagnostics::Severity {
        check_diagnostics::Severity::Information
    }

    fn location(&self) -> check_diagnostics::Location<'_> {
    ...
    fn message(&self, fmt: &mut check_console::fmt::Formatter<'_>) -> std::io::Result<()> {
    ...
    fn advices(&self, visitor: &mut dyn check_diagnostics::Visit) -> std::io::Result<()> {
        if !self.migrated.is_empty() && self.migrated.len() != self.rule_count() {
            visitor.record_log(
                check_diagnostics::LogCategory::Info,
                &if self.write {
            ...
            let list: Vec<_> = self
                .migrated
                .iter()
                .map(|item| item as &dyn check_console::fmt::Display)
                .collect();
...
impl check_console::fmt::Display for EslintRuleName {
    fn fmt(&self, fmt: &mut check_console::fmt::Formatter) -> std::io::Result<()> {
        fmt.write_fmt(format_args!("{self}"))
    }
}

impl eslint_eslint::AnyConfigData {
    pub(crate) fn into_check_config(
        self,
        options: &MigrationOptions,
    ) -> (check_config::Configuration, MigrationResults) {
        match self {
            Self::Flat(config) => config.into_check_config(options),
            Self::Legacy(config) => config.into_check_config(options),
        }
    }
}
...
impl eslint_eslint::FlatConfigData {
    pub(crate) fn into_check_config(
        ...
        let mut check_config = check_config::Configuration::default();
        let mut linter = check_config::LinterConfiguration::default();
        let mut overrides = check_config::Overrides::default();
        let global_config_object = if self.0.len() == 1 {
            ...
            for flat_config_object in self.0 {
                if flat_config_object.is_global_ignores() {
                    ...
                    let mut override_pat = check_config::OverridePattern::default();
                    if let Some(language_options) = flat_config_object.language_options {
                        let globals = language_options
                            .globals
                            .enabled()
                            .collect::<rustc_hash::FxHashSet<_>>();
                        let js_config = check_config::JsConfiguration {
                    ...
                    let includes =
                        to_check_includes(&flat_config_object.files, &flat_config_object.ignores);
                    override_pat.includes = (!includes.is_empty())
                        .then_some(check_configuration::OverrideGlobs::Globs(includes.into()));
                    if let Some(rules) = flat_config_object.rules
                        && !rules.is_empty()
                    {
                        override_pat.linter = Some(check_config::OverrideLinterConfiguration {
                            rules: Some(rules.into_check_rules(options, &mut results)),
                            ..Default::default()
                        });
                    }
            ...
            if !overrides.0.is_empty() {
                check_config.overrides = Some(overrides);
            }
            global_config_object
        };
        let mut rules = if let Some(rules) = global_config_object.rules {
            rules.into_check_rules(options, &mut results)
        } else {
            check_config::Rules::default()
        };
        if let Some(language_options) = global_config_object.language_options {
            ...
            check_config.javascript = Some(js_config)
        }
        rules.recommended = Some(false);
        linter.rules = Some(rules);
        let includes =
            to_check_includes(&global_config_object.files, &global_config_object.ignores);
        linter.includes = (!includes.is_empty()).then_some(includes);
        check_config.linter = Some(linter);
...
impl eslint_eslint::LegacyConfigData {
    pub(crate) fn into_check_config(
        ...
        let mut linter = check_config::LinterConfiguration::default();
        let mut rules = self.rules.into_check_rules(options, &mut results);
        rules.recommended = Some(false);
        linter.rules = Some(rules);
        let includes = to_check_includes(&[] as &[&str], self.ignore_patterns.as_slice());
        linter.includes = (!includes.is_empty()).then_some(includes);
        if !self.overrides.is_empty() {
            let mut overrides = check_config::Overrides::default();
            for override_elt in self.overrides {
                let mut override_pattern = check_config::OverridePattern::default();
                ...
                let includes = to_check_includes(&override_elt.files, &override_elt.excluded_files);
                override_pattern.includes = (!includes.is_empty())
                    .then_some(check_configuration::OverrideGlobs::Globs(includes.into()));
                if !override_elt.rules.is_empty() {
                    override_pattern.linter = Some(check_config::OverrideLinterConfiguration {
                        rules: Some(override_elt.rules.into_check_rules(options, &mut results)),
                        ..Default::default()
                    });
                }
...
impl eslint_eslint::Rules {
    pub(crate) fn into_check_rules(
        self,
        options: &MigrationOptions,
        results: &mut MigrationResults,
    ) -> check_config::Rules {
        let mut rules = check_config::Rules::default();
...
/// Look for an equivalent Check rule for ESLint `rule`,
/// and then mutate `rules` if an equivalent rule is found.
/// Also, takes care of Check's rules with options.
fn migrate_eslint_rule(
    rules: &mut check_config::Rules,
    ...
    match rule {
        ...
        eslint_eslint::Rule::NoConsole(conf) => {
                ...
                && let eslint_eslint::RuleConf::Option(severity, rule_options) = conf
            {
                ...
                if let SeverityOrGroup::Group(group) = group {
                    group.no_console = Some(check_config::RuleFixConfiguration::WithOptions(
                        check_config::RuleWithFixOptions {
                            level: severity.into(),
                            fix: None,
            ...
            if migrate_eslint_any_rule(rules, &name, conf.severity(), opts, results) {
                ...
                if let SeverityOrGroup::Group(group) = group {
                    group.no_restricted_globals =
                        Some(check_config::RuleConfiguration::WithOptions(
                            check_config::RuleWithOptions {
                                level: severity.into(),
                                options: *Box::new(
                ...
                && let eslint_eslint::RuleConf::Option(severity, rule_options) = conf
            {
                ...
                if let SeverityOrGroup::Group(group) = group {
                    group.use_valid_aria_role =
                        Some(check_config::RuleFixConfiguration::WithOptions(
                            check_config::RuleWithFixOptions {
                                level: severity.into(),
                                fix: None,
                                options: *Box::new((*rule_options).into()),
        ...
        eslint_eslint::Rule::TypeScriptConsistentTypeImports(conf) => {
                ...
                && let eslint_eslint::RuleConf::Option(severity, rule_options) = conf
            {
                let group = rules.style.get_or_insert_with(Default::default);
                if let SeverityOrGroup::Group(group) = group {
                    group.use_import_type = Some(check_config::RuleFixConfiguration::WithOptions(
                        check_config::RuleWithFixOptions {
                            level: severity.into(),
                            fix: None,
                            options: rule_options.into(),
...
fn to_check_includes(
    files: &[impl AsRef<str>],
    ignores: &[impl AsRef<str>],
) -> Vec<check_glob::NormalizedGlob> {
    let mut includes: Vec<check_glob::NormalizedGlob> = Vec::new();
    if !files.is_empty() {
    ...
    if !ignores.is_empty() {
        ...
        includes.extend(ignores.iter().filter_map(|glob| {
            ...
            if let Some(rest) = glob.as_ref().strip_prefix('!') {
                ...
                glob.as_ref()
                    .parse()
                    .map(|glob: check_glob::NormalizedGlob| glob.negated())
            }
            .ok()
        }));
    }
...
mod tests {
    use super::*;
    use check_configuration::OverrideGlobs;
    use eslint_eslint::*;
    use std::borrow::Cow;
    ...
    fn flat_config_single_config_object() {
        ...
        let (check_config, _) = flat_config.into_check_config(&MigrationOptions::default());

        assert!(check_config.files.is_none());
        assert!(check_config.overrides.is_none());
        assert!(check_config.formatter.is_none());
        assert!(check_config.assist.is_none());
        let linter = check_config.linter.unwrap();
        assert_eq!(
    ...
    fn flat_config_multiple_config_object() {
        ...
        assert_eq!(
            linter
                ...
                .unwrap_group()
                .no_double_equals,
            Some(check_config::RuleFixConfiguration::Plain(
                check_config::RulePlainConfiguration::Error
            ))
        );
        let overrides = check_config.overrides.unwrap();
        ...
        assert_eq!(
            override0
                ...
                .no_double_equals,
            Some(check_config::RuleFixConfiguration::Plain(
                check_config::RulePlainConfiguration::Off
            ))
        );
    }
}
