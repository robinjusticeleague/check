...
use check_deserialize::Deserializable;
use check_deserialize_macros::Deserializable;
use check_rule_options::restricted_regex::RestrictedRegex;
use check_rule_options::{
...
impl NamingConventionSelection {
    ...
    fn selectors(&self) -> Vec<use_naming_convention::Selector> {
        ...
        for selector in self.selector.iter() {
            match selector {
                Selector::AutoAccessor => {} // currently unsupported by Check
                Selector::Class => {
        ...
        // This avoids to generate errors when loading the Check configuration.
...
impl Deserializable for Anything {
    fn deserialize(
        _ctx: &mut impl check_deserialize::DeserializationContext,
        _value: &impl check_deserialize::DeserializableValue,
        _name: &str,
    ) -> Option<Self> {
        Some(Self)
    }
