...
use check_configuration::HtmlConfiguration;
...
use check_diagnostics::{DiagnosticExt, PrintDiagnostic};
use check_formatter::{
    AttributePosition, BracketSpacing, Expand, IndentWidth, LineEnding, LineWidth, QuoteStyle,
};
use check_fs::{FileSystem, OpenOptions};
use check_html_formatter::context::SelfCloseVoidElements;
use check_js_formatter::context::{ArrowParentheses, QuoteProperties, Semicolons, TrailingCommas};
...
impl TryFrom<PrettierConfiguration> for check_configuration::Configuration {
    type Error = Error;
    fn try_from(value: PrettierConfiguration) -> anyhow::Result<Self> {
        ...
        let indent_style = if value.use_tabs {
            check_formatter::IndentStyle::Tab
        } else {
            check_formatter::IndentStyle::Space
        };
        let formatter = check_configuration::FormatterConfiguration {
        ...
        let js_config = check_configuration::JsConfiguration {
        ...
        if !value.overrides.is_empty() {
            let mut overrides = check_configuration::Overrides::default();
...
impl TryFrom<Override> for check_configuration::OverridePattern {
    type Error = Error;
    fn try_from(Override { files, options }: Override) -> anyhow::Result<Self> {
        let mut result = Self {
            includes: Some(check_configuration::OverrideGlobs::Globs(
                files
                    .into_iter()
            ...
            || options.end_of_line.is_some()
        {
            ...
            let formatter = check_configuration::OverrideFormatterConfiguration {
                indent_width,
                line_width,
                indent_style,
...
pub(crate) fn read_config_file(
    ...
    Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
        reason: "Check couldn't find a Prettier configuration file.".to_string(),
    }))
}
...
fn load_config(
    ...
    if !(deserialized.is_none() && diagnostics.len() > 2 || path.extension().is_none()) {
        for diagnostic in diagnostics.into_iter().filter(|diag| {
            matches!(
                diag.severity(),
                check_diagnostics::Severity::Fatal
                    | check_diagnostics::Severity::Error
                    | check_diagnostics::Severity::Warning
            )
        }) {
    ...
    if let Some(result) = deserialized {
        if result.end_of_line == EndOfLine::Auto {
            console.log(markup! {
                <Warn>"Prettier's `\"endOfLine\": \"auto\"` option is not supported in Check. The default `\"lf\"` option is used instead."</Warn>
            });
        }
        Ok(result)
