...
use check_diagnostics::{Category, category};
use check_diagnostics::{Resource, SerdeJsonError};
...
pub enum TraversalMode {
    /// This mode is enabled when running the command `check check`
    Check {
    ...
    /// This mode is enabled when running the command `check lint`
    Lint {
        /// The type of fixes that should be applied when analyzing a file.
        ///
        ...
        /// A flag to know vcs integrated options such as `--staged` or `--changed` are enabled
        vcs_targeted: VcsTargeted,
        /// Suppress existing diagnostics with a `// check-ignore` comment
        suppress: bool,
    ...
    /// This mode is enabled when running the command `check ci`
    CI {
    ...
    /// This mode is enabled when running the command `check format`
    Format {
        /// It skips parse errors
        skip_parse_errors: bool,
    ...
    /// This mode is enabled when running the command `check migrate`
    Migrate {
        /// Write result to disk
        write: bool,
        /// The path to `check.json`
        configuration_file_path: Utf8PathBuf,
        sub_command: Option<MigrateSubCommand>,
    },
    /// This mode is enabled when running the command `check search`
    Search {
...
pub fn execute_mode(
    ...
    nested_configuration_files: Vec<CheckPath>,
    ...
    if let Some(stdin) = execution.as_stdin_file() {
        let check_path = CheckPath::new(stdin.as_path());
        return std_in::run(
            session,
            project_key,
            &execution,
    ...
    match execution.report_mode {
        ...
        ReportMode::Json { pretty } => {
            ...
            if pretty {
                ...
                let report_file = CheckPath::new(TEMPORARY_INTERNAL_REPORTER_FILE);
