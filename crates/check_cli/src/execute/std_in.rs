...
use check_diagnostics::PrintDiagnostic;
...
pub(crate) fn run<'a>(
    ...
    if check_path.extension().is_none() {
    ...
    if mode.is_format() {
        ...
        if file_features.is_protected() {
            let protected_diagnostic = WorkspaceError::protected_file(check_path.to_string());
        ...
        if file_features.supports_format() {
            ...
            let output = match check_path.extension() {
