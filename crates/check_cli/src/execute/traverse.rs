...
use check_diagnostics::DiagnosticTags;
use check_diagnostics::{DiagnosticExt, Error, Resource, Severity, category};
use check_fs::{CheckPath, FileSystem, PathInterner};
use check_fs::{TraversalContext, TraversalScope};
...
use check_service::{Workspace, WorkspaceError, extension_error, workspace::SupportsFeatureParams};
...
pub(crate) struct TraverseResult {
    ...
    pub(crate) evaluated_paths: BTreeSet<CheckPath>,
...
pub(crate) fn traverse(
    ...
    let (duration, evaluated_paths, diagnostics) = thread::scope(|s| {
        let handler = thread::Builder::new()
            .name(String::from("check::console"))
...
pub(crate) struct TraversalOptions<'ctx, 'app> {
    ...
    pub(crate) evaluated_paths: RwLock<BTreeSet<CheckPath>>,
}

impl TraversalOptions<'_, '_> {
    pub(crate) fn increment_changed(&self, path: &CheckPath) {
    ...
    pub(crate) fn miss_handler_err(&self, err: WorkspaceError, check_path: &CheckPath) {
        let file_path = self
            .fs
            .working_directory()
            .as_ref()
            ...
            .unwrap_or(check_path.to_string());
    ...
    /// Sends a diagnostic regarding the use of a protected file that can't be handled by Check
    pub(crate) fn protected_file(&self, check_path: &CheckPath) {
        self.push_diagnostic(WorkspaceError::protected_file(check_path.to_string()).into())
    }
}
...
impl TraversalContext for TraversalOptions<'_, '_> {
    ...
    fn evaluated_paths(&self) -> BTreeSet<CheckPath> {
    ...
    #[instrument(level = "debug", skip(self, check_path))]
    fn can_handle(&self, check_path: &CheckPath) -> bool {
        if check_path
            .file_name()
        ...
        let can_read = DocumentFileSource::can_read(check_path);
        ...
        let file_features = match file_features {
            Ok(FileFeaturesResult {
                ...
                if file_features.is_protected() {
                    self.protected_file(check_path);
                    return false;
                }
                ...
                if file_features.is_not_supported() && !file_features.is_ignored() && !can_read {
                    ...
                    self.miss_handler_err(extension_error(check_path), check_path);
                    return false;
                }
                file_features
            }
            Err(err) => {
                self.miss_handler_err(err, check_path);
                return false;
            }
        };
        match self.execution.traversal_mode() {
            ...
            // Imagine if Check can't handle its own configuration file...
            TraversalMode::Migrate { .. } => true,
            TraversalMode::Search { .. } => file_features.supports_search(),
        }
    }

    fn handle_path(&self, path: CheckPath) {
        handle_file(self, &path)
    }

    fn store_path(&self, path: CheckPath) {
        self.evaluated_paths
            .write()
            .unwrap()
            .insert(CheckPath::new(path.as_path()));
    }
}
...
fn handle_file(ctx: &TraversalOptions, path: &CheckPath) {
