...
use check_console::{Console, ConsoleExt, fmt, markup};
...
impl ReporterVisitor for ConsoleReporterVisitor<'_> {
    fn report_summary(
        ...
        if execution.is_check() && summary.suggested_fixes_skipped > 0 {
            self.0.log(markup! {
                ...
                <Info>"If you wish to apply the suggested (unsafe) fixes, use the command "<Emphasis>"check check --write --unsafe\n"</Emphasis></Info>
            })
        }
    ...
    fn report_handled_paths(
        &mut self,
        evaluated_paths: BTreeSet<CheckPath>,
