...
use check_diagnostics::display::SourceFile;
use check_diagnostics::{Error, PrintDescription, Resource, Severity};
...
impl Display for GitLabDiagnostics<'_> {
    fn fmt(&self, fmt: &mut Formatter) -> std::io::Result<()> {
        ...
        let gitlab_diagnostics: Vec<_> = payload
            ...
            .filter_map(|check_diagnostic| {
                let absolute_path = match check_diagnostic.location().resource {
                ...
                let initial_fingerprint = self.compute_initial_fingerprint(check_diagnostic, &path);
