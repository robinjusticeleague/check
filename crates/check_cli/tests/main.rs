...
use check_cli::{CliDiagnostic, CliSession, check_command};
use check_console::{BufferConsole, Console, ConsoleExt, markup};
use check_fs::{MemoryFileSystem, OsFileSystem};
use check_resolver::FsWithResolverProxy;
use check_service::App;
...
pub(crate) fn run_cli_with_dyn_fs(
    ...
    use check_cli::SocketTransport;
    use check_lsp::ServerFactory;
    use check_service::{WorkspaceRef, workspace};
    use tokio::{
        io::{duplex, split},
        runtime::Runtime,
    };
    ...
    let command = check_command().run_inner(args);
    match command {
