...
#[test]
fn creates_config_file_when_check_installed_via_package_manager() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();

    let file_path = Utf8Path::new("./node_modules/@checkjs/check/configuration_schema.json");
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "creates_config_file_when_check_installed_via_package_manager",
        fs,
        console,
        result,
    ));
