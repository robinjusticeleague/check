...
use check_console::{BufferConsole, LogLevel, MarkupBuf, markup};
use check_fs::{ErrorEntry, FileSystemExt, MemoryFileSystem, OsFileSystem, TemporaryFs};
use bpaf::Args;
...
fn no_lint_if_linter_is_disabled_when_run_apply() {
    ...
    let config_path = Utf8Path::new("check.json");
...
#[test]
fn no_lint_if_linter_is_disabled_when_run_apply_check_jsonc() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();
    ...
    let config_path = Utf8Path::new("check.jsonc");
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "no_lint_if_linter_is_disabled_when_run_apply_check_jsonc",
        fs,
        console,
        result,
    ));
...
fn downgrade_severity() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();
    let file_path = Utf8Path::new("check.json");
    fs.insert(
        file_path.into(),
...
fn fs_files_ignore_symlink() {
    ...
    let config_path = root_path.join("check.json");
...
fn include_files_in_subdir() {
    ...
    let _ = remove_dir_all(&root_path);
    create_dir(&root_path).unwrap();
    File::create(root_path.join("check.json"))
        .unwrap()
...
fn include_files_in_symlinked_subdir() {
    ...
    File::create(subroot_path.join("check.json"))
        .unwrap()
        .write_all(config.as_bytes())
        .unwrap();
...
fn file_too_large_config_limit() {
    ...
    fs.insert(Utf8PathBuf::from("check.json"), CONFIG_FILE_SIZE_LIMIT);
...
fn suppression_syntax_error() {
    ...
    let file_path = Utf8Path::new("check.js");
    fs.insert(file_path.into(), *b"// check-ignore(:\n");
...
fn group_level_recommended_false_enable_specific() {
    ...
    let config_path = Utf8Path::new("check.json");
    fs.insert(config_path.into(), check_json.as_bytes());
...
fn check_json_files() {
    ...
    let configuration = Utf8Path::new("check.json");
    fs.insert(
        configuration.into(),
        r#"{
	"linter": {
...
// check-ignore lint/performance/noDelete: <explanation>
delete obj.prop;
"#;
...
fn should_report_when_schema_version_mismatch() {
    ...
    let check_json = Utf8Path::new("check.json");
    fs.insert(
        check_json.into(),
        r#"{
    "$schema": "https://checkjs.dev/schemas/0.0.1/schema.json"
}
        "#,
    );
    let (fs, result) = run_cli(
        fs,
        &mut console,
        Args::from(["check", check_json.as_str()].as_slice()),
    );
...
fn should_lint_module_in_commonjs_package() {
    ...
    fs.create_file("check.json", r#"{ "files": { "includes": ["**/*"] } }"#);
...
fn linter_enables_project_domain_based_on_extended_config() {
    ...
    fs.insert(
        Utf8Path::new("check.json").into(),
        r#"{ "extends": ["check.base.json"] }"#.as_bytes(),
    );

    fs.insert(
        Utf8Path::new("check.base.json").into(),
        r#"{
    "linter": {
        "rules": {
            "nursery": {
...
fn should_apply_root_settings_with_stdin_file_path_and_extended_config() {
    ...
    fs.create_file(
        ...
        r#"{
  "exports": {
    "./check": "./configs/check.jsonc"
  }
}"#,
    );

    fs.create_file(
        "node_modules/base-config/configs/check.jsonc",
        r#"{
    "formatter": {
        "indentStyle": "space",
        "indentWidth": 3,
...
fn should_not_choke_on_recursive_function_call() {
    ...
    // Use a hook name to catch https://github.com/checkjs/check/issues/6915.
    fs.create_file(
