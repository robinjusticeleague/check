...
fn migrate_eslintrcjson() {
    let checkjson = r#"{ "linter": { "enabled": true } }"#;
    let eslintrc = r#"{
        "ignorePatterns": [
    ...
    fs.insert(Utf8Path::new("check.json").into(), checkjson.as_bytes());
...
fn migrate_eslintrcjson_override_existing_config() {
    let checkjson = r#"{ "linter": { "rules": { "recommended": true, "suspicious": { "noDoubleEquals": "error" } } } }"#;
...
#[test]
fn migrate_eslintrcjson_missing_checkjson() {
    let eslintrc = r#"{}"#;
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "migrate_eslintrcjson_missing_checkjson",
        fs,
        console,
        result,
    ));
