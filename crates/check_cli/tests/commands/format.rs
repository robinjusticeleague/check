...
use check_console::{BufferConsole, MarkupBuf, markup};
...
fn custom_config_file_path() {
    ...
    let config_path = Utf8Path::new("/test/check.json");
...
#[test]
fn applies_configuration_from_check_jsonc() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();

    let file_path = Utf8Path::new("check.jsonc");
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "applies_configuration_from_check_jsonc",
        fs,
        console,
        result,
    ));
...
fn html_disabled_by_default() {
    ...
    assert!(matches!(
        result,
        Err(check_cli::CliDiagnostic::NoFilesWereProcessed(_))
    ));
