...
use check_fs::{ErrorEntry, FileSystemExt, MemoryFileSystem, OsFileSystem};
use bpaf::Args;
...
fn check_stdin_applies_the_config_based_on_path() {
    ...
    fs.insert(
        "check.json".into(),
        r#"{
  "$schema": "./node_modules/@checkjs/check/configuration_schema.json",
  "linter": {
    "enabled": true,
    "rules": {
      "recommended": true
...
fn should_show_formatter_diagnostics_for_files_ignored_by_linter() {
    ...
    fs.insert(
        check_json.into(),
        r#"{
            "$schema": "https://checkjs.dev/schemas/1.6.1/schema.json",
            "assist": {
                "enabled": true
            },
            "linter": {
...
.dont-report-this-error-please {
    /* check-ignore lint/plugin/noManualZIndex: Should not report */
    z-index: 2;
}
"#,
    );
