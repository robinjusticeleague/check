...
use check_console::{BufferConsole, Markup, markup};
use check_diagnostics::termcolor::NoColor;
use check_diagnostics::{Error, print_diagnostic_to_string};
use check_formatter::{IndentStyle, IndentWidth};
use check_fs::{ConfigName, FileSystemExt, MemoryFileSystem};
...
impl CliSnapshot {
    pub fn emit_content_snapshot(&self) -> String {
        ...
        for (name, file_content) in &self.files {
            if !name.starts_with("check.json") {
...
fn redact_snapshot(input: &str) -> Option<Cow<'_, str>> {
    ...
    // Normalize the name of the current executable to "check"
    let current_exe = current_exe()
        .ok()
        .and_then(|path| Some(path.file_name()?.to_str()?.to_string()));

    if let Some(current_exe) = current_exe {
        replace(&mut output, &current_exe, "check");
    }

    output = replace_temp_dir(output);
    output = replace_check_dir(output);
...
/// Replace the path to the temporary directory with "<TEMP_DIR>"
/// And normalizes the count of `-` at the end of the diagnostic
fn replace_check_dir(input: Cow<str>) -> Cow<str> {
    let mut result = String::new();
    let mut rest = input.as_ref();

    let temp_dir = check_fs::ensure_cache_dir().to_string();
    let temp_dir = temp_dir.trim_end_matches(MAIN_SEPARATOR);
