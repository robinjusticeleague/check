...
fn extends_config_ok_from_npm_package() {
    ...
    fs.insert(
        check_json.into(),
        r#"{ "extends": ["@shared/format/check", "@shared/linter/check"] }"#,
    );

    fs.insert(
        "node_modules/@shared/format/check.json".into(),
        r#"{ "javascript": { "formatter": { "quoteStyle": "single" } } }"#,
    );
    fs.insert(
    ...
    fs.insert(
        "node_modules/@shared/linter/check.jsonc".into(),
...
// See: https://github.com/checkjs/check/issues/6217
#[test]
fn extends_config_ok_from_npm_package_with_author_field() {
    ...
    fs.insert(
        ...
        r#"{
    "name": "@shared/format",
    "author": "this breaks extending check",
    "exports": {
        "./check": "./check.json"
    }
}"#,
...
fn extends_config_ok_from_npm_package_with_condition_names() {
    ...
    fs.insert(
        check_json.into(),
        r#"{ "extends": ["@shared/format", "@shared/linter/check"] }"#,
    );
...
fn extends_resolves_when_using_config_path() {
    ...
    let check_json = Utf8Path::new("config/check.json");
    fs.insert(
        check_json.into(),
        r#"{ "extends": ["format.json", "linter.json"] }"#,
    );
