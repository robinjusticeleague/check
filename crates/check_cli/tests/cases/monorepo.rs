...
fn should_fail_for_nested_roots() {
    ...
    let file_path1 = Utf8Path::new("check.json");
    fs.create_file(file_path1.as_str(), ROOT);

    let file_path2 = Utf8Path::new("packages/lib/check.json");
...
"#,
    ...
    // we simulate the run of check from a nested package
...
"#,
    ...
    fs.create_file("packages/lib/check.json", r#"{ "extends": "//" }"#);
...
fn should_not_lint_when_root_is_disabled_but_nested_is_enabled() {
    ...
    fs.create_file("check.json", r#"{ "linter": {"enabled": false } }"#);

    fs.create_file(
        "packages/lib/check.json",
