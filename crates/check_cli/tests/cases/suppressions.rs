...
use check_fs::{FileSystemExt, MemoryFileSystem};
use bpaf::Args;
use camino::Utf8Path;

const SUPPRESS_BEFORE: &str = "(1 >= -0)";
const SUPPRESS_AFTER: &str =
    "// check-ignore lint/suspicious/noCompareNegZero: ignored using `--suppress`\n(1 >= -0)";

const SUPPRESS_WITH_REASON: &str =
    "// check-ignore lint/suspicious/noCompareNegZero: We love Check\n(1 >= -0)";
...
/**
* check-ignore-all lint/style/useConst: reason
* check-ignore-all lint/suspicious/noDebugger: reason
*/
debugger
let bar = 33;",
    );
...
fn syntax_rule_range_suppression() {
    ...
    fs.insert(
        file_path.into(),
        *b"
// check-ignore-start syntax/correctness/noTypeOnlyImportAttributes: bug
import type { ChalkInstance } from \"chalk\" with { \"resolution-mode\": \"import\" };
import type { ChalkInstance2 } from \"chalk2\" with { \"resolution-mode\": \"import\" };
// check-ignore-end syntax/correctness/noTypeOnlyImportAttributes: bug
...
fn syntax_rule_top_suppression() {
    ...
    fs.insert(
        file_path.into(),
        *b"
// check-ignore-all syntax/correctness/noTypeOnlyImportAttributes: bug
import type { ChalkInstance } from \"chalk\" with { \"resolution-mode\": \"import\" };
import type { ChalkInstance2 } from \"chalk2\" with { \"resolution-mode\": \"import\" };
...
fn should_emit_diagnostics_for_incorrect_reason() {
    ...
    fs.insert(
        file_path.into(),
        *b"// check-ignore-all lint/style/useConst:
var foo = 2;
// check-ignore-all lint/style/useConst: <explanation>
var bar = 33;",
    );
