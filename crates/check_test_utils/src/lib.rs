...
use check_analyze::options::{JsxRuntime, PreferredQuote};
use check_analyze::{AnalyzerAction, AnalyzerConfiguration, AnalyzerOptions};
use check_configuration::Configuration;
use check_console::fmt::{Formatter, Termcolor};
use check_console::markup;
use check_diagnostics::termcolor::Buffer;
use check_diagnostics::{DiagnosticExt, Error, PrintDiagnostic};
use check_fs::{CheckPath, FileSystem, MemoryFileSystem, OsFileSystem};
use check_js_analyze::JsAnalyzerServices;
use check_js_parser::{AnyJsRoot, JsFileSource, JsParserOptions};
use check_js_type_info::{TypeData, TypeResolver};
use check_json_parser::{JsonParserOptions, ParseDiagnostic, parse_json};
use check_module_graph::ModuleGraph;
use check_package::PackageJson;
use check_project_layout::ProjectLayout;
use check_rowan::{Direction, Language, SyntaxKind, SyntaxNode, SyntaxSlot};
use check_service::configuration::to_analyzer_rules;
use check_service::file_handlers::DocumentFileSource;
use check_service::projects::Projects;
...
pub fn create_analyzer_options(
    ...
    let deserialized = check_deserialize::json::deserialize_from_json_str::<Configuration>(
        json.as_str(),
    ...
    paths
        ...
        .filter_map(|path| {
            let root = fs.read_file_from_path(path).ok().and_then(|content| {
                ...
                let parsed =
                    check_js_parser::parse(&content, file_source, JsParserOptions::default());
...
fn get_js_like_paths_in_dir(dir: &Utf8Path) -> Vec<CheckPath> {
    std::fs::read_dir(dir)
        .unwrap()
        .flat_map(|path| {
...
pub fn project_layout_with_node_manifest(
    ...
    if let Ok(json) = std::fs::read_to_string(options_file.clone()) {
        let deserialized = check_deserialize::json::deserialize_from_json_str::<PackageJson>(
            json.as_str(),
            JsonParserOptions::default(),
            "",
        );
...
pub fn diagnostic_to_string(name: &str, source: &str, diag: Error) -> String {
    ...
    markup_to_string(check_console::markup! {
        {PrintDiagnostic::verbose(&error)}
    })
}

fn markup_to_string(markup: check_console::Markup) -> String {
    let mut buffer = Vec::new();
    let mut write =
        check_console::fmt::Termcolor(check_diagnostics::termcolor::NoColor::new(&mut buffer));
...
unsafe extern "C" fn check_leaks() {
    if let Some(report) = check_rowan::check_live() {
        panic!("\n{report}")
    }
}
...
pub fn has_bogus_nodes_or_empty_slots<L: check_rowan::Language>(node: &SyntaxNode<L>) -> bool {
...
pub fn get_test_services<S: BuildHasher>(
    ...
    for (path, src) in files.iter() {
        ...
        let check_path = CheckPath::new(&path_buf);
        if check_path.is_manifest() {
            match check_path.file_name() {
                Some("package.json") => {
                ...
                _ => unimplemented!("Unhandled manifest: {check_path}"),
            }
        } else {
            added_paths.push(check_path);
        }
