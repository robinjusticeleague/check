...
fn generate_deserializable_enum(
    ...
    let validator = if data.with_validator {
        quote! {
            if !check_deserialize::DeserializableValidator::validate(&mut result, ctx, name, value.range()) {
                return None;
            }
        }
    } else {
    ...
    quote! {
        impl #generics check_deserialize::Deserializable for #ident #generics #trait_bounds{
            fn deserialize(
                ctx: &mut impl check_deserialize::DeserializationContext,
                value: &impl check_deserialize::DeserializableValue,
                name: &str,
            ) -> Option<Self> {
                let mut result = match check_deserialize::Text::deserialize(ctx, value, name)?.text() {
                    #(#deserialize_variants),*,
                    unknown_variant => {
                        const ALLOWED_VARIANTS: &[&str] = &[#(#allowed_variants),*];
                        ctx.report(check_deserialize::DeserializationDiagnostic::new_unknown_value(
                            unknown_variant,
                            value.range(),
                            ALLOWED_VARIANTS,
                        ));
    ...
    quote! {
        impl #generics check_deserialize::Deserializable for #ident #generics #trait_bounds {
            fn deserialize(
                ...
                let result = check_deserialize::Deserializable::deserialize(ctx, value, name).map(Self)?;
                #validator
                Some(result)
            }
        }
...
fn generate_deserializable_struct(
    ...
    let validator = if data.with_validator {
        quote! {
            #validator
            if !check_deserialize::DeserializableValidator::validate(&mut result, ctx, name, range) {
                return None;
            }
        }
    } else {
        ...
        match data.unknown_fields {
            UnknownFields::Warn | UnknownFields::Deny => {
                let with_customseverity = if data.unknown_fields == UnknownFields::Warn {
                    quote! { .with_custom_severity(check_diagnostics::Severity::Warning) }
                } else {
                    quote! {}
                };
                quote! {
    ...
    quote! {
        impl #generics check_deserialize::Deserializable for #ident #generics #trait_bounds {
            fn deserialize(
                ...
                impl #generics check_deserialize::DeserializationVisitor for Visitor #generics #trait_bounds {
                    type Output = #ident #generics;

                    const EXPECTED_TYPE: check_deserialize::DeserializableTypes = check_deserialize::DeserializableTypes::MAP;

                    fn visit_map(
                        self,
                        ctx: &mut impl check_deserialize::DeserializationContext,
                        members: impl Iterator<Item = Option<(impl check_deserialize::DeserializableValue, impl check_deserialize::DeserializableValue)>>,
                        range: check_deserialize::TextRange,
                        name: &str,
                    ) -> Option<Self::Output> {
                        use check_deserialize::{Deserializable, DeserializationDiagnostic, Text};
    ...
    quote! {
        impl #generics check_deserialize::Deserializable for #ident #generics #trait_bounds {
            fn deserialize(
                ...
                let result: #from = check_deserialize::Deserializable::deserialize(ctx, value, name)?;
    ...
    quote! {
        impl #generics check_deserialize::Deserializable for #ident #generics #trait_bounds {
            fn deserialize(
                ...
                let mut result: #try_from = check_deserialize::Deserializable::deserialize(ctx, value, name)?;
                match result.try_into() {
                    ...
                    Err(err) => {
                        ctx.report(check_deserialize::DeserializationDiagnostic::new(
                            format_args!("{}", err)
    ...
    if generics.params.is_empty() {
        ...
        let params = generics.params.iter().map(|param| match param {
            GenericParam::Type(ty) => {
                ...
                if bounds.is_empty() {
                    quote! { #ident: check_deserialize::Deserializable }
                } else {
                    quote! { #ident: #bounds + check_deserialize::Deserializable }
                }
            }
            _ => abort!(generics, "Unsupported generic parameter"),
        });
