...
fn generate_merge_enum(ident: Ident) -> TokenStream {
    quote! {
        impl check_deserialize::Merge for #ident {
...
fn generate_merge_newtype(ident: Ident) -> TokenStream {
    quote! {
        impl check_deserialize::Merge for #ident {
            fn merge_with(&mut self, other: Self) {
                check_deserialize::Merge::merge_with(&mut self.0, other.0);
            }
        }
    }
}
...
fn generate_merge_struct(ident: Ident, fields: Fields) -> TokenStream {
    ...
    quote! {
        impl check_deserialize::Merge for #ident {
            fn merge_with(&mut self, other: Self) {
                #( check_deserialize::Merge::merge_with(&mut self.#field_idents, other.#field_idents); )*
            }
        }
    }
}
