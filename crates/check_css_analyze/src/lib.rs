...
use check_css_syntax::{CssLanguage, TextRange};
use check_diagnostics::Error;
use check_suppression::{SuppressionDiagnostic, parse_suppression_comment};
use std::ops::Deref;
    ...
    B: 'a,
{
    ...
    let mut analyzer = check_analyze::Analyzer::new(
        METADATA.deref(),
        check_analyze::InspectMatcher::new(registry, inspect_matcher),
    ...
    (
        analyzer.run(check_analyze::AnalyzerContext {
            root: root.clone(),
            range: filter.range,
            services,
            options,
...
mod tests {
    ...
    use check_console::{Markup, markup};
    ...
    fn top_level_suppression_simple() {
        const SOURCE: &str = "
/**
* check-ignore lint/suspicious/noEmptyBlock: reason
*/
...
/**
* check-ignore lint/correctness/noUnknownProperty: reason2
*/
