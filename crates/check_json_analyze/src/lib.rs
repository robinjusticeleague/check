...
use check_json_syntax::{JsonFileSource, JsonLanguage, TextRange};
...
#[cfg(test)]
mod tests {
    use check_analyze::{AnalyzerOptions, Never, RuleFilter};
    use check_console::fmt::{Formatter, Termcolor};
    use check_console::{Markup, markup};
    use check_diagnostics::termcolor::NoColor;
    use check_diagnostics::{Diagnostic, DiagnosticExt, PrintDiagnostic, Severity};
    use check_json_parser::{JsonParserOptions, parse_json};
    use check_json_syntax::{JsonFileSource, TextRange};
    use std::slice;
