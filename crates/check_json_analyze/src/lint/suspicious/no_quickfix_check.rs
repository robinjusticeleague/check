...
use check_json_syntax::{AnyJsonValue, JsonMember, JsonObjectValue, T, inner_string_text};
use check_rowan::{AstNode, AstSeparatedList, BatchMutationExt, TextRange, TriviaPieceKind};
use check_rule_options::no_quickfix_check::NoQuickfixCheckOptions;

declare_lint_rule! {
    /// Disallow the use if `quickfix.check` inside editor settings file.
    ///
    /// The code action `quickfix.check` can be harmful because it instructs the editors
...
impl Rule for NoQuickfixCheck {
    type Query = Ast<JsonMember>;
    type State = TextRange;
    type Signals = Option<Self::State>;
    type Options = NoQuickfixCheckOptions;
    ...
    fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
        ...
        for default_path in DEFAULT_PATHS {
            if path.ends_with(default_path) {
                ...
                if inner_string_text(&value) == "quickfix.check" {
    ...
    fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
        Some(
            RuleDiagnostic::new(
                ...
                state,
                markup! {
                    "The use of "<Emphasis>"quickfix.check"</Emphasis>" is deprecated."
                },
            )
            .note(markup! {
                    "The code action "<Emphasis>"quickfix.check"</Emphasis>" applies the code fix of rules and actions without being aware of each other. This might cause the emission of malformed code, especially if the code fixes are applied to the same lines of code."
            }),
        )
    }
    ...
    fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsonRuleAction> {
        ...
        let has_fix_all = parent_list.iter().flatten().any(|member| {
            member
                ...
                .map(|name| {
                    name.value_token()
                        .map(|token| inner_string_text(&token) == "source.fixAll.check")
                        .unwrap_or(false)
                })
                .unwrap_or(false)
        });
        ...
        if has_fix_all {
            ...
            new_list.push(json_member(
                json_member_name(json_string_literal("source.fixAll.check")),
