...
pub const SUPPRESSION_INLINE_ACTION_CATEGORY: &str = "quickfix.suppressRule.inline.check";
pub const SUPPRESSION_TOP_LEVEL_ACTION_CATEGORY: &str = "quickfix.suppressRule.topLevel.check";
...
pub enum OtherActionCategory {
    /// Base kind for inline suppressions actions: `quickfix.suppressRule.inline.check`
    InlineSuppression,
    /// Base kind for inline suppressions actions: `quickfix.suppressRule.topLevel.check`
    ToplevelSuppression,
...
impl ActionCategory {
    ...
    /// ```
    /// use std::borrow::Cow;
    /// use check_analyze::{ActionCategory, RefactorKind, OtherActionCategory};
    ///
    ...
    ///
    /// assert!(ActionCategory::Other(OtherActionCategory::InlineSuppression).matches("quickfix.suppressRule.inline.check"));
    /// assert!(ActionCategory::Other(OtherActionCategory::ToplevelSuppression).matches("quickfix.suppressRule.topLevel.check"));
    /// ```
    ...
    pub fn to_str(&self) -> Cow<'static, str> {
        match self {
            Self::QuickFix(tag) => {
                if tag.is_empty() {
                    Cow::Borrowed("quickfix.check")
                } else {
                    Cow::Owned(format!("quickfix.check.{tag}"))
                }
            }

            Self::Refactor(RefactorKind::None) => Cow::Borrowed("refactor.check"),
            Self::Refactor(RefactorKind::Extract) => Cow::Borrowed("refactor.extract.check"),
            Self::Refactor(RefactorKind::Inline) => Cow::Borrowed("refactor.inline.check"),
            Self::Refactor(RefactorKind::Rewrite) => Cow::Borrowed("refactor.rewrite.check"),
            Self::Refactor(RefactorKind::Other(tag)) => Cow::Owned(format!("refactor.{tag}.check")),

            Self::Source(SourceActionKind::None) => Cow::Borrowed("source.check"),
            Self::Source(SourceActionKind::FixAll) => Cow::Borrowed("source.fixAll.check"),
            Self::Source(SourceActionKind::OrganizeImports) => {
                Cow::Borrowed("source.organizeImports.check")
            }
            Self::Source(SourceActionKind::Other(tag)) => Cow::Owned(format!("source.check.{tag}")),

            Self::Other(other_action) => match other_action {
                OtherActionCategory::InlineSuppression => {
                    Cow::Borrowed("quickfix.suppressRule.inline.check")
                }
                OtherActionCategory::ToplevelSuppression => {
                    Cow::Borrowed("quickfix.suppressRule.topLevel.check")
                }
                OtherActionCategory::Generic(tag) => Cow::Owned(format!("{tag}.check")),
            },
        }
    }
}
...
///
/// ```
/// use check_analyze::{RuleCategoriesBuilder, RuleCategory};
