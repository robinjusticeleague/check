...
use check_diagnostics::location::AsSpan;
use check_diagnostics::{
    Advices, Category, Diagnostic, DiagnosticTags, Location, LogCategory, MessageAndDescription,
    Visit,
};
use check_diagnostics::{Applicability, Severity};
use check_rowan::{AstNode, BatchMutation, BatchMutationExt, Language, TextRange};
use std::borrow::Cow;
...
#[cfg_attr(
    feature = "serde",
    derive(
        check_deserialize_macros::Deserializable,
        serde::Deserialize,
        serde::Serialize
    )
)]
...
///
/// ```rust,ignore
///use check_analyze::declare_rule;
///
/// declare_lint_rule! {
///     /// Documentation
...
///
/// ```rust,ignore
///use check_analyze::declare_syntax_rule;
///
/// declare_syntax_rule! {
///     /// Documentation
///     pub(crate) ExampleRule {
    ...
    ( $( #[doc = $doc:literal] )+ $vis:vis $id:ident {
        ...
        check_analyze::declare_rule!(
            ...
            $vis $id {
                ...
                name: $name,
                language: $language,
                severity: check_diagnostics::Severity::Error,
                $( $key: $value, )*
            }
        );
...
///
/// ```rust,ignore
///use check_analyze::declare_refactor_rule;
///
    ...
    ( $( #[doc = $doc:literal] )+ $vis:vis $id:ident {
        ...
        check_analyze::declare_rule!(
            ...
            $vis $id {
                ...
                name: $name,
                language: $language,
                fix_kind: check_analyze::FixKind::None,
                $( $key: $value, )*
            }
        );
        ...
        macro_rules! rule_action_category {
            () => { check_analyze::ActionCategory::Source(check_analyze::SourceActionKind::Other(Cow::Borrowed($name)))  };
        }
    };
}
...
pub trait Rule: RuleMeta + Sized {
    ...
    /// can suppress with `// check-ignore lint/correctness/noUnusedVariables`.
    ///
    ...
    /// `// check-ignore lint/correctness/noUnusedVariables(a)`.
    ///
        ...
        Self: 'static,
    {
        ...
        if matches!(
            ...
            let suppression_text = format!("check-ignore-all {rule_category}");
            let root = ctx.root();
        ...
        Self: 'static,
    {
        ...
        if matches!(
            ...
            let suppression_text = format!("check-ignore {rule_category}");
            let root = ctx.root();
