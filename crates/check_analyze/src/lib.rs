...
pub use check_diagnostics::category_concat;
...
use check_diagnostics::{Diagnostic, DiagnosticExt, category};
use check_rowan::{
    AstNode, BatchMutation, Direction, Language, SyntaxKind as _, SyntaxToken, TextRange, TextSize,
    TokenAtOffset, TriviaPieceKind,
};
use check_suppression::{Suppression, SuppressionKind};
pub use suppression_action::{ApplySuppression, SuppressionAction};

/// The analyzer is the main entry point into the `check_analyze` infrastructure.
            ...
            if reason_text == "<explanation>" {
                let signal = DiagnosticSignal::new(|| {
                    AnalyzerSuppressionDiagnostic::new(
                        ...
                        reason_range,
                        "A suppression shouldn't have an <explanation> placeholder. Example of suppression: // check-ignore lint: false positive",
                    )
                });
                (self.emit_signal)(&signal)?;
            }
...
/// # Examples
///
/// - `// check-ignore format` -> `vec![]`
/// - `// check-ignore lint` -> `vec![Everything]`
/// - `// check-ignore lint/complexity/useWhile` -> `vec![Rule("complexity/useWhile")]`
/// - `// check-ignore lint/complexity/useWhile(foo)` -> `vec![RuleWithValue("complexity/useWhile", "foo")]`
/// - `// check-ignore lint/complexity/useWhile lint/nursery/noUnreachable` -> `vec![Rule("complexity/useWhile"), Rule("nursery/noUnreachable")]`
/// - `/** check-ignore lint/complexity/useWhile */` if the comment is top-level -> `vec![TopLevel("complexity/useWhile")]`
...
pub struct AnalyzerSuppression<'a> {
    ...
    /// The range where the `check-ignore` comment is placed inside the whole text
    pub(crate) ignore_range: Option<TextRange>,

    /// The kind of `check-ignore` comment used for this suppression
...
pub enum AnalyzerSuppressionKind<'a> {
    /// A suppression disabling all lints eg. `// check-ignore lint`
    Everything(RuleCategory),
    /// A suppression disabling a specific rule eg. `// check-ignore lint/complexity/useWhile`
    Rule(&'a str),
    /// A suppression to be evaluated by a specific rule eg. `// check-ignore lint/correctness/useExhaustiveDependencies(foo)`
    RuleInstance(&'a str, &'a str),
    /// A suppression disabling a plugin eg. `// lint/check-ignore plugin/my-plugin`
    Plugin(Option<&'a str>),
}
...
pub struct SuppressionCommentEmitterPayload<'a, L: Language> {
    ...
    /// A string equals to "check-ignore: lint(<RULE_GROUP>/<RULE_NAME>)"
...
impl check_console::fmt::Display for RuleFilter<'_> {
