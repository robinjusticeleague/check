        ...
        if suppression.is_top_level() && !is_leading_in_file {
            ...
            if let Some(ignore_range) = suppression.ignore_range {
                diagnostic = diagnostic.note(
                        markup! {"Rename this to "<Emphasis>"check-ignore"</Emphasis>" or move it to the top of the file"}
                            .to_owned(),
                        ignore_range,
                    );
            }
...
pub(crate) struct LineSuppression {
    ...
    ///
    /// For example, this is `foo` in `// check-ignore lint/correctness/xxx(foo): ...`
...
pub(crate) struct RangeSuppression {
    ...
    /// The range of the `check-ignore-start` suppressions
        ...
        if suppression.is_range_start() {
            ...
            if self.suppressions.is_empty() {
                ...
                return Err(AnalyzerSuppressionDiagnostic::new(
                    category!("suppressions/incorrect"),
                    text_range,
                    markup!{"Found a "<Emphasis>"check-range-end"</Emphasis>" suppression without a "<Emphasis>"check-range-start"</Emphasis>" suppression. This is invalid"}
                ).hint(markup!{
            ...
            if let Some(existing_suppression) = range_suppression {
                ...
                let message = markup! {
                    "Found a "<Emphasis>"check-ignore-end"</Emphasis>" suppression without a "<Emphasis>"check-ignore-start"</Emphasis>" suppression. This is invalid"
                };
        ...
        for suppression in self.suppressions.iter() {
            if !suppression.is_ended {
                let diagnostic = AnalyzerSuppressionDiagnostic::new(
                    ...
                    "Range suppressions must have a matching check-ignore-end",
                );
