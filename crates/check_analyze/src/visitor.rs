...
use check_rowan::{AstNode, Language, SyntaxNode, TextRange, WalkEvent};
...
macro_rules! merge_node_visitors {
    ( $vis:vis $name:ident { $( $id:ident: $visitor:ty, )+ } ) => {
        ...
        impl $crate::Visitor for $name {
            type Language = <( $( <$visitor as $crate::NodeVisitor<$name>>::Node, )* ) as ::check_rowan::macros::UnionLanguage>::Language;
            ...
            fn visit(
                ...
                match event {
                    ::check_rowan::WalkEvent::Enter(node) => {
                        let kind = node.kind();

                        $(
                            if <<$visitor as $crate::NodeVisitor<$name>>::Node as ::check_rowan::AstNode>::can_cast(kind) {
                                let node = <<$visitor as $crate::NodeVisitor<$name>>::Node as ::check_rowan::AstNode>::unwrap_cast(node.clone());
                    ...
                    ::check_rowan::WalkEvent::Leave(node) => {
                        let kind = node.kind();
