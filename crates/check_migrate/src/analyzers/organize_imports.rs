...
use check_json_syntax::{AnyJsonValue, JsonMember, JsonRoot, T};
use check_rowan::{AstNode, BatchMutationExt, TriviaPieceKind};
...
impl Rule for OrganizeImports {
    ...
    fn diagnostic(_ctx: &RuleContext<Self>, member: &Self::State) -> Option<RuleDiagnostic> {
        Some(RuleDiagnostic::new(
            ...
            "The import sorting was the first assist action, however Check analyzer infrastructure wasn't mature enough, so it was exposed as a standalone tool. The infrastructure is now ready to welcome it as an assist action."
        }))
    }
