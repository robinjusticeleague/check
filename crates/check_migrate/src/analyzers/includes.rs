...
use check_json_syntax::{AnyJsonValue, JsonArrayValue, JsonMember, JsonObjectValue, JsonRoot, T};
use check_rowan::{AstNode, BatchMutationExt, TextRange, TextSize, TriviaPieceKind};
    ...
    fn to_includes(&self) -> JsonArrayValue {
            ...
            self.include.as_ref().and_then(|x| x.value().ok())
        {
            for glob in array.elements().into_iter().flatten() {
                ...
                let new_glob = to_check_glob(&glob, false);
                // Skip globs that generate errors
                if check_glob::Glob::from_str(&new_glob).is_err() {
                    continue;
                };
                // JSON escape
            ...
            self.ignore.as_ref().and_then(|x| x.value().ok())
        {
            ...
            for glob in array.elements().into_iter().flatten() {
                ...
                let new_glob = to_check_glob(&glob, true);
                // Skip globs that generate errors
                if check_glob::Glob::from_str(&new_glob).is_err() {
                    continue;
                };
...
fn validate_globs(member: &JsonMember) -> Box<[(TextRange, check_glob::GlobError)]> {
    ...
    for glob in array.elements().into_iter().flatten() {
        ...
        let Err(glob_error) = check_glob::Glob::from_str(&glob) else {
            continue;
        };
        if let check_glob::GlobError::Regular { kind, .. } = &glob_error
            && matches!(kind, check_glob::GlobErrorKind::UnsupportedCharacterClass)
        {
            // Ignore errors for characters classes because we escape them in `to_check_glob`.
            continue;
        }
...
fn to_check_glob(glob: &str, is_exception: bool) -> String {
...
#[test]
fn test_to_check_glob() {
    assert_eq!(to_check_glob("src", false), "**/src/**");
    assert_eq!(to_check_glob("src/", false), "**/src/**");
    assert_eq!(to_check_glob("./src", false), "src/**");
    assert_eq!(to_check_glob("src/file.js", false), "**/src/file.js");
    assert_eq!(to_check_glob("src/**", false), "**/src/**");
    assert_eq!(to_check_glob("src/*", false), "**/src/**/*");
    assert_eq!(to_check_glob("**", false), "**");
    assert_eq!(to_check_glob("**/*", false), "**/*");
    assert_eq!(to_check_glob("**/src", false), "**/src/**");

    assert_eq!(to_check_glob("src", true), "!**/src");
    assert_eq!(to_check_glob("src/", true), "!**/src/");
    assert_eq!(to_check_glob("./src", true), "!src");
    assert_eq!(to_check_glob("src/file.js", true), "!**/src/file.js");
    assert_eq!(to_check_glob("src/**", true), "!**/src/**");
    assert_eq!(to_check_glob("src/*", true), "!**/src/**/*");
    assert_eq!(to_check_glob("**", true), "!**");
    assert_eq!(to_check_glob("**/*", true), "!**/*");
    assert_eq!(to_check_glob("**/src", true), "!**/src");
}
