use check_analyze::{AnalysisFilter, AnalyzerAction, ControlFlow, Never, RuleFilter};
use check_diagnostics::advice::CodeSuggestionAdvice;
use check_diagnostics::{DiagnosticExt, Severity};
...
pub(crate) fn analyze_and_snap(
    ...
    let (_, errors) =
        check_migrate::migrate_configuration(&root, filter, input_file, true, |event| {
