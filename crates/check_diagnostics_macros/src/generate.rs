...
fn generate_struct_diagnostic(input: DeriveStructInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params check_diagnostics::Diagnostic for #ident #generics {
            #category
            #severity
            #description
            #message
...
fn generate_category(input: &DeriveStructInput) -> TokenStream {
    let category = match &input.category {
        Some(StaticOrDynamic::Static(value)) => quote! {
            check_diagnostics::category!(#value)
        },
...
fn generate_severity(input: &DeriveStructInput) -> TokenStream {
    let severity = match &input.severity {
        Some(StaticOrDynamic::Static(value)) => quote! {
            check_diagnostics::Severity::#value
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
        },
...
fn generate_description(input: &DeriveStructInput) -> TokenStream {
    let description = match &input.description {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            ...
            let write = check_diagnostics::termcolor::NoColor::new(&mut buffer);
            let mut write = check_diagnostics::console::fmt::Termcolor(write);
            let mut write = check_diagnostics::console::fmt::Formatter::new(&mut write);

            use check_diagnostics::console as check_console;
            write.write_markup(&check_diagnostics::console::markup!{ #markup })
...
fn generate_message(input: &DeriveStructInput) -> TokenStream {
    let message = match &input.message {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            use check_diagnostics::console as check_console;
            fmt.write_markup(check_diagnostics::console::markup!{ #markup })
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            check_diagnostics::console::fmt::Display::fmt(&self.#value, fmt)
        },
        None => return quote!(),
    };

    quote! {
        fn message(&self, fmt: &mut check_diagnostics::console::fmt::Formatter<'_>) -> ::std::io::Result<()> {
            #message
        }
    }
}
...
fn generate_advices(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn advices(&self, visitor: &mut dyn check_diagnostics::Visit) -> ::std::io::Result<()> {
            #( check_diagnostics::Advices::record(#advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_verbose_advices(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn verbose_advices(&self, visitor: &mut dyn check_diagnostics::Visit) -> ::std::io::Result<()> {
            #( check_diagnostics::Advices::record(&self.#verbose_advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_location(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn location(&self) -> check_diagnostics::Location<'_> {
            check_diagnostics::Location::builder()
                #( .#method(&self.#field) )*
                .build()
        }
    }
...
fn generate_tags(input: &DeriveStructInput) -> TokenStream {
    let tags = match &input.tags {
        Some(StaticOrDynamic::Static(value)) => {
            ...
            quote! {
                #( check_diagnostics::DiagnosticTags::#values )|*
            }
        }
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
    ...
    quote! {
        fn tags(&self) -> check_diagnostics::DiagnosticTags {
            #tags
        }
    }
}
...
fn generate_source(input: &DeriveStructInput) -> TokenStream {
    match &input.source {
        Some(value) => quote! {
            fn source(&self) -> Option<&dyn check_diagnostics::Diagnostic> {
                self.#value.as_deref()
            }
        },
        None => quote!(),
...
fn generate_enum_diagnostic(input: DeriveEnumInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params check_diagnostics::Diagnostic for #ident #generics {
            fn category(&self) -> Option<&'static check_diagnostics::Category> {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::category(error),)*
                }
            }

            fn description(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::description(error, fmt),)*
                }
            }

            fn message(&self, fmt: &mut check_console::fmt::Formatter<'_>) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::message(error, fmt),)*
                }
            }

            fn severity(&self) -> check_diagnostics::Severity {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::severity(error),)*
                }
            }

            fn tags(&self) -> check_diagnostics::DiagnosticTags {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::tags(error),)*
                }
            }

            fn location(&self) -> check_diagnostics::Location<'_> {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::location(error),)*
                }
            }

            fn source(&self) -> Option<&dyn check_diagnostics::Diagnostic> {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::source(error),)*
                }
            }

            fn advices(&self, visitor: &mut dyn check_diagnostics::Visit) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::advices(error, visitor),)*
                }
            }

            fn verbose_advices(&self, visitor: &mut dyn check_diagnostics::Visit) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => check_diagnostics::Diagnostic::verbose_advices(error, visitor),)*
                }
            }
        }
    }
