...
use check_diagnostics::{Category, Diagnostic, DiagnosticTags, Location, Severity, category};
    ...
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidDocument(error) => std::write!(
                fmt,
                "Invalid document: {error}\n\n This is an internal Check error. Please report if necessary."
            ),
            Self::PoorLayout => {
                std::write!(
                    fmt,
                    "Poor layout: The formatter wasn't able to pick a good layout for your document. This is an internal Check error. Please report if necessary."
                )
            }
        }
    }
...
impl Diagnostic for FormatError {
    ...
    fn message(
        &self,
        fmt: &mut check_diagnostics::console::fmt::Formatter<'_>,
    ) -> std::io::Result<()> {
        match self {
