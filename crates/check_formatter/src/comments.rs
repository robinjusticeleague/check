...
use check_rowan::syntax::SyntaxElementKey;
use check_rowan::{Language, SyntaxNode, SyntaxToken, SyntaxTriviaPieceComments};
...
impl CommentKind {
    ...
    ///
    /// ```rust
    /// use check_formatter::comments::CommentKind;
    ///
...
pub trait CommentStyle: Default {
    ...
    /// Returns `true` if a comment with the given `text` is a `check-ignore format:` suppression comment.
...
/// It should probably be moved to `check_js_formatter` when possible, but is
