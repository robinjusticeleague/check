...
use check_rowan::TextLen;
use check_rowan::{Language, SyntaxNode, TextSize};
...
/// formatting the node failed because of a syntax error, or formatting is suppressed with a `check-ignore format:` comment.
///
/// ```text
/// // Source           // Transformed
///  (a+b) + (c + d)   a + b + c + d;
...
impl TransformSourceMapBuilder {
    ...
    /// failed to format because of a syntax error or because it's formatting is suppressed with a `check-ignore format:` comment.
    ///
...
mod tests {
    ...
    use check_rowan::raw_language::{RawLanguageKind, RawSyntaxTreeBuilder};
