...
use check_deserialize_macros::Merge;
use check_rowan::{
...
impl check_console::fmt::Display for IndentWidth {
...
impl check_console::fmt::Display for LineWidth {
    fn fmt(&self, fmt: &mut check_console::fmt::Formatter) -> std::io::Result<()> {
...
impl check_console::fmt::Display for BracketSpacing {
    fn fmt(&self, fmt: &mut check_console::fmt::Formatter) -> std::io::Result<()> {
...
/// Formatting trait for types that can create a formatted representation. The `check_formatter` equivalent
/// to [std::fmt::Display].
///
/// ## Example
...
///
/// ```
/// use check_formatter::{format, write, IndentStyle, LineWidth};
...
///
/// For example, the `check_js_formatter` crate isn't able to implement [Format] on `JsIfStatement`
/// because both the [Format] trait and `JsIfStatement` are external types (Rust's orphan rule).
///
/// That's why the `check_js_formatter` crate must define a new-type that implements the formatting
/// of `JsIfStatement`.
...
/// ## Examples
///
/// This can be useful if you want to format a `SyntaxNode` inside check_formatter.. `SyntaxNode` doesn't implement [Format]
...
/// ```ignore
/// use check_formatter::prelude::*;
/// use check_formatter::{format, Formatted, FormatWithRule};
/// use check_rowan::{Language, SyntaxNode};
...
/// ```
/// use check_formatter::prelude::*;
/// use check_formatter::{VecBuffer, format_args, FormatState, write, Formatted};
///
/// # fn main() -> FormatResult<()> {
...
/// ```
/// use check_formatter::prelude::*;
/// use check_formatter::{format, format_args};
///
/// # fn main() -> FormatResult<()> {
