...
pub trait Buffer {
    ...
    ///
    /// ```
    /// use check_formatter::{Buffer, FormatElement, FormatState, SimpleFormatContext, VecBuffer};
    ///
    ...
    /// use check_formatter::{Buffer, FormatState, SimpleFormatContext, VecBuffer, format_args};
    ///
    /// let mut state = FormatState::new(SimpleFormatContext::default());
    /// let mut buffer = VecBuffer::new(&mut state);
    ///
...
///
/// ```
/// use check_formatter::{FormatState, Formatted, PreambleBuffer, SimpleFormatContext, VecBuffer, write};
/// use check_formatter::prelude::*;
///
/// struct Preamble;
///
...
/// ```
/// use check_formatter::prelude::*;
/// use check_formatter::{format, write};
///
/// # fn main() -> FormatResult<()> {
/// use check_formatter::{RemoveSoftLinesBuffer, SimpleFormatContext, VecBuffer};
/// use check_formatter::prelude::format_with;
                    ...
                    Some((mut cleaned, rest)) => {
                        ...
                        while let Some(element) = element_stack.pop() {
                            match element {
                                ...
                                _ if condition_content_stack.iter().last().is_some_and(
                                    |condition| condition.mode == PrintMode::Expanded,
                                ) => {}
...
pub trait BufferExtensions: Buffer + Sized {
    ...
    /// use std::ops::Deref;
    /// use check_formatter::prelude::*;
    /// use check_formatter::{write, format, SimpleFormatContext};
    ///
    /// # fn main() -> FormatResult<()> {
