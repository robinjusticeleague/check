...
The crate `check_formatter` offers a generic infrastructure to implement a formatting logic for different languages.

The formatting infrastructure of Check is implemented using traits on syntax nodes. This means that *each node* knows how to format itself.

## Getting started

Let's start creating some plumbing, for our new language called `Html`.

Create a new crate using the command `just new-crate check_html_formatter`, where `html` is the language you want to format.

The infrastructure of the formatter requires some preliminary code that can't be vendored from `check_formatter`, due to some constraints of our infrastructure.
...
pub(crate) trait AsFormat<Context> {
    type Format<'a>: check_formatter::Format<Context>
    where
        Self: 'a;
...
impl<T, C> AsFormat<C> for check_rowan::SyntaxResult<T>
where
    T: AsFormat<C>,
{
    type Format<'a> = check_rowan::SyntaxResult<T::Format<'a>> where Self: 'a;
...
pub(crate) trait IntoFormat<Context> {
    type Format: check_formatter::Format<Context>;

    fn into_format(self) -> Self::Format;
}

impl<T, Context> IntoFormat<Context> for check_rowan::SyntaxResult<T>
where
    T: IntoFormat<Context>,
{
    type Format = check_rowan::SyntaxResult<T::Format>;
...
```rust
use check_formatter::comments::Comments;
...
use check_html_syntax::{map_syntax_node, HtmlSyntaxNode};
...
This is small type that you need to instruct the formatter infra about a certain language. This type needs to implement the trait `check_formatter::FormatLanguage`
...
<summary>Wire the specific formatting with the `check_formatter` formatting infra.</summary>
    ...
    N: AstNode<Language = HtmlLanguage>,
{
    ...
    /// Formats the [leading comments](check_formatter::comments#leading-comments) of the node.
    ///
    ...
    /// Formats the [dangling comments](check_formatter::comments#dangling-comments) of the node.
    ///
    ...
    /// Formats the [trailing comments](check_formatter::comments#trailing-comments) of the node.
    ///
    /// You may want to override this method if you want to manually handle the formatting of comments
...
pub fn format_node(
    ...
    check_formatter::format_node(root, HtmlFormatLanguage::new(options))
}
```
...
The initial implementation for the formatting will use the `format_verbatim_node` formatting, which means that the code will be formatted **as is**. From here, you'll have to remove `format_verbatim_node` and use the `check_formatter` utilities to generate the correct IR.

## Testing

### Plumbing

Inside the `check_html_formatter` crate, create a folder called `tests`. Inside this folder you have to have a `specs` folder and two files called `spec_test.rs` and `spec_tests.rs` (the names aren't very important though). Create a `language.rs` file too.

Updated the `Cargo.toml` file to import some testing utility:

```toml
[dev-dependencies]
check_formatter_test = { path = "../check_formatter_test" }
check_html_factory     = { path = "../check_html_factory" }
check_html_parser      = { path = "../check_html_parser" }
check_parser         = { path = "../check_parser" }
check_service        = { path = "../check_service" }
...
```rust
use check_formatter_test::spec::{SpecSnapshot, SpecTestFile};
use std::path::Path;
...
The `options.json` file is a `check.json` file, so you can use the same options as you were and end-user.
