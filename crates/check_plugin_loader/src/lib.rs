...
use check_analyze::{AnalyzerPlugin, AnalyzerPluginVec};
use check_console::markup;
use check_deserialize::json::deserialize_from_json_str;
use check_fs::normalize_path;
...
impl CheckPlugin {
    ...
    pub fn load(
        ...
        let manifest_path = plugin_path.join("check-manifest.jsonc");
...
mod test {
    ...
    fn load_plugin() {
        ...
        fs.insert(
            "/my-plugin/check-manifest.jsonc".into(),
            r#"{
    "version": 1,
    "rules": ["rules/1.grit"]
}"#,
        ...
        let (plugin, _) =
            CheckPlugin::load(fs, "./my-plugin", Utf8Path::new("/")).expect("Couldn't load plugin");
    ...
    fn load_plugin_without_manifest() {
        ...
        let fs = Arc::new(fs) as Arc<dyn FsWithResolverProxy>;
        let error = CheckPlugin::load(fs, "./my-plugin", Utf8Path::new("/"))
    ...
    fn load_single_rule_plugin() {
        ...
        let fs = Arc::new(fs) as Arc<dyn FsWithResolverProxy>;
        let (plugin, _) = CheckPlugin::load(fs, "./my-plugin.grit", Utf8Path::new("/"))
    ...
    fn load_single_rule_js_plugin() {
        ...
        let fs = Arc::new(fs) as Arc<dyn FsWithResolverProxy>;
        let (plugin, _) = CheckPlugin::load(fs, "./my-plugin.js", Utf8Path::new("/"))
            .expect("Couldn't load plugin");
