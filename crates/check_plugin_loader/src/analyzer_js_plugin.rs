...
use check_analyze::{AnalyzerPlugin, PluginTargetLanguage, RuleDiagnostic};
use check_console::markup;
use check_diagnostics::category;
use check_js_runtime::JsExecContext;
use check_js_syntax::AnyJsRoot;
use check_resolver::FsWithResolverProxy;
use check_rowan::{AnySyntaxNode, AstNode, RawSyntaxKind, SyntaxKind};
...
mod tests {
    ...
    use check_js_parser::{JsFileSource, JsParserOptions};
    ...
    fn evaluate_in_worker_threads() {
        ...
        fs.insert(
            "/plugin.js".into(),
            r#"import { registerDiagnostic } from "@checkjs/plugin-api";
        ...
        let worker1 = {
            ...
            std::thread::spawn(move || {
                let parse = check_js_parser::parse(
                    "let foo;",
