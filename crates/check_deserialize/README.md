...
`check_deserialize` consists of data structures that know how to deserialize themselves
...
One of the main difference is the fault-tolerant behavior of `check_deserialize`.
_Serde_ uses a fast-fail strategy, while `check_deserialize` deserialize as much as possible
and report several diagnostics (errors, warning, deprecation messages, ...).
Also, `check_deserialize` is intended to deserialize textual data formats.

`check_deserialize` assumes that every supported data formats supports the following types:
...
`check_deserialize` implements `Deserializable` for common Rust data structure.
...
use check_deserialize::Deserialized;
...
For more complex types, such as structs and enums, `check_deserialize_macros` offers a derive macro
...
`check_deserialize` requires that every data format supports arrays and maps.
A map and an array can be deserialized into several types in Rust.

`check_deserialize` is able to deserialize an array into a `Vec`, a Box<[_]>, a `HashSet`, a `BTreeSet`, or a `IndexSet`, `BtreeSet`.
...
`check_deserialize` is able to deserialize a map into a `HashMap`, a `BTreeMap`, or a `IndexMap`.
...
`check_deserialize::Deserializable` yourself.
...
```rust
use std::str::FromStr;
use check_deserialize::{Deserializable, DeserializableValue, DeserializationDiagnostic, TextNumber};
...
```rust
use check_deserialize::{DeserializationDiagnostic, Deserializable, DeserializationContext, DeserializableValue, DeserializationVisitor, Text, DeserializableTypes};
...
impl Deserializable for Union {
    fn deserialize(
        ...
        if value.visitable_type()? == DeserializableType::Bool {
            check_deserialize::Deserializable::deserialize(ctx, value, name)
                .map(Self::Bool)
        } else {
            check_deserialize::Deserializable::deserialize(ctx, value, name)
                .map(Self::Str)
...
because `check_deserialize` implements `Deserializable` for the `String` type.
...
```rust
use check_deserialize::{Deserializable, DeserializationContext, DeserializableValue, DeserializationDiagnostic};
...
```rust
use check_deserialize::{Deserializable, DeserializationContext, DeserializableValue, DeserializationDiagnostic, Text};
...
Note that if you use _Serde_ in tandem with `check_deserialize`, you have to disambiguate the call to `deserialize`.
