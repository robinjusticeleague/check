...
use check_rowan::{AstNode, SyntaxNodeOptionExt, TextRange};
...
impl SemanticEventExtractor {
    pub fn enter(&mut self, node: &check_css_syntax::CssSyntaxNode) {
        match node.kind() {
    ...
    fn process_at_property(&mut self, node: CssPropertyAtRule) {
        ...
        for declaration in decls.declarations().into_iter().filter_map(|d| {
            ...
            if let Ok(check_css_syntax::AnyCssProperty::CssGenericProperty(prop)) =
    ...
    pub fn leave(&mut self, node: &check_css_syntax::CssSyntaxNode) {
        if matches!(
            node.kind(),
