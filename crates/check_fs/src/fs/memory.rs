...
use crate::{CheckPath, FileSystem, TraversalContext, TraversalScope};
...
impl<'scope> TraversalScope<'scope> for MemoryTraversalScope<'scope> {
        ...
        // those that are prefixed with the provided `base` path
        {
            ...
            for path in files.keys() {
                ...
                if should_process_file {
                    ...
                    if !ctx.can_handle(&check_path) {
                        continue;
                    }
                    ctx.store_path(check_path);
                }
            }
        }
    ...
    fn handle(&self, context: &'scope dyn TraversalContext, path: Utf8PathBuf) {
        context.handle_path(CheckPath::new(path));
    }
}
...
mod tests {
    ...
    use crate::{CheckPath, FileSystem, MemoryFileSystem, PathInterner, TraversalContext};
    ...
    fn traversal() {
        ...
        struct TestContext {
            ...
            visited: Mutex<BTreeSet<CheckPath>>,
        }
        ...
        impl TraversalContext for TestContext {
            ...
            fn can_handle(&self, _: &CheckPath) -> bool {
                true
            }
        ...
        assert!(visited.contains(&CheckPath::new("dir1/file1")));
        assert!(visited.contains(&CheckPath::new("dir1/file2")));
        ...
        assert!(visited.contains(&CheckPath::new("dir2/file2")));
    }
}
