...
use check_diagnostics::{DiagnosticExt, Error, IoError, Severity};
...
use crate::{
    CheckPath, FileSystem, MemoryFileSystem,
...
impl<'scope> TraversalScope<'scope> for OsTraversalScope<'scope> {
    fn evaluate(&self, ctx: &'scope dyn TraversalContext, path: Utf8PathBuf) {
        ...
        //
        // FIXME: This code should be moved to the `traverse_inputs` function in `check_cli/src/traverse.rs`.
        ...
        // See https://github.com/checkjs/check/pull/5017
...
fn handle_any_file<'scope>(
    ...
    if file_type.is_symlink() {
        if !ctx.can_handle(&CheckPath::new(path.clone())) {
            return;
        }
    ...
    if file_type.is_dir() {
        ...
        if ctx.should_store_dirs() {
            ctx.store_path(CheckPath::new(path));
        }
        return;
    }
