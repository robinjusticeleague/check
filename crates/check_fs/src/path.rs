//! This module is responsible to manage paths inside Check.
...
pub enum FileKinds {
    /// A configuration file has the highest priority. It's usually `check.json` and `check.jsonc`
    ///
...
/// This is an internal representation of a path inside the Check daemon.
...
pub struct CheckPath {
    ...
    path: Utf8PathBuf,
    /// Determines the kind of the file inside Check. Some files are considered as configuration files, others as manifest files, and others as files to handle
    kind: FileKinds,
...
impl CheckPath {
    ...
    /// Creates a new [CheckPath], marked as fixed
    ...
    ///
    /// ## Error
    /// If Check doesn't have permissions to read the file
    ...
    /// The priority of the file.
    /// - `check.json` and `check.jsonc` have the highest priority
    ...
    fn priority(file_name: &str) -> FileKinds {
        if file_name == ConfigName::check_json() || file_name == ConfigName::check_jsonc() {
            FileKinds::Config
        } else if matches!(
            file_name,
    ...
    /// enabled) because Check's own functionality relies on them.
    #[inline(always)]
...
impl From<Utf8PathBuf> for CheckPath {
    fn from(value: Utf8PathBuf) -> Self {
        Self::new(value)
    }
}

impl From<&Utf8Path> for CheckPath {
    fn from(value: &Utf8Path) -> Self {
        Self::new(value)
    }
}

impl TryFrom<PathBuf> for CheckPath {
    type Error = PathBuf;
...
impl TryFrom<Cow<'_, Path>> for CheckPath {
    type Error = PathBuf;
...
#[cfg(feature = "serde")]
impl serde::Serialize for CheckPath {
...
impl std::fmt::Display for CheckPath {
...
#[cfg(feature = "serde")]
impl<'de> serde::Deserialize<'de> for CheckPath {
...
#[cfg(feature = "schema")]
impl schemars::JsonSchema for CheckPath {
...
impl From<CheckPath> for Utf8PathBuf {
    fn from(path: CheckPath) -> Self {
        path.path
    }
}

impl PartialOrd for CheckPath {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for CheckPath {
...
mod test {
    ...
    fn test_check_paths() {
        ...
        let check_path = CheckPath::new(path);
        assert_eq!(check_path.file_name(), Some("package.json"));
        assert_eq!(CheckPath::priority("package.json"), FileKinds::Manifest);
        assert_eq!(CheckPath::priority("check.json"), FileKinds::Config);
        assert_eq!(CheckPath::priority("check.jsonc"), FileKinds::Config);
        assert_eq!(CheckPath::priority(".gitignore"), FileKinds::Ignore);
        assert_eq!(CheckPath::priority(".ignore"), FileKinds::Ignore);
    }

    #[test]
    fn test_check_file_names_order() {
        use super::CheckPath;
        use camino::Utf8PathBuf;

        let path1 = CheckPath::new(Utf8PathBuf::from("src/package.json"));
        let path2 = CheckPath::new(Utf8PathBuf::from("src/check.json"));
        let path3 = CheckPath::new(Utf8PathBuf::from("src/check.jsonc"));
        let path4 = CheckPath::new(Utf8PathBuf::from("src/tsconfig.json"));
        let path5 = CheckPath::new(Utf8PathBuf::from("src/README.md"));
        let path6 = CheckPath::new(Utf8PathBuf::from("src/frontend/check.jsonc"));

        let mut paths = [path1, path2, path3, path4, path5, path6];
        paths.sort();
        let mut iter = paths.iter();
        assert_eq!(iter.next().unwrap().file_name(), Some("check.json"));
        assert_eq!(iter.next().unwrap().file_name(), Some("check.jsonc"));
        assert_eq!(iter.next().unwrap().file_name(), Some("check.jsonc"));
    ...
    fn test_check_paths_order() {
        ...
        let path5 = CheckPath::new(Utf8PathBuf::from("src/README.md"));
        let path6 = CheckPath::new(Utf8PathBuf::from("src/frontend/check.jsonc"));
        let path7 = CheckPath::new(Utf8PathBuf::from("src/frontend/package.json"));

        let mut paths = vec![path1, path2, path3, path4, path5, path6, path7];
        paths.sort();
        let mut iter = paths.iter();
        assert_eq!(iter.next().unwrap().to_string(), "src/check.json");
        assert_eq!(iter.next().unwrap().to_string(), "src/check.jsonc");
        assert_eq!(iter.next().unwrap().to_string(), "src/frontend/check.jsonc");
        assert_eq!(
