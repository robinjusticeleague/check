use crate::{CheckPath, PathInterner};
use check_diagnostics::{Advices, Diagnostic, IoError, LogCategory, Visit, console};
use check_diagnostics::{Error, Severity};
...
impl ConfigName {
    const CHECK_JSON: [&'static str; 2] = ["check.json", "check.jsonc"];

    pub const fn check_json() -> &'static str {
        Self::CHECK_JSON[0]
    }

    pub const fn check_jsonc() -> &'static str {
        Self::CHECK_JSON[1]
    }
            ...
            for file_name in search_files {
                ...
                if let Ok(content) = self.read_file_from_path(&file_path) {
                    ...
                    if is_searching_in_parent_dir {
                        info!(
                            "Check auto discovered the file at the following path that isn't in the working directory:\n{:?}",
                            current_search_dir
                        );
                    }
        ...
        match self.open_with_options(file_path, OpenOptions::default().read(true)) {
            Ok(mut file) => {
                ...
                match file.read_to_string(&mut content) {
                    ...
                    Err(err) => {
                        error!(
                            "Check couldn't read the file {:?}, reason:\n{:?}",
                            file_path, err
                        );
            ...
            Err(err) => {
                error!(
                    "Check couldn't open the file {:?}, reason:\n{:?}",
                    file_path, err
                );
                Err(FileSystemDiagnostic {
                    path: file_path.to_string(),
...
pub trait TraversalContext: Sync {
    ...
    fn can_handle(&self, path: &CheckPath) -> bool;
    ...
    fn record(&self, visitor: &mut dyn Visit) -> io::Result<()> {
        match self {
			Self::CantReadFile => visitor.record_log(
		        LogCategory::Error,
			    &"Check can't read the following file, maybe for permissions reasons or it doesn't exist."
			),
            Self::UnknownFileType => visitor.record_log(
                LogCategory::Info,
                &"Check encountered a file system entry that's neither a file, directory or symbolic link",
            ),
            Self::DereferencedSymlink => visitor.record_log(
                LogCategory::Info,
                &"Check encountered a file system entry that is a broken symbolic link.",
            ),
            Self::DeeplyNestedSymlinkExpansion => visitor.record_log(
                LogCategory::Error,
                &"Check encountered a file system entry with too many nested symbolic links, possibly forming an infinite cycle.",
            ),
            Self::NonUtf8Path => visitor.record_log(
                LogCategory::Error,
                &"Check encountered a path with invalid UTF-8 characters."
            )
        }
    }
}
