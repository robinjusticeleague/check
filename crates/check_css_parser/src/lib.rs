...
pub(crate) type CssOffsetLosslessTreeSink<'source> =
    check_parser::tree_sink::OffsetLosslessTreeSink<'source, CssLanguage, CssSyntaxFactory>;
...
pub fn parse_css_with_cache(
    ...
    check_parser::event::process(&mut tree_sink, events, diagnostics);
...
impl CssParse {
    ...
    ///
    /// ```
    /// # use check_css_parser::parse_css;
    /// # use check_css_syntax::CssSyntaxKind;
    /// # use check_rowan::{AstNode, AstNodeList, SyntaxError};
    ///
...
impl CssOffsetParse {
    ...
    pub fn syntax(&self) -> &check_rowan::SyntaxNodeWithOffset<CssLanguage> {
        &self.root
    }
...
/// # Examples
/// ```
/// use check_css_parser::{CssParserOptions, parse_css_with_offset};
