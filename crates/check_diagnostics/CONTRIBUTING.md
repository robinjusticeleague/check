...
The `check_diagnostics` crate implements "Diagnostics", the generic concept of
how the various errors and issues that can be raised in the Check codebase,
and displayed to the final user. This guide is aimed at
contributors to the Check codebase to help writing diagnostics, both at the
technical level, and as general best practices to make diagnostics easier to
understand for the final user.

## What is a Diagnostic

Diagnostics are at the core of the experience of the Check toolchain for the
users, and providing high quality diagnostics is crucial to making the usage of
Check as frictionless as possible even when errors happen. What follows is a
list of best practice for writing good diagnostics:

- The Diagnostic should follow the [Technical Principles](https://checkjs.dev/#technical)
of the Check project
...
`check check --apply` to automatically apply the fix).
...
internal error in Check and was not directly caused by the user, if it is being
...
`check_diagnostics` crate exposes a `derive` procedural macro to make
...
types in the `check_diagnostics::v2` module (`CodeFrameAdvice`, `CommandAdvice`,
...
add it to `crates/check_diagnostics_categories/src/categories.rs`
