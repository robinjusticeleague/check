use check_console::fmt::MarkupElements;
use check_console::{
...
/// implementing [check_console::fmt::Write].
...
{
    if diagnostic.severity() == Severity::Fatal {
        visitor.record_log(LogCategory::Warn, &"Check exited as this error could not be handled and resulted in a fatal error. Please report it if necessary.")?;
    }

    if diagnostic.tags().contains(DiagnosticTags::INTERNAL) {
        visitor.record_log(LogCategory::Warn, &"This diagnostic was derived from an internal Check error. Potential bug, please report it if necessary.")?;
    }
...
fn is_terminal_program(name: &str) -> bool {
    if cfg!(debug_assertions) {
        ...
        // https://github.com/checkjs/check/issues/5358#issuecomment-2726300551
...
mod tests {
    ...
    use check_console::{fmt, markup};
    use check_diagnostics::{DiagnosticTags, Severity};
    use check_diagnostics_categories::{Category, category};
    use check_text_edit::TextEdit;
    use check_text_size::{TextRange, TextSize};
    use serde_json::{from_value, json};

    use crate::{self as check_diagnostics};
    use crate::{
    ...
    impl Advices for CommandAdvice {
        fn record(&self, visitor: &mut dyn Visit) -> io::Result<()> {
            visitor.record_command("check command --argument")
        }
    }
    ...
    fn test_command_advice() {
        ...
        let expected = markup!{
            ...
            "  \n"
            "  "
            <Emphasis>"$"</Emphasis>" check command --argument\n"
            "  \n"
        }.to_owned();
