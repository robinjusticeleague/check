...
use check_deserialize::{Deserializable, DeserializableValue, DeserializationContext, Merge};
use std::{
    fmt,
...
impl check_console::fmt::Display for Bool<true> {
    fn fmt(&self, fmt: &mut Formatter) -> std::io::Result<()> {
        fmt.write_str("true")
    }
}

impl check_console::fmt::Display for Bool<false> {
    fn fmt(&self, fmt: &mut Formatter) -> std::io::Result<()> {
        fmt.write_str("false")
    }
}
