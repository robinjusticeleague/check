//! This module contains the configuration of `check.json`
//!
...
pub use crate::diagnostics::CheckDiagnostic;
...
use check_console::{KeyValuePair, markup};
use check_deserialize::{
...
use check_formatter::{IndentStyle, QuoteStyle};
use bpaf::Bpaf;
...
pub const VERSION: &str = match option_env!("CHECK_VERSION") {
...
pub struct Configuration {
    ...
    /// Indicates whether this configuration file is at the root of a Check
                ...
                .is_some_and(|extends| extends.extends_root())
        {
            ctx.report(
                DeserializationDiagnostic::new(
                    markup!(<Warn>"The configuration file cannot be both a root of a Check project and extended from a root configuration."</Warn>),
                )
                    .with_range(range)
...
impl Configuration {
    /// Returns the initial configuration as generated by `check init`.
    pub fn init() -> Self {
        Self {
            schema: Some(format!("https://checkjs.dev/schemas/{VERSION}/schema.json").into()),
    ...
    /// Whether Check should check for `.editorconfig` file
...
static SCHEMA_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"https://checkjs.dev/schemas/([\d.]+)/schema.json").unwrap());
                            ...
                            Ordering::Less | Ordering::Greater => {
                                ctx.report(
                                    DeserializationDiagnostic::new(
                                        ...
                                        .with_note(markup!("Run the command "<Emphasis>"check migrate"</Emphasis>" to migrate the configuration file."))
                                )
...
pub struct FilesConfiguration {
    ...
    /// A list of glob patterns. Check will handle only those files/folders that will
    /// match these patterns.
    #[bpaf(hide, pure(Default::default()))]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub includes: Option<Vec<check_glob::NormalizedGlob>>,

    /// Set of file and folder names that should be unconditionally ignored by
    /// Check's scanner.
    ///
    /// Check maintains an internal list of default ignore entries, which is
...
pub struct ConfigurationPayload {
    ...
    /// The path of where the `check.json` or `check.jsonc` file was found. This contains the file name.
