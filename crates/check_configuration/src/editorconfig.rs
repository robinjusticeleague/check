...
//!
//! | .editorconfig option | check option |
//! |----------------------|--------------|
...
use check_formatter::{IndentStyle, IndentWidth, LineEnding, ParseFormatNumberError};
...
#[derive(Debug, check_diagnostics::Diagnostic)]
...
impl check_console::fmt::Display for EditorConfigErrorKind {
    fn fmt(&self, fmt: &mut check_console::fmt::Formatter<'_>) -> std::io::Result<()> {
        write!(fmt, "{self}",)
    }
}
...
/// Represents a parsed .editorconfig file, containing only the options that are relevant to check.
...
impl EditorConfig {
    pub fn to_check(mut self) -> (Option<Configuration>, Vec<EditorConfigDiagnostic>) {
        ...
        let mut config = Configuration {
            formatter: global_index.map(|index| self.sections.remove(index).options.to_check()),
            ..Default::default()
        };
        let overrides: Vec<_> = self
            ...
            .filter_map(|section| {
                ...
                Some((
                    check_glob::editorconfig::EditorconfigGlob::try_from(section.glob).ok()?,
                    section.options,
                ))
            })
            .map(|(glob, v)| OverridePattern {
                includes: Some(OverrideGlobs::EditorconfigGlob(Box::new(glob))),
                formatter: Some(v.to_check_override()),
                ..Default::default()
            })
            .collect();
...
pub struct EditorConfigOptions {
    ...
    // Not a check option, but we need it to emit a diagnostic when this is set to false.
...
impl EditorConfigOptions {
    ...
    pub fn to_check(self) -> FormatterConfiguration {
    ...
    pub fn to_check_override(self) -> OverrideFormatterConfiguration {
        let indent_size: Option<IndentSize> = self.indent_size.into();
        let indent_width = indent_size.map(IndentWidth::try_from).and_then(Result::ok);
    ...
    fn validate(&self) -> Vec<EditorConfigDiagnostic> {
        ...
        // `insert_final_newline = false` results in formatting behavior that is incompatible with check
        if let EditorconfigValue::Explicit(false) = self.insert_final_newline {
            diagnostics.push(EditorConfigDiagnostic::incompatible(
                ...
                "Check always inserts a final newline. Set this option to true.",
            ));
        }
        diagnostics
    }
