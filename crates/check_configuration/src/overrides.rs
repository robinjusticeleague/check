...
pub struct OverridePattern {
    /// A list of glob patterns. Check will include files/folders that will
...
pub enum OverrideGlobs {
    Globs(Box<[check_glob::NormalizedGlob]>),
    EditorconfigGlob(Box<check_glob::editorconfig::EditorconfigGlob>),
}
impl OverrideGlobs {
    /// Normalize `path` and match it against the list of globs.
    pub fn is_match_candidate(&self, path: &check_glob::CandidatePath) -> bool {
        match self {
...
impl check_deserialize::Deserializable for OverrideGlobs {
    fn deserialize(
        ctx: &mut impl check_deserialize::DeserializationContext,
        value: &impl check_deserialize::DeserializableValue,
        name: &str,
    ) -> Option<Self> {
        check_deserialize::Deserializable::deserialize(ctx, value, name).map(OverrideGlobs::Globs)
    }
}
#[cfg(feature = "schema")]
impl schemars::JsonSchema for OverrideGlobs {
    ...
    fn json_schema(generator: &mut schemars::r#gen::SchemaGenerator) -> schemars::schema::Schema {
        Vec::<check_glob::Glob>::json_schema(generator)
    }
}
...
pub struct OverrideFormatterConfiguration {
    ...
    /// When formatting `package.json`, Check will use `always` unless configured otherwise. Defaults to "auto".
    #[serde(skip_serializing_if = "Option::is_none")]
