...
use check_console::fmt::Display;
use check_console::{MarkupBuf, markup};
use check_deserialize::DeserializationDiagnostic;
use check_diagnostics::{
    Advices, Category, Diagnostic, Error, Location, LogCategory, MessageAndDescription, Severity,
    Visit, category,
};
use check_resolver::{ResolveError, ResolveErrorDiagnostic};
use check_rowan::{SyntaxError, TextRange};
...
/// Series of errors that can be thrown while computing the configuration, specifically for `check.json`.
...
impl From<SyntaxError> for CheckDiagnostic {
    fn from(_: SyntaxError) -> Self {
        Self::Deserialization(DeserializationDiagnostic::new(markup! {"Syntax Error"}))
    }
}

impl From<DeserializationDiagnostic> for CheckDiagnostic {
...
#[diagnostic(
    ...
    severity = Error,
    message(
        message("Check couldn't find a configuration in the directory "<Emphasis>{self.path}</Emphasis>"."),
        description = "Check couldn't find a configuration in the directory {path}."
    )
)]
...
#[diagnostic(
    ...
    severity = Error,
    message(
        message("Check couldn't find a root configuration to extend from. Remove the "<Emphasis>"extends"</Emphasis>" field or make sure the parent configuration has "<Emphasis>"root: true"</Emphasis>".")
    )
)]
...
pub enum EditorConfigDiagnostic {
    ...
    /// An option is completely incompatible with check.
    Incompatible(IncompatibleDiagnostic),
    /// A glob pattern that check doesn't support.
...
impl EditorConfigDiagnostic {
    pub fn incompatible(key: impl Into<String>, message: impl Into<String>) -> Self {
        Self::Incompatible(IncompatibleDiagnostic {
            message: MessageAndDescription::from(
                markup! { "Key '"{key.into()}"' is incompatible with check: "{message.into()}}
                    .to_owned(),
            ),
        })
    }
    ...
    pub fn unknown_glob_pattern(pattern: impl Into<String>) -> Self {
        Self::UnknownGlobPattern(UnknownGlobPatternDiagnostic {
            message: MessageAndDescription::from(
                markup! { "This glob pattern is incompatible with check: "{pattern.into()}}
                    .to_owned(),
            ),
        })
    }
...
#[cfg(test)]
mod test {
    use crate::{CheckDiagnostic, Configuration};
    ...
    #[test]
    fn diagnostic_size() {
        assert_eq!(std::mem::size_of::<CheckDiagnostic>(), 96);
    }
    ...
    fn config_already_exists() {
        snap_diagnostic(
            ...
            CheckDiagnostic::new_already_exists().with_file_path("check.json"),
        )
    }
