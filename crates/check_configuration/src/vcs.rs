...
pub struct VcsConfiguration {
    ...
    /// Whether Check should use the VCS ignore file. When [true], Check will ignore the files
    /// specified in the ignore file.
    #[bpaf(long("vcs-use-ignore-file"), argument("true|false"))]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub use_ignore_file: Option<VcsUseIgnoreFile>,

    /// The folder where Check should check for VCS files. By default, Check will use the same
    /// folder where `check.json` was found.
    ///
    /// If Check can't find the configuration, it will attempt to use the current working directory.
    /// If no current working directory can't be found, Check won't use the VCS integration, and a diagnostic
    /// will be emitted
    ...
    fn validate(
        ...
        if self.client_kind.is_none() && self.is_enabled() {
            ctx.report(
                ...
                .with_range(range)
                .with_note("Check will disable the VCS integration until the issue is fixed."),
            );
            return false;
        }
