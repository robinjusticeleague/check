...
use check_analyze::options::RuleOptions;
use check_analyze::{FixKind, RuleCategory, RuleFilter};
...
impl RuleSelector {
    ...
    ///
    /// In Check, the only assists that belong to the `source` group can be applied when applying executing the `source.fixAll` signal from the editor.
    /// Hence, these filters are usually written as `source.check.*`. So we already know the group of the rule in advance.
    ///
    /// On the other hand, linter rules work differently. They are prefixed with `quickfix.check.*` and they must have the name of the group in their name. For example:
    /// - `quickfix.check.style.useConst`
    /// - `quickfix.check.a11y.useAltText`
    ///
    /// ```
    /// use check_configuration::analyzer::RuleSelector;
    ///
    /// let filter = "source.check.useSortedKeys";
    /// let selector = RuleSelector::from_lsp_filter(filter).unwrap();
    /// assert_eq!(selector, RuleSelector::Rule("source", "useSortedKeys"));
    /// let filter = "quickfix.check.style.useConst";
    /// let selector = RuleSelector::from_lsp_filter(filter).unwrap();
    /// assert_eq!(selector, RuleSelector::Rule("style", "useConst"));
    /// let filter = "quickfix.check.a11y.useAltText";
    /// let selector = RuleSelector::from_lsp_filter(filter).unwrap();
    /// assert_eq!(selector, RuleSelector::Rule("a11y", "useAltText"));
    /// ```
    pub fn from_lsp_filter(filter: &str) -> Option<Self> {
        if let Some(filter) = filter.strip_prefix("source.check.") {
        ...
        } else if let Some(filter) = filter.strip_prefix("quickfix.check.") {
...
impl FromStr for RuleSelector {
    ...
    fn from_str(selector: &str) -> Result<Self, Self::Err> {
        ...
        if let Ok(rule_name) = linter::RuleName::from_str(rule_or_group_name) {
            ...
            //
            // See https://github.com/checkjs/check/issues/6018
