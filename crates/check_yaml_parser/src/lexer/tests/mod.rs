...
macro_rules! assert_lex {
    ($src:expr, $($kind:ident:$len:expr $(,)?)*) => {{
        ...
        let mut idx = 0;
        let mut tok_idx = check_rowan::TextSize::default();

        let mut new_str = String::with_capacity($src.len());
        let mut tokens = vec![];

        use check_parser::lexer::Lexer;
        while lexer.next_token(()) != check_yaml_syntax::YamlSyntaxKind::EOF {
        ...
        $(
            assert_eq!(
                tokens[idx].0,
                check_yaml_syntax::YamlSyntaxKind::$kind,
