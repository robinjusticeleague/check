...
fn scanner_doesnt_show_errors_for_inaccessible_files() {
    ...
    let mut fs = check_fs::TemporaryFs::new("scanner_doesnt_show_errors_for_inaccessible_files");
...
fn scanner_ignored_files_are_not_loaded() {
    ...
    let configuration = fs
        .read_file_from_path(fixtures_path.join("check.jsonc").as_path())
        .unwrap();
...
fn get_fixtures_path(joined: &str) -> Utf8PathBuf {
    ...
    path.join("crates/check_service/tests/fixtures")
        .join(joined)
}
