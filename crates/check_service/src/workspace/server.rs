...
use check_analyze::{AnalyzerPluginVec, RuleCategory};
use check_configuration::bool::Bool;
use check_configuration::plugins::{PluginConfiguration, Plugins};
use check_configuration::vcs::VcsClientKind;
use check_configuration::{CheckDiagnostic, Configuration, ConfigurationPathHint};
use check_deserialize::json::deserialize_from_json_str;
use check_deserialize::{Deserialized, Merge};
use check_diagnostics::print_diagnostic_to_string;
use check_diagnostics::{
    Diagnostic, DiagnosticExt, Severity, serde::Diagnostic as SerdeDiagnostic,
};
use check_formatter::Printed;
use check_fs::{CheckPath, ConfigName, PathKind};
use check_grit_patterns::{CompilePatternOptions, GritQuery, compile_pattern_with_options};
use check_js_syntax::{AnyJsRoot, LanguageVariant, ModuleKind};
use check_json_parser::JsonParserOptions;
use check_json_syntax::JsonFileSource;
use check_module_graph::{ModuleDependencies, ModuleDiagnostic, ModuleGraph};
use check_package::PackageType;
use check_parser::AnyParse;
use check_plugin_loader::{CheckPlugin, PluginCache, PluginDiagnostic};
use check_project_layout::ProjectLayout;
use check_resolver::FsWithResolverProxy;
use check_rowan::{AstNode, NodeCache, SendNode};
...
impl WorkspaceServer {
    ...
    /// given `path`.
    ///
    /// The root of a project is where the top-level `check.json` resides. So
    /// the returned path is always the given path or one of its parents.
    ///
    /// An error may be returned if no top-level `check.json` can be found, or
    /// if there is an error opening a config file.
    fn find_project_root(&self, path: CheckPath) -> Result<Utf8PathBuf, WorkspaceError> {
        ...
        for ancestor in path.ancestors() {
            ...
            let extension = config_path
                .extension()
                .ok_or_else(|| CheckDiagnostic::invalid_configuration("Missing extension"))?;
            let file_source = JsonFileSource::try_from_extension(extension)
                .map_err(|err| CheckDiagnostic::invalid_configuration(err.to_string()))?;
            ...
            {
                return Err(
                    CheckDiagnostic::invalid_configuration(print_diagnostic_to_string(error))
                        .into(),
                );
            }
    ...
    /// Check configuration file, and returns its path if found.
    ...
    fn get_file_capabilities(&self, path: &CheckPath) -> Capabilities {
    ...
    fn open_file_internal(
        ...
        let path: Utf8PathBuf = check_path.clone().into();
        ...
        if let DocumentFileSource::Js(js) = &mut source {
            ...
            if !js.is_typescript() && !js.is_jsx() {
                let settings = self
                    .projects
                    .get_settings_based_on_path(project_key, &check_path)
        ...
        let (embedded_scripts, embedded_styles) = if let Some(DocumentFileSource::Html(_)) =
            ...
            && let Some(html_root) = check_html_syntax::HtmlRoot::cast(any_parse.syntax())
        {
            let mut node_cache = NodeCache::default();
        ...
        for plugin_config in plugins.iter() {
            match plugin_config {
                PluginConfiguration::Path(plugin_path) => {
                    match CheckPlugin::load(self.fs.clone(), plugin_path, base_path) {
                        Ok((plugin, _)) => {
...
impl Workspace for WorkspaceServer {
    ...
    fn update_settings(
        ...
        let mut diagnostics: Vec<check_diagnostics::serde::Diagnostic> = vec![];
                ...
                match settings.vcs_settings.client_kind {
                    ...
                    Some(VcsClientKind::Git) => {
                        ...
                        let content = match result {
                            ...
                            None => {
                                diagnostics.push(check_diagnostics::serde::Diagnostic::new(
    ...
    fn is_path_ignored(&self, params: PathIsIgnoredParams) -> Result<bool, WorkspaceError> {
        // Never ignore Check's top-level config file regardless of `includes`.
        if params.path.file_name().is_some_and(|file_name| {
            file_name == ConfigName::check_json() || file_name == ConfigName::check_jsonc()
    ...
    fn change_file(
        ...
        let (embedded_scripts, embedded_styles) = if let Some(file_source) = self.get_source(index)
            ...
            && let Some(html_root) =
                check_html_syntax::HtmlRoot::cast(parsed.any_parse.syntax().clone())
        {
...
impl WorkspaceScannerBridge for WorkspaceServer {
    #[inline]
    fn fs(&self) -> &dyn check_fs::FileSystem {
        self.fs.as_ref()
    }
    ...
    fn update_project_config_files(
        ...
        for filtered_path in filtered_paths {
            ...
            if nested_configuration.is_root() {
                returned_diagnostics.push(check_diagnostics::serde::Diagnostic::new(
                    CheckDiagnostic::root_in_root(
            ...
            let result = self.update_settings(UpdateSettingsParams {
                project_key,
                workspace_directory: nested_directory_path.map(CheckPath::from),
                configuration: nested_configuration,
            })?;
    ...
    fn unload_file(
        &self,
        path: &Utf8Path,
    ) -> Result<Vec<check_diagnostics::serde::Diagnostic>, WorkspaceError> {
...
fn init_thread_pool(threads: Option<usize>) {
    ...
    INIT_ONCE.call_once(|| {
        rayon::ThreadPoolBuilder::new()
            .thread_name(|index| format!("check::workspace_worker_{index}"))
