...
use check_css_formatter::format_node;
use check_css_parser::CssParserOptions;
use check_css_semantic::semantic_model;
use check_css_syntax::{CssLanguage, CssRoot, CssSyntaxNode};
...
fn parse(
    ...
    settings
        .override_settings
        .apply_override_css_parser_options(check_path, &mut options);

    let parse = check_css_parser::parse_css_with_cache(text, cache, options);
    ParseResult {
...
fn debug_formatter_ir(
    ...
    let options = settings.format_options::<CssLanguage>(check_path, document_file_source);
...
fn format_range(
    ...
    let tree = parse.syntax();
    let printed = check_css_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
fn format_on_type(
    ...
    let printed = check_css_formatter::format_sub_tree(options, &root_node)?;
    Ok(printed)
}
...
mod test {
    use super::*;
    use check_css_syntax::CssFileSource;
