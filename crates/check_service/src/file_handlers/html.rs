...
use check_analyze::AnalyzerOptions;
use check_configuration::html::{
    HtmlFormatterConfiguration, HtmlFormatterEnabled, HtmlParseInterpolation,
    HtmlParserConfiguration,
};
use check_css_parser::{CssParserOptions, parse_css_with_offset_and_cache};
...
use check_html_formatter::{
    HtmlFormatOptions,
    context::{IndentScriptAndStyle, WhitespaceSensitivity},
    format_node,
};
use check_html_parser::{HtmlParseOptions, parse_html_with_cache};
use check_html_syntax::{HtmlElement, HtmlLanguage, HtmlRoot, HtmlSyntaxNode};
use check_js_parser::{JsParserOptions, parse_js_with_offset_and_cache};
use check_js_syntax::JsFileSource;
use check_parser::AnyParse;
use check_rowan::{AstNode, AstNodeList, NodeCache};
use camino::Utf8Path;
...
fn debug_syntax_tree(_check_path: &CheckPath, parse: AnyParse) -> GetSyntaxTreeResult {
...
fn format(
    ...
    let options = settings.format_options::<HtmlLanguage>(check_path, document_file_source);
...
pub(crate) fn fix_all(params: FixAllParams) -> Result<FixFileResult, WorkspaceError> {
    ...
    let code = if params.should_format {
        format_node(
            params
                .settings
                .format_options::<HtmlLanguage>(params.check_path, &params.document_file_source),
            tree.syntax(),
        )?
        .print()?
        .into_code()
