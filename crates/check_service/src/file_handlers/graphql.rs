...
use check_analyze::{AnalysisFilter, AnalyzerOptions, ControlFlow, Never, RuleError};
use check_configuration::graphql::{
...
use check_graphql_analyze::analyze;
use check_graphql_formatter::context::GraphqlFormatOptions;
use check_graphql_formatter::format_node;
use check_graphql_parser::parse_graphql_with_cache;
use check_graphql_syntax::{GraphqlLanguage, GraphqlRoot, GraphqlSyntaxNode, TextRange, TextSize};
use check_parser::AnyParse;
use check_rowan::{AstNode, NodeCache, TokenAtOffset};
use camino::Utf8Path;
use std::borrow::Cow;
...
fn debug_formatter_ir(
    ...
    let options = settings.format_options::<GraphqlLanguage>(check_path, document_file_source);
...
fn format_range(
    ...
    let tree = parse.syntax();
    let printed = check_graphql_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
fn format_on_type(
    ...
    let printed = check_graphql_formatter::format_sub_tree(options, &root_node)?;
    Ok(printed)
}
