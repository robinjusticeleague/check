...
use check_fs::{CheckPath, ConfigName};
use check_json_analyze::analyze;
use check_json_formatter::context::{JsonFormatOptions, TrailingCommas};
use check_json_formatter::format_node;
use check_json_parser::JsonParserOptions;
use check_json_syntax::{JsonFileSource, JsonLanguage, JsonRoot, JsonSyntaxNode};
use check_parser::AnyParse;
use check_rowan::{AstNode, NodeCache};
use check_rowan::{TextRange, TextSize, TokenAtOffset};
use camino::Utf8Path;
use std::borrow::Cow;
...
impl ServiceLanguage for JsonLanguage {
    ...
    fn resolve_format_options(
        ...
        // ensure it never formats check.json into a form it can't parse
        let trailing_commas = if matches!(path.file_name(), Some("check.json")) {
            TrailingCommas::None
        } else {
...
fn parse(
    ...
    let options = if check_path.ends_with(ConfigName::check_jsonc()) {
        ...
        overrides.apply_override_json_parser_options(check_path, &mut options);

        options
    };

    let parse = check_json_parser::parse_json_with_cache(text, cache, options);
...
fn format_range(
    ...
    let tree = parse.syntax();
    let printed = check_json_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
fn format_on_type(
    ...
    let printed = check_json_formatter::format_sub_tree(options, &root_node)?;
    Ok(printed)
}
...
fn lint(params: LintParams) -> LintResults {
    ...
    // if we're parsing the `check.json` file, we deserialize it, so we can emit diagnostics for
    // malformed configuration
    if params.path.ends_with(ConfigName::check_json())
        || params.path.ends_with(ConfigName::check_jsonc())
    {
...
fn fix_all(params: FixAllParams) -> Result<FixFileResult, WorkspaceError> {
    ...
    let Some(file_source) = params
        ...
        .to_json_file_source()
        .or(JsonFileSource::try_from(params.check_path.as_path()).ok())
    else {
