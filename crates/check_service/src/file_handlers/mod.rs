...
use check_configuration::Rules;
use check_configuration::analyzer::{RuleDomainValue, RuleSelector};
use check_console::fmt::Formatter;
use check_css_analyze::METADATA as css_metadata;
use check_css_syntax::{CssFileSource, CssLanguage};
use check_diagnostics::{Diagnostic, DiagnosticExt, Severity, category};
use check_formatter::Printed;
use check_fs::CheckPath;
use check_graphql_analyze::METADATA as graphql_metadata;
use check_graphql_syntax::{GraphqlFileSource, GraphqlLanguage};
use check_grit_patterns::{GritQuery, GritQueryEffect, GritTargetFile};
use check_grit_syntax::file_source::GritFileSource;
use check_html_syntax::HtmlFileSource;
use check_js_analyze::METADATA as js_metadata;
...
use check_json_analyze::METADATA as json_metadata;
use check_json_syntax::{JsonFileSource, JsonLanguage};
use check_module_graph::ModuleGraph;
use check_parser::AnyParse;
use check_project_layout::ProjectLayout;
use check_rowan::{FileSourceError, NodeCache};
use check_string_case::StrLikeExtension;

use crate::file_handlers::ignore::IgnoreFileHandler;
use check_configuration::vcs::{GIT_IGNORE_FILE_NAME, IGNORE_FILE_NAME};
use check_package::PackageJson;
use camino::Utf8Path;
...
impl DocumentFileSource {
    ...
    pub(crate) fn try_from_path(path: &Utf8Path) -> Result<Self, FileSourceError> {
        ...
        // TODO: This should be extracted to a dedicated function, maybe in check_fs
    ...
    /// use check_json_syntax::JsonFileSource;
...
impl check_console::fmt::Display for DocumentFileSource {
...
pub struct FixAllParams<'a> {
    ...
    pub(crate) check_path: &'a CheckPath,
...
type Parse = fn(&CheckPath, DocumentFileSource, &str, &Settings, &mut NodeCache) -> ParseResult;
...
type DebugSyntaxTree = fn(&CheckPath, AnyParse) -> GetSyntaxTreeResult;
type DebugControlFlow = fn(AnyParse, TextSize) -> String;
type DebugFormatterIR =
    fn(&CheckPath, &DocumentFileSource, AnyParse, &Settings) -> Result<String, WorkspaceError>;
type DebugTypeInfo =
    fn(&CheckPath, Option<AnyParse>, Arc<ModuleGraph>) -> Result<String, WorkspaceError>;
type DebugRegisteredTypes = fn(&CheckPath, AnyParse) -> Result<String, WorkspaceError>;
type DebugSemanticModel = fn(&CheckPath, AnyParse) -> Result<String, WorkspaceError>;
...
pub(crate) struct LintResults {
    pub(crate) diagnostics: Vec<check_diagnostics::serde::Diagnostic>,
...
pub(crate) struct ProcessLint<'a> {
    ...
    errors: usize,
    diagnostics: Vec<check_diagnostics::serde::Diagnostic>,
...
impl<'a> ProcessLint<'a> {
    ...
    pub(crate) fn process_signal<L: check_rowan::Language>(
        &mut self,
        ...
        if let Some(mut diagnostic) = signal.diagnostic() {
            ...
            self.diagnostics
                .push(check_diagnostics::serde::Diagnostic::new(error));
        }
    ...
    pub(crate) fn into_result(
        self,
        parse_diagnostics: Vec<check_diagnostics::serde::Diagnostic>,
        analyzer_diagnostics: Vec<check_diagnostics::Error>,
    ) -> LintResults {
        ...
        diagnostics.extend(
            analyzer_diagnostics
                .into_iter()
                .map(check_diagnostics::serde::Diagnostic::new)
                .collect::<Vec<_>>(),
        );
...
type Rename = fn(&CheckPath, AnyParse, TextSize, String) -> Result<RenameResult, WorkspaceError>;
...
type Format =
    fn(&CheckPath, &DocumentFileSource, AnyParse, &Settings) -> Result<Printed, WorkspaceError>;
type FormatRange = fn(
    &CheckPath,
    &DocumentFileSource,
    AnyParse,
...
/// Main trait to use to add a new language to Check
...
impl Features {
    ...
    /// Returns the [Capabilities] associated with a [CheckPath]
...
/// Type meant to register all the syntax rules for each language supported by Check
///
...
/// Type meant to register all the lint rules for each language supported by Check
///
#[derive(Debug)]
...
impl<'b> AnalyzerVisitorBuilder<'b> {
    ...
    pub(crate) fn finish(self) -> (Vec<RuleFilter<'b>>, Vec<RuleFilter<'b>>, AnalyzerOptions) {
        ...
        check_js_analyze::visit_registry(&mut syntax);
        check_css_analyze::visit_registry(&mut syntax);
        check_json_analyze::visit_registry(&mut syntax);
        check_graphql_analyze::visit_registry(&mut syntax);
        ...
        check_js_analyze::visit_registry(&mut lint);
        check_css_analyze::visit_registry(&mut lint);
        check_json_analyze::visit_registry(&mut lint);
        check_graphql_analyze::visit_registry(&mut lint);
        ...
        check_js_analyze::visit_registry(&mut assist);
        check_css_analyze::visit_registry(&mut assist);
        check_json_analyze::visit_registry(&mut assist);
        check_graphql_analyze::visit_registry(&mut assist);
