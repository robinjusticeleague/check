...
use check_analyze::options::PreferredQuote;
use check_analyze::{
...
use check_configuration::javascript::{
...
use check_diagnostics::Applicability;
use check_formatter::{
...
use check_js_analyze::utils::rename::{RenameError, RenameSymbolExtensions};
use check_js_analyze::{
    ControlFlowGraph, JsAnalyzerServices, analyze, analyze_with_inspect_matcher,
};
use check_js_formatter::context::trailing_commas::TrailingCommas;
use check_js_formatter::context::{
    ArrowParentheses, JsFormatOptions, OperatorLinebreak, QuoteProperties, Semicolons,
};
use check_js_formatter::format_node;
use check_js_parser::JsParserOptions;
use check_js_semantic::{SemanticModelOptions, semantic_model};
use check_js_syntax::{
...
use check_js_type_info::{GlobalsResolver, ScopeId, TypeData, TypeResolver};
use check_module_graph::ModuleGraph;
use check_parser::AnyParse;
use check_rowan::{AstNode, BatchMutationExt, Direction, NodeCache, WalkEvent};
use camino::Utf8Path;
...
impl ServiceLanguage for JsLanguage {
    ...
    fn resolve_analyzer_options(
        ...
        let jsx_runtime = match overrides.override_jsx_runtime(
            ...
            JsxRuntime::Transparent => check_analyze::options::JsxRuntime::Transparent,
            JsxRuntime::ReactClassic => check_analyze::options::JsxRuntime::ReactClassic,
        };
...
fn parse(
    ...
    settings
        .override_settings
        .apply_override_js_parser_options(check_path, &mut options);

    let file_source = file_source.to_js_file_source().unwrap_or_default();
    let parse = check_js_parser::parse_js_with_cache(text, file_source, options, cache);
    ParseResult {
...
fn debug_syntax_tree(_rome_path: &CheckPath, parse: AnyParse) -> GetSyntaxTreeResult {
...
fn debug_registered_types(_path: &CheckPath, parse: AnyParse) -> Result<String, WorkspaceError> {
    let tree: AnyJsRoot = parse.tree();
    let mut result = String::new();
    let preorder = tree.syntax().preorder();
...
fn debug_semantic_model(_path: &CheckPath, parse: AnyParse) -> Result<String, WorkspaceError> {
    let tree: AnyJsRoot = parse.tree();
...
pub(crate) fn fix_all(params: FixAllParams) -> Result<FixFileResult, WorkspaceError> {
    ...
    let rules = params.settings.as_linter_rules(params.check_path.as_path());
    let analyzer_options = params.settings.analyzer_options::<JsLanguage>(
        params.check_path,
    ...
    let (enabled_rules, disabled_rules, analyzer_options) =
        AnalyzerVisitorBuilder::new(params.settings, analyzer_options)
            .with_only(&params.only)
            .with_skip(&params.skip)
            .with_path(params.check_path.as_path())
            .with_enabled_rules(&params.enabled_rules)
            .with_project_layout(params.project_layout.clone())
            .finish();
    ...
    let Some(file_source) = params
        .document_file_source
        .to_js_file_source()
        .or(JsFileSource::try_from(params.check_path.as_path()).ok())
    else {
        return Err(extension_error(params.check_path));
    };
...
pub(crate) fn format(
    ...
    let options = settings.format_options::<JsLanguage>(check_path, document_file_source);
    debug!("{:?}", &options);
    let tree = parse.syntax();
    let formatted = format_node(options, &tree)?;
    match formatted.print() {
        ...
        Err(error) => {
            error!("The file {} couldn't be formatted", check_path.as_str());
...
pub(crate) fn format_range(
    ...
    debug!("{:?}", &options);
    let tree = parse.syntax();
    let printed = check_js_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
pub(crate) fn format_on_type(
    ...
    let printed = check_js_formatter::format_sub_tree(options, &root_node)?;
    Ok(printed)
}

fn rename(
    _rome_path: &CheckPath,
    parse: AnyParse,
    symbol_at: TextSize,
    new_name: String,
