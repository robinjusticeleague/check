...
use check_analyze::{AnalyzerOptions, QueryMatch};
use check_configuration::grit::{
...
use check_diagnostics::{Diagnostic, Severity};
use check_formatter::{FormatError, IndentStyle, IndentWidth, LineEnding, LineWidth, Printed};
use check_fs::CheckPath;
use check_grit_formatter::{context::GritFormatOptions, format_node, format_sub_tree};
use check_grit_parser::parse_grit_with_cache;
use check_grit_syntax::{GritLanguage, GritRoot, GritSyntaxNode};
use check_parser::AnyParse;
use check_rowan::{AstNode, NodeCache, TextRange, TextSize, TokenAtOffset};
use camino::Utf8Path;
...
fn debug_formatter_ir(
    ...
    let options = settings.format_options::<GritLanguage>(check_path, document_file_source);
...
fn format_range(
    ...
    let tree = parse.syntax();
    let printed = check_grit_formatter::format_range(options, &tree, range)?;
    Ok(printed)
}
...
pub(crate) fn fix_all(params: FixAllParams) -> Result<FixFileResult, WorkspaceError> {
    ...
    let code = if params.should_format {
        format_node(
            params
                .settings
                .format_options::<GritLanguage>(params.check_path, &params.document_file_source),
            tree.syntax(),
        )?
        .print()?
        .into_code()
