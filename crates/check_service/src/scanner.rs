...
use check_diagnostics::serde::Diagnostic;
use check_diagnostics::{Diagnostic as _, DiagnosticExt, Error, Severity};
use check_fs::{CheckPath, PathInterner, PathKind, TraversalContext, TraversalScope};
use check_module_graph::ModuleDependencies;
...
pub(crate) struct ScanOptions {
    ...
    ///
    /// If `false`, only diagnostics related to Check configuration files are
    /// reported.
    pub verbose: bool,
...
pub(crate) struct ScanResult {
    ...
    /// Check configuration files are reported.
...
fn scan_dependencies<W: WorkspaceScannerBridge>(
    ...
    rayon::scope(|s| {
        fn index_dependency<'a, W: WorkspaceScannerBridge>(
            ...
            let dependencies = open_file(ctx, CheckPath::new(dependency_path), ctx.trigger);
            ctx.dependencies
                .lock()
                .unwrap()
