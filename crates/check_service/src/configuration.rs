...
use check_configuration::analyzer::{RuleDomainValue, RuleSelector};
use check_configuration::diagnostics::{
    CantLoadExtendFile, CantResolve, EditorConfigDiagnostic, ParseFailedDiagnostic,
};
use check_configuration::editorconfig::EditorConfig;
use check_configuration::{
    CheckDiagnostic, ConfigurationPathHint, ConfigurationPayload, push_to_analyzer_rules,
};
use check_configuration::{Configuration, VERSION, push_to_analyzer_assist};
use check_console::markup;
use check_css_analyze::METADATA as css_lint_metadata;
use check_css_syntax::CssLanguage;
use check_deserialize::json::deserialize_from_json_str;
use check_deserialize::{Deserialized, Merge};
use check_diagnostics::{DiagnosticExt, Error, Severity};
use check_fs::{AutoSearchResult, ConfigName, FileSystem, OpenOptions};
use check_graphql_analyze::METADATA as graphql_lint_metadata;
use check_graphql_syntax::GraphqlLanguage;
use check_js_analyze::METADATA as js_lint_metadata;
use check_js_syntax::JsLanguage;
use check_json_analyze::METADATA as json_lint_metadata;
...
use check_resolver::{FsWithResolverProxy, ResolveOptions, resolve};
...
///     be `check.json` or `check.jsonc`. And if it doesn't end with `.json`,
///     Check will try to deserialize it as a `.jsonc` file.
///
/// - If the path hint is a path to a directory which is provided by the user,
///     the function will try to find a `check.json` or `check.jsonc` file in
...
///     system until it finds a `check.json` or `check.jsonc` file, or there
///     aren't directories anymore. In this case, the function will not error
///     but return an `Ok(None)`, which means Check will use the default
///     configuration.
///
...
pub fn read_config(
    ...
    // We search for the first non-root `check.json` or `check.jsonc` files:
    ...
    let Some(auto_search_result) = fs.auto_search_files_with_predicate(
        ...
        &[ConfigName::check_json(), ConfigName::check_jsonc()],
        &mut predicate,
    ) else {
        return Ok(None);
    };
    ...
    if fs.path_is_file(config_file_path) {
        ...
        let parser_options = match config_file_path.extension() {
            ...
            Some("jsonc") => JsonParserOptions::default()
                .with_allow_comments()
                .with_allow_trailing_commas(),
            _ => return Err(CheckDiagnostic::invalid_configuration_file(config_file_path).into()),
        };
        ...
        let check_json_path = config_file_path.join(ConfigName::check_json());
        let check_jsonc_path = config_file_path.join(ConfigName::check_jsonc());
        let check_json_exists = fs.path_exists(check_json_path.as_path());
        let check_jsonc_exists = fs.path_exists(check_jsonc_path.as_path());

        if !check_json_exists && !check_jsonc_exists {
            return Err(CheckDiagnostic::no_configuration_file_found(config_file_path).into());
        }

        let (config_path, parser_options) = if check_json_exists {
            (check_json_path, JsonParserOptions::default())
        } else {
            (
                check_jsonc_path,
        ...
        {
            return Err(CheckDiagnostic::non_root_configuration(config_file_path).into());
        }
...
pub fn load_editorconfig(
    ...
    // We currently don't support the `root` property, so we just search for the file like we do for check.json.
    // And we make some judge for the case when `check.json` and `.editorconfig` both exists.
    // If we found a `.editorconfig` directory higher than `check.json`, we'll don't use it.
    if let Some(auto_search_result) = fs.auto_search_files(&workspace_root, &[".editorconfig"]) {
        ...
        if let Some(config_path) = config_path {
            // if `.edirotconfig` is higher than `check.json`
            if is_parent_of(directory_path, config_path) {
                Ok((None, vec![]))
            } else {
                Ok(editorconfig.to_check())
            }
        } else {
            // If we don't find `check.json`, we'll use `.editorconfig`
            Ok(editorconfig.to_check())
        }
    } else {
        Ok((None, vec![]))
...
pub fn create_config(
    ...
    let json_path = Utf8PathBuf::from(ConfigName::check_json());
    let jsonc_path = Utf8PathBuf::from(ConfigName::check_jsonc());

    if fs.path_exists(&json_path) || fs.path_exists(&jsonc_path) {
        return Err(CheckDiagnostic::new_already_exists().into());
    }
    ...
    let mut config_file = fs.open_with_options(&path, options).map_err(|err| {
        if err.kind() == ErrorKind::AlreadyExists {
            CheckDiagnostic::new_already_exists().into()
        } else {
            WorkspaceError::cant_read_file(path.to_string())
        }
    })?;

    // we now check if check is installed inside `node_modules` and if so, we
    if VERSION == "0.0.0" {
        let schema_path = Utf8Path::new("./node_modules/@checkjs/check/configuration_schema.json");
        ...
        configuration.schema =
            Some(format!("https://checkjs.dev/schemas/{VERSION}/schema.json").into());
    }

    let contents = serde_json::to_string_pretty(&configuration)
        .map_err(|_| CheckDiagnostic::new_serialization_error())?;
            ...
            for extend_entry in extends.iter() {
                ...
                let extend_configuration_file_path = if extend_entry_as_path.starts_with(".") {
                    ...
                    const RESOLVE_OPTIONS: ResolveOptions = ResolveOptions::new()
                        ...
                        .with_condition_names(&["check", "default"]);
                    ...
                    .map_err(|error| {
                        CantResolve::new(Utf8PathBuf::from(extend_entry), error)
                            .with_verbose_advice(markup! {
                                "Check tried to resolve the configuration file \""<Emphasis>{
                                    extend_entry
                ...
                let mut file = fs
                    ...
                    .map_err(|err| {
                        ...
                        .with_verbose_advice(markup! {
                            "Check tried to load the configuration file \""<Emphasis>{
                                extend_configuration_file_path.to_string()
                            }</Emphasis>"\" in \"extends\" using \""<Emphasis>{
                                external_resolution_base_path.to_string()
                            }</Emphasis>"\" as the base path."
...
mod test {
    ...
    use check_configuration::{
        CheckDiagnostic, ConfigurationPathHint, diagnostics::ConfigurationDiagnostic,
    };
    use check_fs::MemoryFileSystem;
    use camino::Utf8PathBuf;

    #[test]
    fn should_not_load_a_configuration_yml() {
        let fs = MemoryFileSystem::default();
        fs.insert(Utf8PathBuf::from("check.yml"), "content".to_string());
        let path_hint = ConfigurationPathHint::FromUser(Utf8PathBuf::from("check.yml"));
    ...
    fn should_skip_non_root_configuration() {
        let fs = MemoryFileSystem::default();
        fs.insert(
            Utf8PathBuf::from("/check.json"),
            r#"{ "linter": { "enabled": false } }"#.to_string(),
        );
        fs.insert(
            Utf8PathBuf::from("/nested/check.json"),
    ...
    fn should_refuse_user_provided_non_root_configuration() {
        ...
        match load_configuration(&fs, path_hint) {
            ...
            Err(err) => {
                assert!(matches!(
                    err,
                    WorkspaceError::Configuration(ConfigurationDiagnostic::Check(
                        CheckDiagnostic::NonRootConfiguration(_)
                    ))
                ));
            }
        }
...
impl<'a> ProjectScanComputer<'a> {
    ...
    pub fn compute(mut self) -> ScanKind {
        ...
        check_graphql_analyze::visit_registry(&mut self);
        check_css_analyze::visit_registry(&mut self);
        check_json_analyze::visit_registry(&mut self);
        check_js_analyze::visit_registry(&mut self);
...
mod tests {
    use super::*;
    use check_configuration::analyzer::{
