...
use check_analyze::RuleError;
use check_configuration::diagnostics::{
    CantResolve, ConfigurationDiagnostic, EditorConfigDiagnostic,
};
use check_configuration::{CheckDiagnostic, CantLoadExtendFile};
use check_console::fmt::Bytes;
use check_console::markup;
use check_css_parser::ParseDiagnostic;
use check_diagnostics::{
    Advices, Category, Diagnostic, DiagnosticTags, Location, LogCategory, MessageAndDescription,
    Severity, Visit, category,
};
use check_formatter::{FormatError, PrintError};
use check_fs::{CheckPath, FileSystemDiagnostic};
use check_grit_patterns::CompileError;
use check_js_analyze::utils::rename::RenameError;
use check_plugin_loader::PluginDiagnostic;
use camino::Utf8Path;
...
/// Generic errors thrown during check operations
#[derive(Deserialize, Diagnostic, Serialize)]
pub enum WorkspaceError {
    /// Thrown when Check can't read a generic file.
    ...
    /// Error thrown when Check cannot rename a symbol.
    ...
    ///
    /// Use this error if Check is trying to process a file that Check can't
    /// understand.
...
impl From<CheckDiagnostic> for WorkspaceError {
    fn from(value: CheckDiagnostic) -> Self {
...
impl From<CantLoadExtendFile> for WorkspaceError {
    fn from(value: CantLoadExtendFile) -> Self {
        Self::Configuration(CheckDiagnostic::CantLoadExtendFile(value).into())
    }
}

impl From<CantResolve> for WorkspaceError {
    fn from(value: CantResolve) -> Self {
        Self::Configuration(CheckDiagnostic::CantResolve(value).into())
    }
}

impl From<WorkspaceError> for check_diagnostics::serde::Diagnostic {
...
#[diagnostic(
     ...
     severity = Error,
     message(
         description = "Check does not support non-UTF8 characters in path: {path}",
         message("Check does not support non-UTF8 characters in path: "<Emphasis>{self.path}</Emphasis>)
     ),
 )]
...
impl From<Panic> for check_diagnostics::serde::Diagnostic {
...
#[diagnostic(
    category = "internalError/fs",
    message(
        message("Check couldn't read the following file, maybe for permissions reasons or it doesn't exist: "{self.path}),
        description = "Check couldn't read the following file, maybe for permissions reasons or it doesn't exist: {path}"
    )
)]
...
#[diagnostic(
    ...
    message(
        message(
            "Check attempted to perform an operation on the registered project, but no project was registered. This is a bug in Check. If this problem persists, please report here: https://github.com/checkjs/check/issues/"
        ),
        description = "Check attempted to perform an operation on the registered project, but no project was registered. This is a bug in Check. If this problem persists, please report here: https://github.com/checkjs/check/issues/"
    )
)]
...
impl Diagnostic for SourceFileNotSupported {
    ...
    fn message(&self, fmt: &mut check_console::fmt::Formatter<'_>) -> std::io::Result<()> {
        if self.file_source != DocumentFileSource::Unknown {
            fmt.write_markup(markup! {
                "Check doesn't support this feature for the language "<Emphasis>{{&self.file_source}}</Emphasis>
            })
        } else if let Some(ext) = self.extension.as_ref() {
            fmt.write_markup(markup! {
                "Check could not determine the language for the file extension "<Emphasis>{{ext}}</Emphasis>
            })
        } else {
            fmt.write_markup(
                markup!{
                    "Check could not determine the language for the file"<Emphasis>{self.path}</Emphasis>" because it doesn't have a clear extension"
                }
            )
        }
    }
    ...
    fn description(&self, fmt: &mut Formatter<'_>) -> fmt::Result {
        if self.file_source != DocumentFileSource::Unknown {
            write!(
                fmt,
                "Check doesn't support this feature for the language {}",
                &self.file_source
            )
        } else if let Some(ext) = self.extension.as_ref() {
            write!(
                fmt,
                "Check could not determine the language for the file extension {ext}"
            )
        } else {
            write!(
                fmt,
                "Check could not determine the language for the file {} because it doesn't have a clear extension",
                &self.path
            )
        }
    }
...
#[diagnostic(
    category = "search",
    message(
        message("Invalid pattern -- this is a bug in Check."),
        description = "If this problem persists, please report here: https://github.com/checkjs/check/issues/"
    )
)]
...
impl Diagnostic for QueryDiagnostic {
    ...
    fn verbose_advices(&self, visitor: &mut dyn Visit) -> std::io::Result<()> {
        visitor.record_log(
            LogCategory::Info,
            &markup! { "Please consult "<Hyperlink href="https://checkjs.dev/reference/gritql">"our GritQL reference"</Hyperlink>"." }
        )
    }
}

pub fn extension_error(path: &CheckPath) -> WorkspaceError {
...
#[diagnostic(
    ...
    severity = Error,
    message(
        description = "Check couldn't find an ignore file in the following folder: {path}",
        message("Check couldn't find an ignore file in the following folder: "<Emphasis>{self.path}</Emphasis>),
    )
)]
...
#[diagnostic(
    category = "internalError/fs",
    severity = Warning,
    message = "Check couldn't determine a folder for the VCS integration. VCS integration will be disabled."
)]
...
#[diagnostic(
    ...
    severity = Information,
    message(
        message("The file "<Emphasis>{self.file_path}</Emphasis>" is protected because is handled by another tool. Check won't process it."),
        description = "The file {file_path} is protected because is handled by another tool. Check won't process it.",
    ),
    tags(VERBOSE)
)]
...
#[diagnostic(
    ...
    severity = Error,
    message(
        message("Check cannot watch files on disk: "<Info>{self.reason}</Info>),
        description = "Check cannot watch files on disk: {reason}",
    ),
)]
...
mod test {
    ...
    use check_diagnostics::{DiagnosticExt, Error, print_diagnostic_to_string};
    use check_formatter::FormatError;
    use check_fs::CheckPath;
    use check_module_graph::{JsModuleInfoDiagnostic, ModuleDiagnostic};
    use std::ffi::OsString;
    ...
    fn source_file_not_supported() {
        let path = CheckPath::new("not_supported.toml");
        snap_diagnostic(
