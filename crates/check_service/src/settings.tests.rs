...
use check_analyze::RuleFilter;
use check_configuration::analyzer::{GroupPlainConfiguration, Nursery, SeverityOrGroup};
use check_configuration::javascript::JsxRuntime;
use check_configuration::json::{JsonAssistConfiguration, JsonLinterConfiguration};
...
fn correctly_computes_analyzer_options() {
    ...
    assert_eq!(
        options.jsx_runtime(),
        Some(check_analyze::options::JsxRuntime::ReactClassic)
    );
}
    ...
    let configuration = Configuration {
        ...
        overrides: Some(Overrides(vec![OverridePattern {
            includes: Some(OverrideGlobs::Globs(Box::new([
                check_glob::NormalizedGlob::from_str("**/*").unwrap(),
            ]))),
