//! The [Workspace] is the main entry point for high-level clients (the Check
//! CLI and Language Server) into the various language-specific services of the
//! Check toolchain (parser, formatter, analyzer ...)
//!
//! # Documents
//!
...
use check_analyze::{ActionCategory, RuleCategories};
use check_configuration::{Configuration, analyzer::RuleSelector};
use check_console::{Markup, MarkupBuf, markup};
use check_diagnostics::{CodeSuggestion, serde::Diagnostic};
use check_formatter::Printed;
use check_fs::CheckPath;
use check_grit_patterns::GritTargetLanguage;
use check_js_syntax::{TextRange, TextSize};
use check_module_graph::SerializedJsModuleInfo;
use check_resolver::FsWithResolverProxy;
use check_text_edit::TextEdit;
use camino::Utf8Path;
...
pub struct UpdateSettingsParams {
    ...
    pub workspace_directory: Option<CheckPath>,
}
...
pub struct PullDiagnosticsResult {
    pub diagnostics: Vec<check_diagnostics::serde::Diagnostic>,
    pub errors: usize,
...
pub enum FixFileMode {
    /// Applies [safe](check_diagnostics::Applicability::Always) fixes
    SafeFixes,
    /// Applies [safe](check_diagnostics::Applicability::Always) and [unsafe](check_diagnostics::Applicability::MaybeIncorrect) fixes
    SafeAndUnsafeFixes,
...
pub struct PathIsIgnoredParams {
    ...
    /// When this field is empty, Check checks only `files.includes`.
    pub features: FeatureName,
    #[serde(default)]
...
pub struct FileExitsParams {
    pub file_path: CheckPath,
}

impl From<CheckPath> for FileExitsParams {
...
pub trait Workspace: Send + Sync + RefUnwindSafe {
    ...
    /// the Check project, but it may not have to be the root itself. The first
    /// ancestor directory of the path (including the path itself) that contains
    /// a **top-level** `check.json` is determined to be the project root and
    /// will be used for opening the project.
    ///
    /// If no `check.json` file can be found in any of the ancestors, and
    /// `open_uninitialized` is `true` the directory will be opened as a Check
    ...
    /// invocations to Check can reuse the loaded settings and open files. Only
    ...
    /// - Check doesn't recognize a file, so it can't provide the feature;
