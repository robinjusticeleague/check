...
use check_fs::{ConfigName, FileSystem};
...
/// ## Terminology
///
/// Every project within a Check workspace correlates with a single
/// **top-level** `check.json`. This means that if the `check.json` is at the
...
impl Projects {
    ...
    pub fn get_file_features(
        ...
        {
            // Never ignore Check's top-level config file
