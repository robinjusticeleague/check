...
use check_analyze::{AnalyzerOptions, AnalyzerRules};
use check_configuration::analyzer::assist::{Actions, AssistConfiguration, AssistEnabled};
use check_configuration::analyzer::{LinterEnabled, RuleDomains};
use check_configuration::bool::Bool;
use check_configuration::diagnostics::InvalidIgnorePattern;
use check_configuration::formatter::{FormatWithErrorsEnabled, FormatterEnabled};
use check_configuration::html::HtmlConfiguration;
use check_configuration::javascript::JsxRuntime;
use check_configuration::max_size::MaxSize;
use check_configuration::plugins::Plugins;
use check_configuration::vcs::{VcsClientKind, VcsConfiguration, VcsEnabled, VcsUseIgnoreFile};
use check_configuration::{
    CheckDiagnostic, Configuration, CssConfiguration, FilesConfiguration,
...
use check_css_formatter::context::CssFormatOptions;
...
use check_graphql_formatter::context::GraphqlFormatOptions;
use check_graphql_syntax::GraphqlLanguage;
use check_grit_formatter::context::GritFormatOptions;
use check_grit_syntax::GritLanguage;
use check_html_formatter::HtmlFormatOptions;
...
const DEFAULT_SCANNER_IGNORE_ENTRIES: &[&[u8]] = &[
    ...
    b".vercel",
    b".DS_Store",
    // TODO: Remove when https://github.com/checkjs/check/issues/6172 is fixed.
...
pub trait ServiceLanguage: check_rowan::Language {
    ...
    type FormatOptions: check_formatter::FormatOptions + Clone + std::fmt::Display + Default;
...
pub struct FilesSettings {
    ...
    /// Files not recognized by Check should not emit a diagnostic
    ...
    fn git_ignore(path: &Utf8Path, patterns: &[&str]) -> Result<Gitignore, WorkspaceError> {
        ...
        for the_match in patterns {
            gitignore_builder
                ...
                .map_err(|err| {
                    CheckDiagnostic::InvalidIgnorePattern(InvalidIgnorePattern {
...
pub struct Includes {
    ...
    globs: Option<Vec<check_glob::NormalizedGlob>>,
}
impl Includes {
    fn new(
        working_directory: Option<Utf8PathBuf>,
    ...
    pub fn store_globs(&mut self, globs: impl Into<Box<[check_glob::NormalizedGlob]>>) {
    ...
    pub fn matches_with_exceptions(&self, path: &Utf8Path) -> bool {
        ...
        let candidate_path = check_glob::CandidatePath::new(path);
...
impl OverrideSettings {
    ...
    pub fn override_jsx_runtime(&self, path: &CheckPath, base_setting: JsxRuntime) -> JsxRuntime {
        self.patterns
            .iter()
            // Reverse the traversal as only the last override takes effect
            .rev()
            ...
            if pattern.is_file_included(path) {
                if let Some(rules) = pattern.linter.rules.as_ref() {
                    push_to_analyzer_rules(
                        rules,
                        check_js_analyze::METADATA.deref(),
                        &mut analyzer_rules,
                    );
                    push_to_analyzer_rules(
                        rules,
                        check_json_analyze::METADATA.deref(),
                        &mut analyzer_rules,
                    );
                    push_to_analyzer_rules(
                        rules,
                        check_css_analyze::METADATA.deref(),
                        &mut analyzer_rules,
                    );
                    push_to_analyzer_rules(
                        rules,
                        check_graphql_analyze::METADATA.deref(),
                        &mut analyzer_rules,
                    );
                }
