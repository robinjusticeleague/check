use super::*;
use check_js_syntax::{AnyJsFunction, AnyJsRoot};
...
impl SemanticModel {
    ...
    /// use check_rowan::{AstNode, SyntaxNodeCast};
    /// use check_js_syntax::{JsFileSource, JsReferenceIdentifier};
    /// use check_js_semantic::{semantic_model, SemanticModelOptions, SemanticScopeExtensions};
    ///
    /// let r = check_js_parser::parse("function f(){let a = arguments[0]; let b = a + 1;}", JsFileSource::js_module(), JsParserOptions::default());
    ...
    /// use check_rowan::{AstNode, SyntaxNodeCast};
    /// use check_js_syntax::{JsFileSource, JsReferenceIdentifier};
    /// use check_js_semantic::{semantic_model, BindingExtensions, SemanticModelOptions};
    ///
    /// let r = check_js_parser::parse("function f(){let a = arguments[0]; let b = a + 1;}", JsFileSource::js_module(), JsParserOptions::default());
    /// let model = semantic_model(&r.tree(), SemanticModelOptions::default());
    ///
    ...
    /// use check_js_parser::JsParserOptions;
    /// use check_rowan::{AstNode, SyntaxNodeCast};
    /// use check_js_syntax::{JsFileSource, AnyJsFunction};
    /// use check_js_semantic::{semantic_model, CallsExtensions, SemanticModelOptions};
    ///
    /// let r = check_js_parser::parse("function f(){} f() f()", JsFileSource::js_module(), JsParserOptions::default());
    /// let model = semantic_model(&r.tree(), SemanticModelOptions::default());
    ///
    /// let f_declaration = r
    ///     .syntax()
