...
use check_js_syntax::{AnyJsRoot, JsFileSource, JsSyntaxToken, TextRange, TextSize, WalkEvent};
use check_rowan::{AstNode, NodeOrToken};
...
pub fn assert(code: &str, test_name: &str) {
    let r = check_js_parser::parse(code, JsFileSource::tsx(), JsParserOptions::default());
...
impl SemanticAssertions {
    fn from_root(root: AnyJsRoot, code: &str, test_name: &str) -> Self {
        ...
        for node in root
            .syntax()
            .preorder_with_tokens(check_rowan::Direction::Next)
        {
