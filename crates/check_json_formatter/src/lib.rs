...
use check_json_syntax::{AnyJsonValue, JsonLanguage, JsonSyntaxNode, JsonSyntaxToken};
use check_rowan::{AstNode, SyntaxNode, TextRange};
...
pub fn format_range(
    ...
    check_formatter::format_range(root, range, JsonFormatLanguage::new(options))
}
...
pub fn format_node(
    ...
    check_formatter::format_node(root, JsonFormatLanguage::new(options))
}
...
pub fn format_sub_tree(options: JsonFormatOptions, root: &JsonSyntaxNode) -> FormatResult<Printed> {
    check_formatter::format_sub_tree(root, JsonFormatLanguage::new(options))
}
