...
This document will explain how to contribute to creating a new Parser with Check.
...
1. Create two new crates: `check_html_syntax` and `check_html_factory`. Use `cargo new --lib crates/check_html_syntax`.
...
### Conventions when writing a new grammar in Check
...
There are generally three actors you'll need to have in order to create a parser in Check:
...
The lexer is the lower primitive of a Check parser. It usually consumes characters as bytes, but it can also consume characters with different encodings, e.g., UTF-8, UTF-16, etc.

Create a `lexer/mod.rs` inside the parser crate.

```rust,ignore
use check_beta_syntax::BetaSyntaxKind;
use check_parser::ParseDiagnostic;
...
The import and implementation of the `Lexer` trait from the `check_parser` crate.
...
```rust,ignore
use check_parser::lexer::BufferedLexer;
use check_beta_syntax::BetaSyntaxKind;
...
A parser is a `struct` that must implement the [`Parser`](https://docs.rs/check_parser/latest/check_parser/trait.Parser.html) trait.
...
This is a short, or not so short, guide to implement parse rules using the Check parser infrastructure.
...
fn parse_if_statement(p: &mut Parser) -> ParsedSyntax {
 ...
 p.expect(T![if]);
 p.expect(T!['(']);
 parse_any_expression(p).or_add_diagnostic(p, js_parse_error::expected_if_statement);
 p.expect(T![')']);
 parse_block_statement(p).or_add_diagnostic(p, js_parse_error::expected_block_statement);
...
Hold on, what are these *missing* markers? Check's AST facade uses fixed offsets to retrieve a particular child from a node.
