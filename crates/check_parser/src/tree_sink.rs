    ...
    Factory: SyntaxFactory<Kind = L::Kind>,
{
    ...
    ///
    /// If tree is finished without a [check_rowan::SyntaxKind::EOF], one will be generated and all pending trivia
    ...
    Factory: SyntaxFactory<Kind = L::Kind>,
{
    ...
    pub fn finish(self) -> (check_rowan::SyntaxNodeWithOffset<L>, Vec<ParseDiagnostic>) {
        let (node, diagnostics) = self.inner.finish();
        let offset_node = check_rowan::SyntaxNodeWithOffset::new(node, self.base_offset);
