...
use check_formatter::{FormatLanguage, FormatOptions};
...
const CHECK_IGNORE: &str = "check-ignore format: prettier ignore";
...
impl<'a> PrettierTestFile<'a> {
    pub fn new(input: &'static str, root_path: &'a Utf8Path) -> Self {
        ...
        let parse_input = input_code.replace(PRETTIER_IGNORE, CHECK_IGNORE);
    ...
    L: TestFormatLanguage,
{
    ...
    fn formatted(&self, parsed: &AnyParse) -> Option<String> {
        ...
        let formatted = formatted.replace(CHECK_IGNORE, PRETTIER_IGNORE);
