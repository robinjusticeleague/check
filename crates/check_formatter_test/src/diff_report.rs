...
impl DiffReport {
            ...
            Ok(value) if value == "1" => {
                if !Self::is_ignored(file_name) {
                    self.state.lock().unwrap().push(DiffReportItem {
                        file_name,
                        check_formatted_result: check_formatted_result.to_owned(),
    ...
    fn report_prettier(
        ...
        for DiffReportItem {
            file_name,
            check_formatted_result,
            prettier_formatted_result,
        } in state.iter()
        {
            file_count += 1;

            let check_lines = check_formatted_result.lines().count();
            let prettier_lines = prettier_formatted_result.lines().count();

            let (matched_lines, ratio, diff) =
                if check_formatted_result == prettier_formatted_result {
                    (check_lines, 1f64, None)
                } else {
                    ...
                    let ratio = matched_lines as f64 / check_lines.max(prettier_lines) as f64;

                    (matched_lines, ratio, Some(diff))
                };

            total_lines += check_lines.max(prettier_lines);
