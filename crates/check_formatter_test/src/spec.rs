...
use check_console::EnvConsole;
use check_deserialize::json::deserialize_from_str;
use check_diagnostics::print_diagnostic_to_string;
use check_formatter::{FormatLanguage, FormatOptions, Printed};
...
impl<'a> SpecTestFile<'a> {
    pub fn try_from_file(
        ...
        let mut input_file = CheckPath::new(file_path);
    ...
    pub fn input_file(&self) -> &CheckPath {
        &self.input_file
    }
    ...
    L: TestFormatLanguage,
{
    ...
    pub fn test(self) {
        ...
        if options_path.exists() {
            let mut options_path = CheckPath::new(&options_path);
