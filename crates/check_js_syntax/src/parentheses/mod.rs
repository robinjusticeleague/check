...
//!
//! Check is opinionated about which parentheses to keep or where to insert parentheses.
//! It removes parentheses that aren't necessary to keep the original semantic and to improve readability.
//! Check also inserts parentheses around nodes where we think this improves readability.
//!
//! The [NeedsParentheses] trait forms the foundation of Check's parentheses formatting.
...
//! Tests are available in the `check_js_formatter` crate.
//!
...
//!
//! There are two measures taken by Check to ensure formatting is stable regardless of the number of parenthesized nodes in a tree:
//!
//! ## Removing parenthesized nodes
//!
//! The JavaScript formatter pre-processes (See `check_js_formatter::JsFormatSyntaxRewriter`)
...
pub trait NeedsParentheses: check_rowan::AstNode<Language = crate::JsLanguage> {
