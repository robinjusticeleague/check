...
use check_rowan::{AstNode, AstNodeList, SyntaxResult, TokenText, declare_node_union};
...
impl JsxString {
    ...
    /// use check_rowan::TriviaPieceKind;
    ///
...
impl JsxOpeningElement {
    ...
    ///
    /// use check_js_factory::make;
    /// use check_js_factory::make::{ident, jsx_attribute, jsx_name, jsx_opening_element, token, jsx_attribute_list};
    /// use check_js_syntax::{AnyJsxAttribute, AnyJsxAttributeName, AnyJsxElementName, T};
    ///
    ...
    /// ```
    /// use check_js_factory::make;
    /// use check_js_factory::make::{ident, jsx_attribute, jsx_name, jsx_opening_element, token, jsx_attribute_list, jsx_self_closing_element, jsx_spread_attribute, jsx_ident, js_identifier_expression, js_reference_identifier};
    /// use check_js_syntax::{AnyJsExpression, AnyJsxAttribute, AnyJsxAttributeName, AnyJsxElementName, T};
    ///
...
impl JsxSelfClosingElement {
    ...
    ///
    /// use check_js_factory::make;
    /// use check_js_factory::make::{ident, jsx_attribute, jsx_name, jsx_opening_element, token, jsx_attribute_list, jsx_self_closing_element};
    /// use check_js_syntax::{AnyJsxAttribute, AnyJsxAttributeName, AnyJsxElementName, T};
    ///
    /// let div = AnyJsxAttribute::JsxAttribute(jsx_attribute(
    ///     AnyJsxAttributeName::JsxName(
...
impl AnyJsxElementName {
    pub fn name_value_token(&self) -> SyntaxResult<JsSyntaxToken> {
        match self {
            Self::JsxMemberName(member) => member.member()?.value_token(),
            Self::JsxName(name) => name.value_token(),
            Self::JsxNamespaceName(name) => name.name()?.value_token(),
            Self::JsxReferenceIdentifier(name) => name.value_token(),
...
impl AnyJsxElement {
    ...
    /// use check_js_factory::make::{ident, js_boolean_literal_expression, jsx_attribute, jsx_attribute_initializer_clause, jsx_attribute_list, jsx_expression_attribute_value, jsx_name, jsx_self_closing_element, jsx_string, jsx_string_literal, token};
    /// use check_js_syntax::{AnyJsExpression, AnyJsLiteralExpression, AnyJsxAttribute, AnyJsxAttributeName, AnyJsxAttributeValue, AnyJsxElementName, T};
    ///
...
impl check_aria::Element for AnyJsxElement {
    ...
    fn attributes(&self) -> impl Iterator<Item = impl check_aria::Attribute> {
...
impl check_aria::Attribute for JsxAttribute {
    fn name(&self) -> Option<impl AsRef<str>> {
        Self::name(self).ok().and_then(|name| {
...
impl AnyJsxChild {
    ...
    pub fn is_accessible_node(&self) -> Option<bool> {
        Some(match self {
            ...
            Self::JsxExpressionChild(expression) => {
