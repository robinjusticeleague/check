...
use check_js_semantic::{SemanticEvent, SemanticEventExtractor};
use check_js_syntax::{
...
use check_rowan::{AstNode, Text, TextRange, TextSize, TokenText};
...
pub(super) struct JsModuleInfoCollector {
    ...
    ///
    /// Re-used from the semantic model in `check_js_semantic`.
...
impl JsModuleInfoCollector {
    ...
    pub fn leave_node(&mut self, node: &JsSyntaxNode) {
        ...
        if let Some(expr) = AnyJsExpression::cast_ref(node) {
            ...
            self.variable_declarations
                .insert(decl.syntax().clone(), type_bindings);
        } else if let Some(import) = check_js_syntax::JsImport::cast_ref(node) {
            self.push_static_import(import);
        }
    }

    fn push_static_import(&mut self, node: check_js_syntax::JsImport) -> Option<()> {
...
fn find_jsdoc(node: &JsSyntaxNode) -> Option<JsdocComment> {
    match node.ancestors().find_map(check_js_syntax::JsExport::cast) {
