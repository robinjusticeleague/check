...
use check_js_type_info::{ImportSymbol, ScopeId, TypeData, TypeReference, TypeResolver};
use check_jsdoc_comment::JsdocComment;
use check_resolver::{ResolveOptions, resolve};
use check_rowan::{AstNode, TokenText, WalkEvent};
use camino::Utf8Path;
        ...
        for event in iter {
            match event {
                WalkEvent::Enter(node) => {
                    ...
                    } else if let Some(export) = check_js_syntax::JsExport::cast_ref(&node) {
