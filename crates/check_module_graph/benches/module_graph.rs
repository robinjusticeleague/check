use check_fs::{CheckPath, FileSystem, MemoryFileSystem};
use check_js_parser::JsParserOptions;
use check_js_syntax::{AnyJsRoot, JsFileSource};
...
const INDEX_D_TS_CASES: &[(&str, &[u8])] = &[
    (
        "react/index.d.ts",
        include_bytes!(
            "../../check_resolver/tests/fixtures/resolver_cases_5/node_modules/@types/react/index.d.ts"
        ) as &[u8],
    ),
    (
...
fn get_js_root(fs: &dyn FileSystem, path: &CheckPath) -> AnyJsRoot {
    ...
    let parsed = check_js_parser::parse(&content, file_source, JsParserOptions::default());
