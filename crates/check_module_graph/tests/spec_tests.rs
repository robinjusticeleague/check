...
use check_fs::{CheckPath, FileSystem, MemoryFileSystem, OsFileSystem, normalize_path};
use check_js_type_info::{ScopeId, TypeData, TypeResolver};
use check_jsdoc_comment::JsdocComment;
use check_json_parser::{JsonParserOptions, parse_json};
use check_json_value::{JsonObject, JsonString};
use check_module_graph::{
    ImportSymbol, JsExport, JsImport, JsImportPath, JsImportPhase, JsReexport, ModuleGraph,
    ModuleResolver, ResolvedPath,
};
use check_package::{Dependencies, PackageJson};
use check_project_layout::ProjectLayout;
use check_rowan::Text;
use check_test_utils::get_added_paths;
...
fn get_fixtures_path() -> Utf8PathBuf {
    ...
    path.join("crates/check_module_graph/tests/fixtures")
}
...
fn test_resolve_relative_import() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/src/bar.ts"),
    ];
...
fn test_resolve_package_import() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/node_modules/shared/dist/index.js"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_import_through_path_alias() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/src/components/Hello.tsx"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_package_import_in_monorepo_fixtures() {
    ...
    let added_paths = [
        CheckPath::new(format!("{fixtures_path}/frontend/src/bar.ts")),
        CheckPath::new(format!("{fixtures_path}/frontend/src/index.ts")),
        CheckPath::new(format!(
            "{fixtures_path}/frontend/node_modules/shared/dist/index.js"
        )),
        CheckPath::new(format!("{fixtures_path}/shared/dist/index.js")),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_export_referenced_function() {
    ...
    let added_paths = [CheckPath::new("/src/index.ts")];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_export_const_type_declaration_with_namespace() {
    ...
    let added_paths = [CheckPath::new("/src/index.d.ts")];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_exports() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/src/reexports.ts"),
        CheckPath::new("/src/renamed-reexports.ts"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
"#,
    ...
    let added_paths = [CheckPath::new("/node_modules/@types/iso-3166-2/index.d.ts")];
    let added_paths = get_added_paths(&fs, &added_paths);
...
"#,
    ...
    let added_paths = [CheckPath::new("/node_modules/vfile/types/index.d.ts")];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_react_types() {
    ...
    fs.insert(
        ...
        include_bytes!("../../check_resolver/tests/fixtures/resolver_cases_5/node_modules/@types/react/index.d.ts")
    );
    fs.insert(
        "/src/index.ts".into(),
    ...
    let added_paths = [
        CheckPath::new("/node_modules/@types/react/index.d.ts"),
        CheckPath::new("/src/index.ts"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_redis_commander_types() {
    ...
    let added_paths = [
        CheckPath::new("/RedisCommander.d.ts"),
        CheckPath::new("/index.ts"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_type_of_union_from_imported_module() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/src/reexport.ts"),
        CheckPath::new("/node_modules/react.d.ts"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_export_type_referencing_imported_type() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/src/promisedResult.ts"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_promise_from_imported_function_returning_imported_promise_type() {
    ...
    let added_paths = [
        CheckPath::new("/src/index.ts"),
        CheckPath::new("/src/promisedResult.ts"),
        CheckPath::new("/src/returnPromiseResult.ts"),
    ];
    let added_paths = get_added_paths(&fs, &added_paths);
...
fn test_resolve_swr_types() {
    ...
    let mut added_paths = vec![CheckPath::new(format!(
        "{fixtures_path}/frontend/src/index.ts"
    ))];
    for path in find_files_recursively_in_directory(&swr_path, |path| {
        path.extension().is_some_and(|ext| ext != "json")
    }) {
        added_paths.push(CheckPath::new(path));
    }
    let added_paths = get_added_paths(&fs, &added_paths);
