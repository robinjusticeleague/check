use check_diagnostics_categories::category;
use check_formatter::{
    FormatRule,
    comments::{
...
use check_html_syntax::{HtmlClosingElement, HtmlLanguage, HtmlOpeningElement, HtmlSyntaxKind};
use check_rowan::{SyntaxNodeCast, SyntaxTriviaPieceComments};
...
impl CommentStyle for HtmlCommentStyle {
    ...
    ///
    /// While every comment is directly attached to a **syntax token**, Check actually builds a map of comments to **syntax nodes** separately. This map lives in [`HtmlComments`]. This is so that we can easily look up comments that are associated with a specific node. It's part of how suppression comments are handled.
    ///
    ...
    fn place_comment(
        ...
        // ```html
        // 123<!--check-ignore format: prettier ignore-->456
        // ```
