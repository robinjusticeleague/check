...
use check_html_syntax::{HtmlElement, HtmlElementFields};
...
impl FormatNodeRule<HtmlElement> for FormatHtmlElement {
    ...
    fn fmt_trailing_comments(&self, node: &HtmlElement, f: &mut HtmlFormatter) -> FormatResult<()> {
        ...
        // See prettier test case: crates/check_html_formatter/tests/specs/prettier/html/comments/hidden.html
        // The current implementation for `check_formatter::FormatTrailingComments` actually has a ton of js specific behavior that we don't want in the html formatter.
