...
use check_formatter::comments::{CommentKind, SourceComment};
use check_formatter::format_element::tag::VerbatimKind;
...
use check_html_syntax::{HtmlLanguage, HtmlSyntaxNode};
use check_rowan::{Direction, SyntaxElement, TextRange};
...
/// has the risk that Check misinterprets the structure of the code and formatting it could
...
impl<'a> Format<HtmlFormatContext> for FormatLHtmlLeadingComments<'a> {
    fn fmt(&self, f: &mut HtmlFormatter) -> FormatResult<()> {
        ...
        for comment in leading_comments_iter {
            ...
            check_formatter::write!(f, [format_comment])?;
                    ...
                    match comment.lines_after() {
                        ...
                        1 => {
                            if comment.lines_before() == 0 {
                                check_formatter::write!(f, [soft_line_break_or_space()])?;
                            } else {
                                check_formatter::write!(f, [hard_line_break()])?;
                            }
                        }
                        _ => check_formatter::write!(f, [empty_line()])?,
                    }
                }
            }
    ...
    fn fmt(&self, f: &mut HtmlFormatter) -> FormatResult<()> {
        ...
        for comment in trailing_comments {
            ...
            if total_lines_before > 0 {
                            ...
                            match comment.lines_before() {
                                ...
                                0 => {
                                    ...
                                    if previous_comment.is_some_and(|previous_comment| {
                                        ...
                                        check_formatter::write!(f, [space()])?;
                                    }
                                }
                                1 => check_formatter::write!(f, [hard_line_break()])?,
                                _ => check_formatter::write!(f, [empty_line()])?,
                            };
                ...
                let content = format_with(|f| check_formatter::write!(f, [format_comment]));
                if comment.kind().is_line() {
                    check_formatter::write!(f, [line_suffix(&content), expand_parent()])?;
                } else {
                    check_formatter::write!(f, [content])?;
                }
            }
