use check_configuration::{Configuration, HtmlConfiguration, html::HtmlFormatterConfiguration};
use check_formatter_test::spec::{SpecSnapshot, SpecTestFile};
use check_fs::CheckPath;
use check_html_formatter::{HtmlFormatLanguage, context::HtmlFormatOptions};
use check_html_syntax::HtmlFileSource;
use check_service::workspace::UpdateSettingsParams;
use camino::Utf8Path;
...
pub fn run(spec_input_file: &str, _expected_file: &str, test_directory: &str, _file_type: &str) {
    ...
    let Some(test_file) = SpecTestFile::try_from_file(spec_input_file, root_path, |project_key| {
        Some(UpdateSettingsParams {
            ...
            project_key,
            workspace_directory: Some(CheckPath::new(test_directory)),
        })
    }) else {
