//! `check_glob` provides globbing functionality. When listing the globs to match,
...
//!
//! Check globs are case-sensitive. This means that `lib.RS` doesn't match `*.rs`.
//!
...
//! A Check glob supports the following patterns:
//!
...
//!
//! ```
//! let glob: check_glob::Glob = "*.rs".parse().expect("correct glob");
...
//!
//! ```
//! let normalized_glob: check_glob::NormalizedGlob = "./*.rs".parse().expect("correct glob");
...
//!
//! ```
//! use check_glob::{CandidatePath, Glob};
//!
//! let globs: &[Glob] = &[
...
//! Check provides a dedicated method [CandidatePath::matches_directory_with_exceptions] for this purpose.
...
#[cfg_attr(
    feature = "check_deserialize",
    derive(check_deserialize_macros::Deserializable)
)]
...
impl NormalizedGlob {
    ...
    ///
    /// ```
    /// let glob = "!*.js".parse::<check_glob::NormalizedGlob>().unwrap();
...
impl Glob {
    ...
    ///
    /// ```
    /// let glob = "!*.js".parse::<check_glob::Glob>().unwrap();
...
#[cfg(feature = "check_deserialize")]
impl check_deserialize::Deserializable for Glob {
    fn deserialize(
        ...
        match glob.parse() {
            ...
            Err(error) => {
                ...
                let range = error.index().map_or(range, |index| {
                    check_text_size::TextRange::at(
                        range.start() + check_text_size::TextSize::from(1 + index),
                        1u32.into(),
                    )
                });
                ctx.report(
                    check_deserialize::DeserializationDiagnostic::new(format_args!("{error}"))
                        .with_range(range),
                );
                None
            }
