use check_deserialize::{Deserializable, DeserializationContext, Text};
use check_deserialize_macros::Deserializable;
use check_glob::{CandidatePath, Glob};
use check_resolver::is_builtin_node_module;
use check_string_case::comparable_token::ComparableToken;
...
impl Deserializable for ImportGroup {
    fn deserialize(
        ...
        match value.visitable_type() {
            Some(check_deserialize::DeserializableType::Array) => {
                Deserializable::deserialize(ctx, value, name).map(Self::MatcherList)
            }
            Some(check_deserialize::DeserializableType::Map) => {
...
impl Deserializable for GroupMatcher {
    fn deserialize(
        ...
        if value.visitable_type() == Some(check_deserialize::DeserializableType::Map) {
...
impl Deserializable for SourcesMatcher {
    fn deserialize(
        ...
        if value.visitable_type() == Some(check_deserialize::DeserializableType::Array) {
            Deserializable::deserialize(ctx, value, name).map(Self::MatcherList)
        } else {
...
impl check_deserialize::Deserializable for SourceMatcher {
    fn deserialize(
        ctx: &mut impl DeserializationContext,
        value: &impl check_deserialize::DeserializableValue,
        name: &str,
    ) -> Option<Self> {
        let text = check_deserialize::Text::deserialize(ctx, value, name)?;
