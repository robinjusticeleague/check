...
use check_deserialize::{DeserializationContext, TextRange};
...
impl check_deserialize::DeserializableValidator for FilenameCases {
    fn validate(
        &mut self,
        ...
        if !self.allow_export && self.cases.is_empty() {
            ctx.report(
                check_deserialize::DeserializationDiagnostic::new(markup! {
