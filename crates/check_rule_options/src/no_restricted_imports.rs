...
use check_rowan::{AstNode, AstSeparatedList, SyntaxNode, SyntaxNodeCast, SyntaxToken, TokenText};
...
impl Deserializable for Paths {
    fn deserialize(
        ...
        if value.visitable_type()? == DeserializableType::Str {
            check_deserialize::Deserializable::deserialize(ctx, value, name).map(Self::Plain)
        } else {
            check_deserialize::Deserializable::deserialize(ctx, value, name).map(Self::WithOptions)
        }
    }
}
    ...
    if let Some(bindings) = get_context_for_import_call(import_call) {
        match bindings {
            AnyJsBindingPattern::AnyJsBinding(namespace_binding) => match namespace_binding {
                // const ... = import(...)
                check_js_syntax::AnyJsBinding::JsIdentifierBinding(namespace_binding) => {
